...
AGENT_IMPORT = "harbor_kura_orchestra:KuraOrchestraAgent"
PROVISIONER_FACTORY = "harbor_kura_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Kura relay/Postgres provisioner",
    )
    parser.add_argument(
        "--kura-bin-dir",
        type=Path,
...
def build_command(
    ...
    kwargs = {
        ...
        "kura_acp_binary": agent_binaries["kura-acp"],
        "kura_agent_binary": agent_binaries["kura-agent"],
        "kura_dev_mcp_binary": agent_binaries["kura-dev-mcp"],
        "kura_cli_binary": binaries["kura"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        ...
        "agent_display_name": f"Kura Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.kura_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.kura_bin_dir)
