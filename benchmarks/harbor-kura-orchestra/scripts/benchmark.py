#!/usr/bin/env python3
"""One-command benchmark: bring up the Kura stack in Docker and run it.
...
Kura-native tasks use their ``evaluation_layer`` metadata: regression runs
...
- ``--gui`` adds that user to the relay membership list and opens the Kura
...
class KuraTask:
    """The identity and evaluation layer declared by one Kura task."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Kura evaluation layer to run (selected from task metadata)",
    )
    parser.add_argument(
        "--attempts",
        "-k",
        type=int,
        default=None,
        help="Runs per problem (default: Terminal-Bench 5, Kura regression 1, "
        "Kura workflow 3)",
    )
    parser.add_argument(
        "--manifest",
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open the Kura desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
...
def _read_kura_task(task_toml: Path) -> KuraTask:
    ...
    try:
        ...
        raise SystemExit(
            f"invalid Kura task metadata in {task_toml}: {error}"
        ) from error
    if not isinstance(name, str) or not name:
        raise SystemExit(f"invalid Kura task name in {task_toml}: expected a string")
    ...
    return KuraTask(name=name, layer=layer, path=task_toml.parent)


def kura_tasks_for_path(path: Path | None) -> tuple[KuraTask, ...] | None:
    """Return validated Kura tasks, or ``None`` for an unrelated problem set."""
    if path is None:
        return None
    ...
    if not task_files:
        raise SystemExit(f"no Kura tasks found under {path}")
    tasks = tuple(_read_kura_task(task_file) for task_file in task_files)
    names = [task.name for task in tasks]
    if len(names) != len(set(names)):
        raise SystemExit(f"duplicate Kura task names found under {path}")
    return tasks


def _matches_task(task: KuraTask, pattern: str) -> bool:
    return fnmatch.fnmatchcase(task.name, pattern) or fnmatch.fnmatchcase(
        task.path.name, pattern
    )


def select_kura_tasks(
    tasks: tuple[KuraTask, ...],
    *,
    layer: str | None,
    include: list[str],
    exclude: list[str],
    ...
    if not selected:
        ...
        raise SystemExit(f"no Kura tasks selected{detail}")
    return selected
...
def plan_benchmark_runs(
    ...
    tasks = kura_tasks_for_path(args.path)
...
def load_state() -> dict[str, str]:
    ...
    from harbor_kura_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_kura_testbed.keys import encode_nsec
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Kura desktop app logged in as the benchmark user.
    ...
    print(
        f"Opening Kura GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Kura Benchmark"}
    )
