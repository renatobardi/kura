"""Run the production Kura agent stack inside the Harbor task container.
...
PATH) that the desktop app gives a Kura agent. The harness stays outside:
...
from .evidence import build_kura_evidence
...
# Container-side layout for the uploaded Kura stack.
...
DELIVERY_RECEIPT_MARKER = "turn delivered Kura events for channel"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Kura agent process cannot be launched or exits early."""
...
class KuraContainerRuntime:
    """Launch one production Kura agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        kura_acp_binary: str = "kura-acp",
        kura_agent_binary: str = "kura-agent",
        kura_dev_mcp_binary: str = "kura-dev-mcp",
        kura_cli_binary: str = "kura",
        relay_gateway: str = "",
        ...
        self.kura_acp_binary = kura_acp_binary
        self.kura_agent_binary = kura_agent_binary
        self.kura_dev_mcp_binary = kura_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.kura_cli_binary = kura_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            # The task arrives exactly as it would in production Kura: a
        ...
        return RuntimeResult(
            metadata={
                ...
                "kura_evidence_exported": evidence_exported,
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/kura-acp": self.kura_acp_binary,
            f"{REMOTE_BIN}/kura-agent": self.kura_agent_binary,
            f"{REMOTE_BIN}/kura-dev-mcp": self.kura_dev_mcp_binary,
        }
    ...
    def _rust_log(configured: str | None) -> str:
        # ``kura_acp=info`` carries the subscription-readiness line; the turn
        ...
        required = "kura_acp=info,pool::prompt=info"
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._kura_json(
                trial.user,
                trial,
                "messages",
                "get",
    ...
    async def _turn_ended(environment: BaseEnvironment, agent: _Agent) -> bool:
        _, ends = await KuraContainerRuntime._turn_counts(environment, agent)
        return ends > 0
    ...
    async def _turn_counts(
        ...
        starts, ends, _ = await KuraContainerRuntime._turn_status(environment, agent)
        return starts, ends
    ...
    async def _turn_status(
        ...
        return KuraContainerRuntime._parse_turn_status(result.stdout or "")
    ...
    # -- Kura CLI as the trial user / provisioning identities -------------------
    ...
    async def _send(
        ...
        *,
        ...
        return await self._kura_json(credential, trial, *args)
    ...
    async def _send_scripted_messages(
        ...
        *,
        ...
        Messages are sent back-to-back so Kura's normal queueing and batching
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return KuraContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Kura workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Kura channel `{trial.channel_id}`.",
            (
