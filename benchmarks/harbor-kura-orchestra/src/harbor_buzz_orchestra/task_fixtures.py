"""Public setup declarations for Kura-native benchmark tasks."""
...
_CREATE_CHANNEL_FIXTURE = KuraTaskFixture(
    directory=tuple(
        [
...
_USER_MENTION_FIXTURE = KuraTaskFixture(
    user_display_name=USER_MENTION_DISPLAY_NAME,
    requires_evidence=True,
)

_NARRATIVE_AGENT_NAMES_FIXTURE = KuraTaskFixture(
    directory=(
...
_INTERLEAVED_AGENT_REPORTS_FIXTURE = KuraTaskFixture(
    directory=(
...
_CROSS_THREAD_REQUESTS_FIXTURE = KuraTaskFixture(
    scripted_messages=(
        ScriptedMessage(
            label="beta-request",
            actor="user",
...
_AMBIGUOUS_USER_MENTION_FIXTURE = KuraTaskFixture(
    directory=(
        DirectoryEntry(
            "Taylor Morgan Lee",
            "user",
...
_FIXTURES = {
    ...
    REPLY_TO_THREAD_TASK: KuraTaskFixture(requires_evidence=True),
    READ_NAMED_PATH_TASK: KuraTaskFixture(requires_evidence=True),
    MULTILINE_MESSAGE_TASK: KuraTaskFixture(
...
def fixture_for(task_name: str | None) -> KuraTaskFixture:
    """Return the declared setup for a task, or an empty setup."""
    return _FIXTURES.get(task_name or "", KuraTaskFixture())
