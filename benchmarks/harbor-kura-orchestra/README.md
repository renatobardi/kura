...
Kura stack. Harbor sees one `KuraOrchestraAgent`; behind that adapter, one
...
`kura` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/harbor-kura-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_kura_orchestra:KuraOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_kura_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-kura-orchestra --agent-kwarg kura_acp_binary=<LINUX_BIN>/kura-acp --agent-kwarg kura_agent_binary=<LINUX_BIN>/kura-agent --agent-kwarg kura_dev_mcp_binary=<LINUX_BIN>/kura-dev-mcp --agent-kwarg kura_cli_binary=target/debug/kura --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`kura_acp_binary`/`kura_agent_binary`/`kura_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `kura_cli_binary` is the **host** CLI the harness
...
```bash
just benchmark \
  --path benchmarks/kura-dataset/reply-to-thread \
  --manifest benchmarks/harbor-kura-orchestra/manifests/kura-native-solo-luna.yaml \
  --endpoint-config benchmarks/harbor-kura-orchestra/testbed/endpoints/openai-live.json \
  --n-concurrent 1
```
...
The default condition is `kura-native-solo-luna.yaml` — one solo agent on
...
`anthropic-live.json`. Swap in `kura-native-solo-sonnet.yaml` (no
...
Replace the path with `benchmarks/kura-dataset/create-channel-invite-users`
...
`/logs/artifacts/kura-evidence.json`. The task verifier reads that post-agent
...
cause is written to the trial's `kura/kura-evidence-error.txt`.
...
stack (`kura-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
```bash
cd benchmarks/harbor-kura-orchestra
