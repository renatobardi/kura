...
def test_kura_task_metadata_defines_the_expected_layers():
    tasks = benchmark.kura_tasks_for_path(benchmark.KURA_DATASET_ROOT)
...
def test_kura_task_metadata_rejects_missing_or_unknown_layers(
    ...
    with pytest.raises(SystemExit, match="evaluation_layer|metadata"):
        benchmark.kura_tasks_for_path(dataset)
...
def test_layer_selects_metadata_and_uses_its_default_attempts(layer, attempts):
    ...
    selected = benchmark.kura_tasks_for_path(benchmark.KURA_DATASET_ROOT)
...
def test_omitted_layer_splits_kura_dataset_into_two_jobs():
...
def test_single_kura_task_infers_its_layer_default():
...
def test_explicit_attempts_override_keeps_one_mixed_kura_job():
    args = benchmark.parse_args(
