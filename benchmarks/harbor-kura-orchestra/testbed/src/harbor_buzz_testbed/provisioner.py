"""Channel-per-trial Kura provisioning against a local benchmark stack."""
...
from harbor_kura_orchestra.task_fixtures import DirectoryEntry, fixture_for

from .kura_cli import KuraCli
...
def provisioner_from_dict(config: dict[str, object]) -> KuraTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return KuraTrialProvisioner(TestbedConfig(**config))


class KuraTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Kura relay.
    ...
    def _provision(
        ...
        # mirroring production Kura, where a human owns the channel their
        # agents work in.
    ...
    def _seed_directory(
        self, task_name: str | None, observer: KuraCli
    ...
    def _cli_for(self, credential: AgentCredential) -> KuraCli:
        return KuraCli(
