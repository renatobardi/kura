...
from harbor_kura_orchestra import (
    AgentCredential,
    KuraOrchestraAgent,
    RuntimeResult,
    TrialHandle,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = KuraOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = KuraOrchestraAgent(
        ...
        kura_acp_binary="/pinned/kura-acp",
        kura_agent_binary="/pinned/kura-agent",
        kura_dev_mcp_binary="/pinned/kura-dev-mcp",
        kura_cli_binary="/pinned/kura",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.kura_acp_binary == "/pinned/kura-acp"
    assert agent.runtime.kura_agent_binary == "/pinned/kura-agent"
    assert agent.runtime.kura_dev_mcp_binary == "/pinned/kura-dev-mcp"
    assert agent.runtime.kura_cli_binary == "/pinned/kura"
