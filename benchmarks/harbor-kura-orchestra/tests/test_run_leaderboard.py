...
def test_command_uses_standard_settings_only(args, binaries, agent_binaries):
    ...
    assert any(k.startswith("kura_acp_binary=") for k in kwargs)
    assert any(k.startswith("kura_agent_binary=") for k in kwargs)
    assert any(k.startswith("kura_dev_mcp_binary=") for k in kwargs)
