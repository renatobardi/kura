...
from harbor_kura_orchestra.container_runtime import (
    REMOTE_BIN,
    REMOTE_EVIDENCE,
    REMOTE_LOGS,
    THINKING_EFFORT,
...
from harbor_kura_orchestra.manifest import ExperimentManifest
from harbor_kura_orchestra.provisioning import (
    AgentCredential,
    FixtureActor,
    TrialHandle,
)
from harbor_kura_orchestra.task_fixtures import fixture_for
...
async def test_collects_task_declared_channel_membership(tmp_path, monkeypatch):
    ...
    async def kura_json(credential_arg, trial_arg, *args):
    ...
    monkeypatch.setattr(rt, "_kura_json", kura_json)
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    ...
    rt = runtime(
        tmp_path,
        kura_acp_binary=binaries["kura-acp"],
        kura_agent_binary=binaries["kura-agent"],
        kura_dev_mcp_binary=binaries["kura-dev-mcp"],
    )
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, kura_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from harbor_kura_orchestra.container_runtime import FORWARDER
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from harbor_kura_orchestra.container_runtime import _Agent
...
async def test_send_mentions_by_pubkey_so_task_text_stays_inert(tmp_path, monkeypatch):
    ...
    async def kura_json(credential, trial, *args):
        calls.append(args)
        return {}
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def kura_json(credential, *args, **kwargs):
...
async def test_solo_turn_end_completes_without_done_message(tmp_path, monkeypatch):
    ...
    async def kura_json(*args, **kwargs):
        return []
...
def test_turn_status_parses_completed_batch_and_successful_steer_receipts():
    ...
    output = "\n".join(
        [
            ...
            f"turn delivered Kura events for channel test: {batch_event_id}",
    ...
    assert KuraContainerRuntime._parse_turn_status(output) == (
        1,
        1,
...
def test_runtime_logging_keeps_readiness_and_turn_completion_signals(tmp_path):
    rt = runtime(tmp_path)
    assert rt._rust_log(None) == "kura_acp=info,pool::prompt=info"
    assert rt._rust_log("custom=debug") == (
        "custom=debug,kura_acp=info,pool::prompt=info"
    )
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await KuraContainerRuntime._stop_agents(environment, agents)
