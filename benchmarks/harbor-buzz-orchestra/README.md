...
```bash
uv run --project benchmarks/harbor-kura-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_buzz_orchestra:BuzzOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_buzz_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-kura-orchestra --agent-kwarg buzz_acp_binary=<LINUX_BIN>/kura-acp --agent-kwarg buzz_agent_binary=<LINUX_BIN>/kura-agent --agent-kwarg buzz_dev_mcp_binary=<LINUX_BIN>/kura-dev-mcp --agent-kwarg buzz_cli_binary=target/debug/kura --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```
