...
reading it. See upstream issue #6261.

## Environment

`python:3.12-slim-bookworm` with `HOME=/home/kura`, so `~` in the instruction
...
Reads the post-agent `/logs/artifacts/kura-evidence.json` snapshot plus the
...
```bash
just benchmark \
  --path benchmarks/kura-dataset/read-named-path-outside-workspace \
  --attempts 1 \
...
`../harbor-kura-orchestra/tests/test_read_named_path_outside_workspace_verifier.py`.
