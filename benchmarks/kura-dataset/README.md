...
absent from `instruction.md`** — it has to come from `kura-acp`'s production
...
The task identity remains `kura-native/<task>` in both layers; the wrapper reads
the metadata instead of encoding the layer in task names.

## Running

These tasks need the [`harbor-kura-orchestra`](../harbor-kura-orchestra)
harness, which launches the real `kura-acp` → `kura-agent` → `kura-dev-mcp`
...
whole layer with `--path benchmarks/kura-dataset --layer regression` or
...
[the harness README](../harbor-kura-orchestra/README.md#kura-native-tasks) for
