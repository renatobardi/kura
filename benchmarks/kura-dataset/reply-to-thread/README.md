...
> the behavior under test, and it must come from `kura-acp`'s production base
...
container's shell. `KuraOrchestraAgent` launches the real `kura-acp` /
`kura-agent` stack against a dedicated relay, and the agent works entirely
through Kura. Agent timeout 300s; the manifest's `trial_budget` is the
effective clock.

## Verifier

Reads the post-agent `/logs/artifacts/kura-evidence.json` snapshot (written by
`KuraContainerRuntime._collect_evidence` after the agent stops, so the agent
...
shipped: the Oracle agent replaces `KuraOrchestraAgent`, so no relay trial is
...
`../harbor-kura-orchestra/tests/test_reply_to_thread_verifier.py` (run from
`benchmarks/harbor-kura-orchestra`: `uv run --extra dev pytest -q`).
