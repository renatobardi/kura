...
> mention is the behavior under test and must come from `kura-acp`'s production
...
`kura-agent` stack against a dedicated relay. Agent timeout 300s.

## Verifier

Reads the post-agent `/logs/artifacts/kura-evidence.json` snapshot. Every
...
```bash
just benchmark \
  --path benchmarks/kura-dataset/user-mention \
  --attempts 1 \
...
`../harbor-kura-orchestra/tests/test_user_mention_verifier.py`.
