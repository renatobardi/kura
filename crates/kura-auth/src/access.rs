...
//!
//! Defines [`ChannelAccessChecker`] so `kura-auth` can enforce access
//! without depending on `kura-db` directly.
...
///
/// Implemented by the database layer (`kura-db`) in production. The `kura-auth`
