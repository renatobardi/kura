#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `kura-auth` — Authentication and authorization for the Kura relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("kura-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("kura-test-key:{username}");
