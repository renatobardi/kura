...
/// this private supertrait, so no external `kura_auth` consumer can implement
...
/// outside `kura_auth`. That closes the authority-construction seam: an
...
///
/// ```compile_fail
/// use kura_auth::{AssertionKeySet, IssuerKeySource};
/// struct Forge;
