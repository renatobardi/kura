...
//! `kura-relay` / `kura-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `kura-relay` / `kura-pubsub`.
...
/// key is community-prefixed (`kura:{community}:ratelimit:{pubkey}:{suffix}`). The
...
/// `kura:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Redis key for IP-based rate limit: `kura:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("kura:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("kura:{}:ratelimit:", ctx.community());
        assert!(
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(ip_rate_limit_key(&ip), "kura:ratelimit:ip:192.168.1.1:conn");
    }
