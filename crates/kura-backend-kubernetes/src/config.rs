...
/// `KURA_ACP_EXIT_AFTER_INACTIVITY` are one knob, not two.
...
/// A fresh `kura-agents-<rand6>` namespace default.
///
...
mod tests {
    ...
    fn minimal() -> serde_json::Value {
        serde_json::json!({"namespace": "kura-agents-abc123", "image": digest_ref()})
    }
    ...
    fn generated_namespaces_are_fresh_and_valid() {
        ...
        assert!(a.starts_with("kura-agents-"));
        assert_eq!(a.len(), "kura-agents-".len() + 6);
    }
    ...
    fn schema_default_image_round_trips_through_parse() {
        ...
        let cfg = serde_json::json!({"namespace": "kura-agents-abc123", "image": default});
