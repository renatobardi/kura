...
mod tests {
    ...
    fn terminated_pods_and_their_secrets_are_collected_together() {
        let id = identity();
        let now = Utc::now();
        let pod = pod_named(&id, "kura-agent-dead", Some("kura-agent-dead-gen1"));
        let plan = plan(&id, &[pod], &[], always, Some(now));
        assert_eq!(plan.pods, ["kura-agent-dead"]);
        assert_eq!(plan.secrets, ["kura-agent-dead-gen1"]);
    }

    #[test]
    fn live_pods_are_never_collected() {
        let id = identity();
        let pod = pod_named(&id, "kura-agent-live", Some("kura-agent-live-gen1"));
    ...
    fn young_unreferenced_secrets_are_protected() {
        let id = identity();
        let now = Utc::now();
        let fresh = secret_named(&id, "kura-agent-x-gen-inflight", 5, now);
        assert_eq!(
    ...
    fn secrets_older_than_twice_the_deadline_are_collected() {
        ...
        let old = secret_named(
            &id,
            "kura-agent-x-gen-abandoned",
            ORPHAN_SECRET_MIN_AGE_SECS + 1,
            now,
        );
        let plan = plan(&id, &[], &[old], never, Some(now));
        assert_eq!(plan.secrets, ["kura-agent-x-gen-abandoned"]);
    }
    ...
    fn without_a_server_clock_the_orphan_sweep_is_skipped() {
        let id = identity();
        let now = Utc::now();
        let ancient = secret_named(&id, "kura-agent-x-gen-ancient", 10_000_000, now);
    ...
    fn secrets_referenced_by_a_pending_pod_are_protected() {
        let id = identity();
        let now = Utc::now();
        let pending = pod_named(&id, "kura-agent-pending", Some("kura-agent-pending-gen1"));
        let old = secret_named(&id, "kura-agent-pending-gen1", 10_000_000, now);
    ...
    fn secrets_without_a_creation_timestamp_are_skipped() {
        let id = identity();
        let now = Utc::now();
        let mut no_timestamp = secret_named(&id, "kura-agent-x-gen-unknown", 10_000_000, now);
    ...
    fn a_collected_pods_secret_is_listed_once() {
        let id = identity();
        let now = Utc::now();
        let dead = pod_named(&id, "kura-agent-dead", Some("kura-agent-dead-gen1"));
        let its_secret = secret_named(&id, "kura-agent-dead-gen1", 10_000_000, now);
