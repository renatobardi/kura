use kura_sdk::{DeleteMessageOptions, DiffMeta, ThreadRef, VoteDirection};
use nostr::PublicKey;
use uuid::Uuid;

use crate::client::{normalize_events, normalize_write_response, KuraClient};
...
///
/// Delegates marker parsing and collapse to [`kura_core::nip10`] (shared with
...
fn find_root_from_tags(tags: &serde_json::Value) -> Option<String> {
    let parts: Vec<Vec<String>> = tags
        ...
        .collect();
    kura_core::nip10::parse_thread_markers_from_parts(parts.iter().map(Vec::as_slice))
        .resolve()
        .map(|(root, _)| root)
}
...
async fn fetch_event(client: &KuraClient, event_id: &str) -> Result<serde_json::Value, CliError> {
...
async fn resolve_channel_id(client: &KuraClient, event_id: &str) -> Result<Uuid, CliError> {
...
pub fn resolve_thread_target(
    ...
    if expected_root_id.is_some_and(|expected| expected != root_event_id) {
        return Err(CliError::Usage(
            "Kura message link thread root does not match the selected message".into(),
        ));
    }
    Ok(root_event_id)
}
...
async fn resolve_author(client: &KuraClient, author: &str) -> Result<String, CliError> {
...
pub async fn cmd_send_message(
    ...
    if !missing.is_empty() {
        return Err(CliError::Usage(
            serde_json::json!({
                ...
                "add_member_command": format!("kura channels add-member --channel {} --pubkey <pubkey> --role <member|bot>", p.channel_id),
            })
            .to_string(),
        ));
    }
    ...
    let builder = match p.kind {
        Some(45001) => {
            kura_sdk::build_forum_post(channel_uuid, &final_content, &mention_refs, &media_tags)
        ...
        None | Some(9) => kura_sdk::build_message(
            channel_uuid,
            &final_content,
            thread_ref.as_ref(),
            &mention_refs,
...
pub async fn cmd_send_diff_message(client: &KuraClient, p: SendDiffParams) -> Result<(), CliError> {
    ...
    let builder =
        kura_sdk::build_diff_message(channel_uuid, &diff, &diff_meta, thread_ref.as_ref())
...
pub async fn cmd_delete_message(
    ...
    let builder = kura_sdk::build_delete_message_with_options(
        channel_uuid,
        target_eid,
...
pub async fn cmd_edit_message(
    ...
    let builder = kura_sdk::build_edit(channel_uuid, target_eid, content)
...
pub async fn cmd_vote_on_post(
    ...
    let builder = kura_sdk::build_vote(channel_uuid, target_eid, vote_dir)
...
mod tests {
    ...
    async fn malformed_channel_is_rejected_before_thread_fetch() {
        let client =
            KuraClient::new("http://127.0.0.1:1".into(), Keys::generate(), None, None).unwrap();
