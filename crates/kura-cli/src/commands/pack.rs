//! `kura pack` subcommands — local persona pack operations.
//!
...
pub fn cmd_validate(path: &str) -> Result<(), CliError> {
    ...
    let report = kura_persona::validate::validate_pack(pack_dir);

    for diag in &report.diagnostics {
        match diag {
            kura_persona::validate::ValidationDiagnostic::Error(msg) => {
                eprintln!("  ERROR: {msg}");
            }
            kura_persona::validate::ValidationDiagnostic::Warning(msg) => {
                eprintln!("  WARN:  {msg}");
            }
        }
    }
...
pub fn cmd_inspect(path: &str) -> Result<(), CliError> {
    ...
    let pack = kura_persona::resolve::resolve_pack(pack_dir)
