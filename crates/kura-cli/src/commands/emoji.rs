...
use crate::client::{normalize_write_response, KuraClient};
use crate::error::CliError;
use kura_sdk::CustomEmoji;
...
const CUSTOM_EMOJI_SET_D_TAG: &str = kura_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
/// own kind:30030 set (d=`kura:custom-emoji`).
async fn cmd_list(client: &KuraClient) -> Result<(), CliError> {
    let filter = serde_json::json!({
        "kinds": [kura_sdk::kind::KIND_EMOJI_SET],
...
async fn fetch_own_emoji(client: &KuraClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &KuraClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = kura_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &KuraClient, shortcode: &str, url: &str) -> Result<(), CliError> {
...
async fn cmd_rm(client: &KuraClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = kura_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &KuraClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
