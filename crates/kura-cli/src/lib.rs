...
use client::KuraClient;
use error::CliError;
use nostr::Keys;
use uuid::Uuid;

/// Run the Kura CLI from raw arguments (including `argv[0]`).
///
...
///
/// ```ignore
/// let code = kura_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from kura-acp/kura-dev-mcp)
    ...
    // kura-dev-mcp delegates to run_from_args, it has already installed ring; the
...
#[command(
    name = "kura",
    about = "Kura CLI — interact with a Kura relay",
    long_about = "\
Kura CLI — interact with a Kura relay

Configuration (flags override env vars):
  KURA_RELAY_URL     Relay base URL        [default: http://localhost:3000]
  KURA_PRIVATE_KEY   Nostr private key (hex or nsec)  [required]
  KURA_AUTH_TAG      NIP-OA auth tag JSON  [optional]
...
struct Cli {
    /// Relay URL (http:// or https://). Overrides KURA_RELAY_URL env var.
    #[arg(long, env = "KURA_RELAY_URL", default_value = "http://localhost:3000")]
    relay: String,

    /// Nostr private key (hex or nsec). This is the CLI's identity.
    #[arg(long, env = "KURA_PRIVATE_KEY", hide_env_values = true)]
    private_key: Option<String>,

    /// NIP-OA auth tag JSON (owner attestation). Injected into every signed event.
    #[arg(long, env = "KURA_AUTH_TAG", hide_env_values = true)]
...
#[derive(Subcommand)]
pub enum AgentsCmd {
    /// Open a prefilled create-agent form in the owner's Kura Desktop
    DraftCreate {
    ...
    /// Open a prefilled edit-agent form in the owner's Kura Desktop
    DraftUpdate {
...
running under KURA_AUTH_TAG signs as itself, so it can only ever satisfy \
the self path (target == signer) — not the owner-of-agent path for another \
identity.\n\n\
Examples:\n  \
kura agents archive <PUBKEY> --reason retired\n  \
kura agents archive <PUBKEY> --reason bot-rebuilt --replaced-by <NEW_PUBKEY>"
    )]
    Archive {
...
Examples:\n  \
kura agents unarchive <PUBKEY> --reason returned"
    )]
    Unarchive {
        /// Target identity pubkey (hex)
        target_pubkey: String,
...
pub enum MessagesCmd {
    ...
    #[command(
        after_help = "Examples:\n  kura messages send --channel <UUID> --content \"hello\"\n  kura messages send --channel <UUID> --content \"@alice check this\"\n  echo \"hello from stdin\" | kura messages send --channel <UUID> --content -"
    )]
    Send {
        /// Channel UUID (from 'kura channels list')
        #[arg(long)]
        channel: String,
    ...
    #[command(
        after_help = "Examples:\n  kura messages get --channel <UUID>\n  kura messages get --channel <UUID> --limit 50 --kinds 1,1984"
    )]
    Get {
        /// Channel UUID
        #[arg(long)]
    ...
    /// Get the containing thread for a message or Kura message link
    #[command(
        after_help = "Examples:\n  kura messages thread --channel <UUID> --event <EVENT_ID>\n  kura messages thread --link 'kura://message?channel=<UUID>&id=<EVENT_ID>&thread=<ROOT_ID>'"
    )]
    Thread {
        ...
        #[arg(long, required_unless_present = "link", conflicts_with = "link")]
        event: Option<String>,
        /// Canonical kura://message deep link; uses the configured relay and identity
    ...
    #[command(
        after_help = "Examples:\n  kura messages search --query checkout\n  kura messages search --author npub1... --since 1783497600\n  kura messages search --author Aaron --query checkout --limit 20"
    )]
    Search {
...
pub enum ChannelsCmd {
    ...
    #[command(
        after_help = "Examples:\n  kura channels list\n  kura channels list --visibility open"
    )]
    List {
    ...
    #[command(
        after_help = "Examples:\n  kura channels search --query composer\n  kura channels search --query kura-chat-composer --exact\n  kura channels search --query design --include-archived"
    )]
    Search {
    ...
    #[command(
        after_help = "Examples:\n  kura channels create --name general --type stream --visibility open\n  kura channels create --name design --type forum --visibility open --description \"Design discussions\"\n  kura channels create --name standup --type stream --visibility open --ttl 3600  # ephemeral, archived after 1h idle\n  kura channels create --name project-x --template \"Kura Team\"  # type/visibility/canvas/roster from the template; explicit flags override"
    )]
    Create {
        /// Channel name
        #[arg(long)]
    ...
    #[command(
        after_help = "Examples:\n  kura channels update --channel <uuid> --name general\n  kura channels update --channel <uuid> --visibility open\n  kura channels update --channel <uuid> --visibility private"
    )]
    Update {
        /// Channel UUID
        #[arg(long)]
...
pub enum WorkflowsCmd {
    ...
    #[command(
        after_help = "Examples:\n  kura workflows trigger --workflow <UUID>\n  kura workflows trigger --workflow <UUID> --inputs '{\"key\":\"value\"}'"
    )]
    Trigger {
        /// Workflow UUID
        #[arg(long)]
...
pub enum NotesCmd {
    ...
    #[command(
        after_help = "Examples:\n  echo '# Hello' | kura notes set --name hello --title 'Hello' --content -\n  kura notes set --name hello --tag onboarding --content - < draft.md"
    )]
    Set {
...
pub enum ReposCmd {
    ...
    Create {
        ...
        relays: Vec<String>,
        /// Channel UUID to bind the repo to. The `kura-channel` tag is the
        /// git ACL: without it the relay 404s every clone/fetch/push until
        /// the author runs `kura repos bind` (issue #3527).
        #[arg(long)]
        channel: Option<String>,
    },
    ...
    ///
    /// The `kura-channel` tag on the announcement is the git ACL: the relay
...
pub enum ProjectsCmd {
    ...
    Create {
        ...
        slug: String,
        /// Member repository coordinate: bare Kura repo id (e.g. `kura`) or full
    ...
    /// Draft a project-linked channel for owner review in Kura Desktop
    ...
    Update {
        ...
        /// Set the associated Kura channel UUID
        #[arg(long, group = "mutation")]
        channel: Option<String>,
...
pub enum PatchesCmd {
    ...
    #[command(
        after_help = "Examples:\n  git format-patch -1 HEAD --stdout | kura patches send --repo-owner <hex> --repo-id myrepo --patch-file - --root\n  kura patches send --repo-owner <hex> --repo-id myrepo --patch-file 0001-fix.patch --reply-to <prev-patch-id>"
    )]
    Send {
...
pub enum PrCmd {
    ...
    #[command(
        after_help = "Examples:\n  kura pr open --repo-owner <hex> --repo-id myrepo --subject 'Fix bug' --body-file - --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo --branch-name fix-bug\n  kura pr update --repo-owner <hex> --repo-id myrepo --pr <event> --pr-author <hex> --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo"
    )]
    Open {
        /// Repo owner pubkey (64-char hex)
        #[arg(long)]
...
pub enum IssuesCmd {
    ...
    Create {
        ...
        /// `KURA_GIT_ORIGIN_CHANNEL_ID`) names a project home.
        #[arg(long)]
        repo_owner: Option<String>,
        ...
        /// this project when none exists. Defaults to `KURA_GIT_ORIGIN_CHANNEL_ID`.
        #[arg(long)]
        channel: Option<String>,
        /// Issue title
...
/// Subcommands for `kura mem`.
#[derive(Subcommand)]
pub enum MemCmd {
    /// List non-tombstoned memory entries
    Ls {
        /// Owner pubkey (hex). Overrides KURA_AUTH_TAG.
        #[arg(long)]
        owner: Option<String>,
    ...
    Patch {
        ...
        /// against. Hashes the exact UTF-8 bytes returned by `kura mem get`,
        /// not normalized lines. Run `kura mem hash <slug>` to capture this
        /// before editing.
        #[arg(long)]
...
/// `KURA_RELAY_URL` — moderation commands are community-global and carry no
...
pub enum ModerationCmd {
    ...
    #[command(
        after_help = "Examples:\n  kura moderation reports\n  kura moderation reports --status open --limit 20"
    )]
    Reports {
    ...
    #[command(
        after_help = "Examples:\n  kura moderation resolve --report <REPORT_EVENT_ID> --status dismissed --action dismiss\n  kura moderation resolve --report <REPORT_EVENT_ID> --status resolved --action ban --reason \"rule 3\""
    )]
    Resolve {
    ...
    #[command(
        after_help = "Examples:\n  kura moderation ban --pubkey <HEX>\n  kura moderation ban --pubkey <HEX> --expires-in 604800 --reason \"repeated spam\""
    )]
    Ban {
    ...
    #[command(
        after_help = "Examples:\n  kura moderation timeout --pubkey <HEX> --expires-in 3600\n  kura moderation timeout --pubkey <HEX> --expires-at 1783500000 --reason \"cool off\""
    )]
    Timeout {
        /// Target member pubkey (hex)
        #[arg(long)]
...
/// Normalize hand-authored `KURA_AUTH_TAG` input to strict JSON.
///
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let private_key_str = cli.private_key.ok_or_else(|| {
        CliError::Auth("KURA_PRIVATE_KEY is required (use --private-key or set env var)".into())
    })?;
    let keys = Keys::parse(&private_key_str)
        .map_err(|e| CliError::Key(format!("invalid KURA_PRIVATE_KEY: {e}")))?;

    // NIP-OA: parse and verify the auth tag if provided.
    //
    // `KURA_AUTH_TAG` is hand-authored configuration, so the unquoted raw
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref input) if !input.is_empty() => {
            ...
            let tag = kura_sdk::nip_oa::parse_auth_tag(&json)
                .map_err(|e| CliError::Auth(format!("KURA_AUTH_TAG is malformed: {e}")))?;
            kura_sdk::nip_oa::verify_auth_tag(&json, &keys.public_key()).map_err(|e| {
                CliError::Auth(format!(
                    "KURA_AUTH_TAG verification failed for pubkey {}: {e}",
                    keys.public_key().to_hex()
                ))
            })?;
            // Canonical wire form derives from the parsed-and-verified tag
            // (same shape as kura-acp's RestClient), never from raw input.
            let canonical = serde_json::to_string(tag.as_slice())
                .map_err(|e| CliError::Auth(format!("KURA_AUTH_TAG serialization failed: {e}")))?;
            (Some(tag), Some(canonical))
        }
        _ => (None, None),
    };

    let client = KuraClient::new(relay_url, keys, auth_tag, auth_tag_json)?;
...
mod tests {
    ...
    fn messages_thread_accepts_link_or_explicit_identifiers() {
        ...
        let link = format!("kura://message?channel={channel}&id={event}");

        assert!(
            Cli::try_parse_from(["kura", "messages", "thread", "--link", link.as_str(),]).is_ok()
        );
    ...
    fn messages_thread_rejects_partial_or_mixed_targets() {
        ...
        assert!(Cli::try_parse_from(["kura", "messages", "thread"]).is_err());
        assert!(Cli::try_parse_from(["kura", "messages", "thread", "--channel", channel]).is_err());
        assert!(Cli::try_parse_from([
            "kura",
            "messages",
            "thread",
    ...
    fn set_status_clear_rejects_text_and_emoji() {
        for extra in [["--text", "busy"], ["--emoji", "🎶"]] {
            let args = ["kura", "users", "set-status", "--clear"]
                .into_iter()
                .chain(extra);
            assert!(
    ...
    fn set_status_requires_text_or_clear() {
        assert!(Cli::try_parse_from(["kura", "users", "set-status"]).is_err());
        assert!(
            Cli::try_parse_from(["kura", "users", "set-status", "--emoji", "🎶"]).is_err(),
            "--emoji alone must not imply a status"
        );
        assert!(Cli::try_parse_from(["kura", "users", "set-status", "--clear"]).is_ok());
    }
    ...
    fn projects_update_no_mutation_is_rejected_by_clap() {
        ...
        assert!(
            Cli::try_parse_from(["kura", "projects", "update", "my-slug"]).is_err(),
