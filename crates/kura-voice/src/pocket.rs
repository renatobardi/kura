//! April 2026 Pocket TTS engine for Kura Desktop.
//!
...
//! Kura selects the upstream three-graph INT8 variant while retaining the
...
/// sessions via `KURA_TTS_THREADS`. Default preserves production's 1.
fn tts_num_threads() -> usize {
    std::env::var("KURA_TTS_THREADS")
        .ok()
...
/// Load Kura Desktop's pinned April INT8 model.
...
mod tests {
    ...
    #[test]
    #[ignore = "requires KURA_POCKET_TEST_MODEL_DIR"]
    fn production_api_emits_non_silent_april_int8_pcm() {
        let dir = std::env::var("KURA_POCKET_TEST_MODEL_DIR")
            .expect("set KURA_POCKET_TEST_MODEL_DIR to an April INT8 model directory");
