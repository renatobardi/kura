//! Relay-driven mesh lifecycle smoke — the full Kura join story, CI-shaped.
//!
...
//!      kura-relay (kind:13534 roster via kura-admin); a third is not.
...
//! cargo build --profile ci -p kura-admin
//! KURA_ADMIN_BIN=target/ci/kura-admin \
//!   cargo run --profile ci -p kura-relay --example mesh_relay_lifecycle_smoke
//! ```
...
/// (`KIND_KURA_MESH_MEMBER_STATUS` in the desktop coordinator).
...
const STATUS_K_TAG: &str = "kura-mesh-status";
...
async fn role_serve() -> anyhow::Result<()> {
    ...
    let keys = Keys::parse(&env("KURA_MEMBER_NSEC")?)?;
    ...
    let mut relay = KuraTestClient::connect(&relay_ws_url(), &keys)
        .await
    ...
    let cfg = serve::EmbeddedServeConfig::builder()
        ...
        // auto-discover. The Kura relay is the only discovery surface.
        .publish(false)
        .auto_join(false)
...
async fn role_stranger() -> anyhow::Result<()> {
    ...
    match KuraTestClient::connect(&relay_ws_url(), &keys).await {
        Err(error) => {
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let admin =
        std::env::var("KURA_ADMIN_BIN").unwrap_or_else(|_| "target/ci/kura-admin".to_string());
    anyhow::ensure!(
        std::path::Path::new(&admin).exists(),
        "kura-admin binary not found at {admin} (set KURA_ADMIN_BIN)"
    );

    let scratch = std::env::temp_dir().join(format!("kura-mesh-lifecycle-{}", std::process::id()));
    ...
    // MEMBERSHIP: A and B become relay members via kura-admin (publishes the
    ...
    for (label, keys) in [("A", &member_a), ("B", &member_b)] {
        let status = Command::new(&admin)
            ...
            .status()?;
        anyhow::ensure!(status.success(), "kura-admin add-member {label} failed");
        eprintln!(
    ...
    let mut serve_child = Command::new(&exe)
        ...
        .env("KURA_MEMBER_NSEC", secret_hex(&member_a))
    ...
    let mut client_child = Command::new(&exe)
        ...
        .env("KURA_MEMBER_NSEC", secret_hex(&member_b))
    ...
    let mut stranger_child = Command::new(&exe)
        ...
        .env("KURA_MEMBER_NSEC", secret_hex(&stranger))
