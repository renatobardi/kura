...
//!
//! Proves the security claim of Kura's membership-derived mesh admission:
...
//!      same builder calls Kura desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how Kura runs it in production anyway (one
//! desktop = one node).
//!
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch = std::env::temp_dir().join(format!("kura-mesh-admission-{}", std::process::id()));
