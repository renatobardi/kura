//! Relay startup wiring for the inter-relay mesh (`KURA_MESH` seam).
//!
//! [`boot_mesh`] is the ONLY place the relay constructs mesh machinery. It
//! returns `None` — and touches nothing — when `KURA_MESH=off`, so mesh-off
//! deployments stay byte-identical to a relay built before this module
//! existed. When enabled, it:
//!
//! 1. binds the iroh endpoint on `KURA_MESH_BIND_ADDR` (boot-unique keypair =
...
use kura_relay_mesh::endpoint::MeshEndpoint;
use kura_relay_mesh::gossip::GossipRecord;
use kura_relay_mesh::registry::{ReadyRecord, ReadyRegistry};
use kura_relay_mesh::{
...
///   either the `KURA_MESH_DEMO_ECHO` consumer (testbed evidence runs: echo
...
/// `KURA_MESH_DEMO_ECHO` consumer: echo every validated `Data` frame back to
...
const PROTO_VERSION: u16 = kura_relay_mesh::WIRE_VERSION as u16;
...
/// `KURA_MESH_ADVERTISE_ADDR` (explicit, classic-LB shapes) →
...
fn advertise_addrs(endpoint: &MeshEndpoint) -> Vec<String> {
    if let Ok(addr) = std::env::var("KURA_MESH_ADVERTISE_ADDR") {
...
/// Boot the mesh, or return `None` when `KURA_MESH=off`.
///
...
/// sets `KURA_MESH=on` wants the mesh or wants to know why not; silently
...
pub async fn boot_mesh(
    ...
    if !config.mesh.enabled {
        tracing::info!("mesh disabled (KURA_MESH is not 'on') — single-instance behavior");
        return Ok(None);
    }
    ...
    kura_relay_mesh::runtime::spawn_registry_heartbeat(
        registry.clone(),
        ready_record,
...
mod tests {
    ...
    /// KURA_MESH=off must be a hard no-op: no endpoint bind, no Redis write,
    ...
    /// Blocker fix (Wren review of 8b077fdb): absent `KURA_MESH`, the mesh is
    /// OFF — an env-untouched image upgrade must not bind or write Redis.
    #[test]
    fn mesh_defaults_off_when_env_absent() {
        // `Config::from_env` in the test env has no KURA_MESH set unless a
        // caller exported it; assert the fail-safe reading.
        if std::env::var("KURA_MESH").is_ok() {
        ...
        assert!(!config.mesh.enabled, "KURA_MESH absent must mean mesh off");
    }
    ...
    fn session_hello(sender: RuntimeId, profile: Profile) -> StreamHello {
        StreamHello {
            sender,
            role: kura_relay_mesh::StreamRole::Session {
                fenced: FencedHeader {
    ...
    async fn wired_datagram_consumer_shares_the_handle_fence() {
        ...
        impl kura_relay_mesh::RelayPeerTransport for NoopTransport {
