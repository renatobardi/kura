...
use kura_db::admin_moderation::AdminReportDetail;
use kura_db::relay_admin_actions::{AdminActionRecord, ClaimResult};
...
impl From<kura_db::DbError> for ResolutionError {
    fn from(e: kura_db::DbError) -> Self {
    ...
    loop {
        ...
        if rec.step_marker.is_none() {
            ...
            let lease_token = if let Some(token) = held_lease {
                ...
                match lease {
                    kura_db::relay_admin_actions::LeaseResult::Acquired(token) => token,
                    kura_db::relay_admin_actions::LeaseResult::Contended => {
                    ...
                    kura_db::relay_admin_actions::LeaseResult::NotLeasable => {
...
async fn run_atomic_mutation(
    ...
    let raw: anyhow::Result<bool> = match ctx.action {
        ...
        "kick" => {
            ...
            {
                kura_db::relay_admin_actions::KickWithMarkerResult::Removed => Ok(true),
                kura_db::relay_admin_actions::KickWithMarkerResult::AlreadyMarked => Ok(false),
                kura_db::relay_admin_actions::KickWithMarkerResult::AlreadyGone => Err(
...
mod tests {
    use super::*;
    use kura_db::admin_moderation::{AdminReport, AdminReportedMessage};
