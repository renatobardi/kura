...
use kura_core::tenant::{normalize_host, TenantContext};
use url::{Host, Url};
...
/// present, the owner is (re)bootstrapped via [`kura_db::Db::bootstrap_owner`]
...
pub async fn provision_community(
    ...
    if request.create_only {
        ...
        {
            kura_db::CreateCommunityWithOwnerResult::Created(record) => record,
            kura_db::CreateCommunityWithOwnerResult::HostExists => {
                return Err("community already exists".to_string());
            }
            kura_db::CreateCommunityWithOwnerResult::LimitReached => {
                return Err(
...
mod tests {
    ...
    fn host_valid_bare_domain() {
        assert!(validate_host("acme.communities.kura.xyz").is_ok());
    }
