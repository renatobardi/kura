...
use kura_db::relay_admin_actions::OutboxRecord;
...
async fn resolve_tenant(
    ...
    task_type: &str,
) -> Result<kura_core::tenant::TenantContext, String> {
    let host = state
        .db
    ...
    Ok(kura_core::tenant::TenantContext::resolved(
        community_id,
        host,
    ))
}
...
async fn deliver_tombstone(state: &Arc<AppState>, row: &OutboxRecord) -> Result<(), String> {
    ...
    let community_id = kura_core::CommunityId::from_uuid(community_uuid);
