...
use kura_core::kind::KIND_STREAM_MESSAGE;
use kura_core::tenant::CommunityId;
use kura_workflow::action_sink::{ActionSink, ActionSinkError};
use chrono::Utc;
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = kura_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    kura_db::DbError::ChannelNotFound(_) | kura_db::DbError::NotFound(_) => {
            ...
            //    - `kura:workflow` tag prevents recursive workflow triggering
            ...
            // the `root` + `reply` pair — matching `kura_sdk::builders::thread_tags`
            ...
            let thread_meta = Some(match &thread_meta_owned {
                ...
                None => kura_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    //!   `cargo test -p kura-relay --lib workflow_sink -- --ignored`
    use super::*;
    use kura_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use kura_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
