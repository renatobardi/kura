//! Smart HTTP git transport for Kura.
//!
...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        let auth_header = parts
            ...
            .ok_or_else(|| {
                Response::builder()
                    ...
                    .header(
                        "WWW-Authenticate",
                        format!("Nostr realm=\"kura\", method=\"{method}\""),
                    )
        ...
        let pubkey =
            kura_auth::nip98::verify_nip98_event(&event_json, &expected_url, &event_method, None)
                .map_err(|e| {
...
async fn git_restriction_state(
    ...
    community: kura_core::CommunityId,
    pubkey: &nostr::PublicKey,
) -> Result<kura_db::moderation::RestrictionState, Response> {
...
fn enforce_git_ban(restriction: &kura_db::moderation::RestrictionState) -> Result<(), StatusCode> {
...
fn enforce_git_ban_cascade(
    agent: &kura_db::moderation::RestrictionState,
    owner: Option<&kura_db::moderation::RestrictionState>,
) -> Result<(), StatusCode> {
...
fn acquire_git_permit(
    ...
    Arc::clone(&state.git_semaphore)
        ...
        .map_err(|_| {
            metrics::counter!(
                "kura_git_semaphore_rejections_total",
                "operation" => operation
            )
            .increment(1);
            Response::builder()
...
///    resolve;
/// 2. its `kura-channel` tag → channel UUID;
/// 3. [`kura_db::Db::get_member_role`] for the caller — a read is allowed
///    only on `Ok(Some(role))` with a role the relay recognizes.
///
/// Fail-closed: missing/deleted announcement, invalid owner, missing or
/// malformed `kura-channel` binding, non-member, unknown role, and every DB
...
/// `kura-channel` tag, and the repo then 404s forever with no explanation
        ...
        RepoBinding::NotBound => {
            ...
            if repo_event.event.pubkey == *caller {
                ...
                return Err((
                    ...
                    format!(
                        "run: kura repos bind --id {repo_name} --channel <channel-uuid> — repository {repo_name:?} has no channel binding, so the relay cannot authorize access"
                    ),
                )
                    .into_response());
            }
            warn!(repo = %repo_name, "git read gate: missing kura-channel binding (deny)");
            return Err(denied());
        }
        RepoBinding::Broken => {
            warn!(repo = %repo_name, "git read gate: malformed kura-channel binding (deny)");
            return Err(denied());
        }
    };
...
fn read_role_allows(role: Option<&str>) -> bool {
    match role {
        Some(r) => r.parse::<kura_core::channel::MemberRole>().is_ok(),
        None => false,
    }
}
...
/// 0000
/// <pkt><head-oid> HEAD\0<caps> symref=HEAD:<head-ref> object-format=<fmt> agent=kura-git\n
...
pub async fn receive_pack(
    ...
    let mut hook_env = vec![
        ("KURA_HOOK_URL", hook_url),
        (
            "KURA_HOOK_SECRET",
            state.config.git_hook_hmac_secret.clone(),
        ),
        ("KURA_REPO_ID", repo_name.to_string()),
        ("KURA_REPO_OWNER", params.owner.clone()),
        (
            "KURA_COMMUNITY_ID",
            auth.tenant.community().as_uuid().to_string(),
        ),
        ("KURA_PUSHER_PUBKEY", pusher_hex.clone()),
    ];
    ...
    S: futures_util::Stream<Item = Result<bytes::Bytes, std::io::Error>>,
{
    ...
    fn poll_next(
        ...
        if self.deadline.as_mut().poll(cx).is_ready() {
            self.finished = true;
            metrics::counter!("kura_git_upload_pack_timeouts_total").increment(1);
...
impl<S> Drop for TimedByteStream<S> {
    fn drop(&mut self) {
        metrics::histogram!("kura_git_upload_pack_stream_seconds")
            .record(self.started_at.elapsed().as_secs_f64());
        metrics::histogram!("kura_git_upload_pack_stream_bytes").record(self.streamed_bytes as f64);
    }
}
...
async fn finalize_push_inner(
    ...
    let serving_write = match kura_deletion::acquire_serving_write(
        &state.db,
        ...
        match build_ref_state_event(&inputs, &state.relay_keypair) {
            Ok(event) => {
                ...
                let insert_result = if hooks.fail_ref_state_insert {
                    Err(kura_db::DbError::InvalidData(
...
mod track_c_tests {
    ...
    fn receive_pack_config_allows_deleting_current_branch() {
        ...
        assert_git_success(
            run_test_git(source.as_path(), &["config", "user.name", "Kura Test"], &[]),
            "configure user name",
        );
        assert_git_success(
            run_test_git(
                source.as_path(),
                &["config", "user.email", "kura-test@example.com"],
                &[],
            ),
    ...
    async fn finalize_test_state() -> (Arc<AppState>, sqlx::PgPool) {
        ...
        config.database_url = std::env::var("KURA_TEST_DATABASE_URL")
    ...
    async fn approved_deletion(
        ...
        kura_db::deletion::DeletionRequest,
        kura_db::deletion::ClaimedDeletion,
    ) {
        ...
        let inventory = FrozenInventory {
            ...
            storage: StorageManifest {
                version: 4,
                prefixes: kura_media::tenant_prefixes(*request.community_id.as_uuid())
                    .into_iter()
    ...
    async fn repo_announcement_holds_serving_lease_until_pointer_is_seeded() {
        ...
        assert!(matches!(
            error,
            kura_db::DbError::ServingWritesNotDrained { .. }
        ));
        ...
        assert_eq!(
            state
                ...
                .stage,
            kura_db::deletion::DeletionStage::Fenced
        );
        drop(state);
        pool.close().await;
    }
    ...
    async fn finalize_push_holds_serving_lease_through_post_cas_publication() {
        ...
        let mut query = kura_db::event::EventQuery::for_community(community);
    ...
    fn git_nip98_rejects_token_signed_for_wrong_community_host() {
        ...
        let err = kura_auth::nip98::verify_nip98_event(&event_json, &expected_for_b, "GET", None)
    ...
    fn git_nip98_accepts_token_signed_for_matching_community_host() {
        ...
        let pubkey =
            kura_auth::nip98::verify_nip98_event(&event_json, &expected_for_a, "GET", None)
...
mod sec005_read_gate_tests {
    ...
    fn durable_ban_denies_git_even_with_otherwise_valid_auth() {
        let restriction = kura_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
    ...
    fn restriction(banned: bool) -> kura_db::moderation::RestrictionState {
        kura_db::moderation::RestrictionState {
            banned,
            muted_until: None,
        }
    }
    ...
    async fn setup_db() -> kura_db::Db {
        let url = std::env::var("KURA_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| TEST_DB_URL.to_string());
        let pool = sqlx::PgPool::connect(&url).await.expect("connect test DB");
        kura_db::Db::from_pool(pool)
    }

    /// How the fixture's kind:30617 binds (or fails to bind) a channel.
    enum Binding {
        /// `kura-channel` tag carrying the fixture channel's UUID.
        Channel,
        /// No `kura-channel` tag at all.
        Missing,
        /// `kura-channel` tag whose value is not a UUID.
        Malformed,
        /// `kura-channel` tag carrying a well-formed UUID that names no
    ...
    async fn setup_repo(binding: Binding) -> RepoFixture {
        ...
        db.create_channel_with_id(
            ...
            kura_db::channel::ChannelType::Stream,
            kura_db::channel::ChannelVisibility::Open,
            None,
            &creator_pk,
            None,
        )
        ...
        db.add_member(
            ...
            channel,
            &member_pk,
            kura_core::channel::MemberRole::Member,
            Some(&creator_pk),
        )
        .await
        .expect("member");
        ...
        match binding {
            Binding::Channel => {
                tags.push(Tag::parse(["kura-channel", &channel.to_string()]).unwrap());
            }
            Binding::Missing => {}
            Binding::Malformed => {
                tags.push(Tag::parse(["kura-channel", "not-a-uuid"]).unwrap());
            }
            Binding::UnknownChannel => {
                tags.push(Tag::parse(["kura-channel", &uuid::Uuid::new_v4().to_string()]).unwrap());
            }
        }
    ...
    async fn read_gate_denies_missing_or_malformed_binding_and_absent_repo() {
        // Missing kura-channel tag → deny even for a channel member, with
        ...
        // Malformed kura-channel tag → deny with the generic body EVEN FOR
    ...
    async fn read_gate_gives_author_of_unbound_repo_remediation_body() {
        ...
        assert!(
            body.starts_with(&format!("run: kura repos bind --id {}", f.repo)),
    ...
    async fn setup_ban_community() -> (kura_db::Db, kura_core::CommunityId, Vec<u8>) {
        let db = setup_db().await;
    ...
    async fn ban_gate_cascades_to_a_banned_nip_oa_owner() {
        ...
        let auth_tag = kura_sdk::nip_oa::compute_auth_tag(&owner, &agent.public_key(), "kind=9")
            .expect("auth tag");
    ...
    async fn ban_gate_fails_closed_with_503_when_the_store_is_unreachable() {
        ...
        let community = kura_core::CommunityId::from_uuid(uuid::Uuid::new_v4());
        let (status, body) = denial_parts(
