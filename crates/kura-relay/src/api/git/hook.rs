...
/// - `KURA_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `KURA_HOOK_SECRET` — per-push HMAC secret
/// - `KURA_REPO_ID` — repo identifier (d-tag)
/// - `KURA_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `KURA_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# Kura pre-receive hook — FAIL-CLOSED
...
: "${KURA_REPO_ID:?error: KURA_REPO_ID not set}"
: "${KURA_REPO_OWNER:?error: KURA_REPO_OWNER not set}"
: "${KURA_COMMUNITY_ID:?error: KURA_COMMUNITY_ID not set}"
: "${KURA_PUSHER_PUBKEY:?error: KURA_PUSHER_PUBKEY not set}"
: "${KURA_HOOK_URL:?error: KURA_HOOK_URL not set}"
: "${KURA_HOOK_SECRET:?error: KURA_HOOK_SECRET not set}"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$KURA_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$KURA_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${KURA_REPO_OWNER}\",\"community_id\":\"${KURA_COMMUNITY_ID}\",\"pusher_pubkey\":\"${KURA_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
