...
//! GIF bytes are never downloaded, cached, or stored by Kura.
//!
...
async fn enforce_search_admission(
    ...
    .await
    {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("kura_gif_search_rejections_total", "reason" => "quota").increment(1);
            Err(api_error(
...
mod tests {
    ...
    async fn unconfigured_test_state() -> Arc<AppState> {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            .connect_lazy("postgres://kura:kura_dev@127.0.0.1:1/kura") // sadscan:disable np.postgres.1
        ...
        let media_storage =
            kura_media::MediaStorage::new(&config.media).expect("test media storage config");
        let (state, _audit_shutdown) = AppState::new(
            config,
            db,
            redis_pool,
            None::<kura_audit::AuditService>,
            pubsub,
            auth,
            search,
            workflow_engine,
