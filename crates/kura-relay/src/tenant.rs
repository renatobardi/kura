...
//! implementation that queries the `communities` table lives in `kura-db`
//! (`Db::resolve_host`); the relay depends on the trait, not the query, so the
//! binding is testable without a database.

use kura_core::tenant::{normalize_host, CommunityId, TenantContext};
...
/// [`kura_core::tenant::normalize_host`] — [`bind_community`] guarantees that,
...
pub async fn bind_deployment_community<R: HostResolver>(
    ...
    bind_community(resolver, &kura_core::tenant::relay_url_authority(relay_url)).await
}
...
///
/// This is a thin re-export of [`kura_core::tenant::relay_url_authority`]: the
/// canonical implementation lives in `kura-core` so the relay seam *and* the
/// `kura-admin` CLI derive a byte-identical authority (same port/IPv6 handling).
pub use kura_core::tenant::relay_url_authority;

/// Production [`HostResolver`]: the relay resolves hosts against the durable
/// `communities` host map in Postgres.
///
/// This is the *only* place the relay couples the row-zero seam to kura-db. The
/// trait keeps `bind_community` and every call site database-free and testable;
/// this impl is the thin adapter from kura-db's `lookup_community_by_host`
...
impl HostResolver for kura_db::Db {
    type Error = kura_db::DbError;
...
mod tests {
    ...
    mod redteam_attack2 {
        ...
        /// `cargo test -p kura-relay --include-ignored
        ...
        /// trims to empty (`kura-core::tenant::normalize_host_empty_stays_empty`),
        ...
        async fn whitespace_only_raw_host_fails_closed_even_if_db_has_empty_host_row() {
            ...
            let err = bind_community(&r, "   ").await.expect_err(
                "Inv_RowZero: whitespace-only raw_host normalizes to empty \
                 (see kura-core::tenant::normalize_host) and carries no \
                 community evidence",
            );
            assert!(
                matches!(err, BindError::UnmappedHost),
