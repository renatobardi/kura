...
//! from `kura-conformance`. See `crates/kura-conformance/src/lib.rs` for
...
///   [`kura_db::Kura::communities_of_channels`]). On a hit, return the
...
fn project_row_community(
    ...
    channel_communities: &std::collections::HashMap<Uuid, kura_core::CommunityId>,
...
/// [`kura_db::Kura::communities_of_channels`] over the distinct
/// channel ids in `rows`.
