...
/// A1: KURA_ACP_MODEL must NOT be present in the spawned-child env after
/// `apply_claude_model_env`, even if it was set before (dual-authority defect).
/// ANTHROPIC_MODEL must be set to the resolved model.
#[test]
fn a1_kura_acp_model_absent_anthropic_model_present_after_env_apply() {
    let mut cmd = std::process::Command::new("true");
    // Simulate descriptor.env writing KURA_ACP_MODEL (the pre-A1 path).
    cmd.env("KURA_ACP_MODEL", "claude-opus-4");
    apply_claude_model_env(&mut cmd, Some("claude-opus-4"));

    let env_map: std::collections::HashMap<_, _> = cmd.get_envs().collect();

    // KURA_ACP_MODEL must be removed. Command::get_envs returns None for
    // explicitly-removed keys.
    let kura_acp = env_map.get(std::ffi::OsStr::new("KURA_ACP_MODEL"));
    assert!(
        kura_acp.is_none() || kura_acp.unwrap().is_none(),
        "KURA_ACP_MODEL must be absent (or explicitly removed) after A1 policy"
    );
...
fn a1_anthropic_model_removed_when_no_effective_model() {
    ...
    cmd.env("KURA_ACP_MODEL", "claude-3-5-sonnet");
    ...
    assert!(
        kura_acp.is_none() || kura_acp.unwrap().is_none(),
        "KURA_ACP_MODEL must always be absent after A1 policy"
    );
}
...
/// Simulates the defect scenario: descriptor.env wrote KURA_ACP_EFFORT_LEVEL=low,
...
fn b5_canonical_effort_wins_over_user_env_collision() {
    ...
    cmd.env("KURA_ACP_EFFORT_LEVEL", "low");

    // Post-loop canonical application — the fix.
    apply_effort_env(&mut cmd, Some("high"));

    let env_map: std::collections::HashMap<_, _> = cmd.get_envs().collect();
    let effort = env_map.get(std::ffi::OsStr::new("KURA_ACP_EFFORT_LEVEL"));
    assert!(effort.is_some(), "KURA_ACP_EFFORT_LEVEL must be present");
    assert_eq!(
...
/// Simulates: descriptor.env wrote KURA_ACP_EFFORT_LEVEL=low (already in command),
...
fn b5_canonical_effort_injected_when_no_user_collision() {
    let mut cmd = std::process::Command::new("true");
    // No user-supplied KURA_ACP_EFFORT_LEVEL in descriptor.env.
