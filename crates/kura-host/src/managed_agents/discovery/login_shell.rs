...
pub fn login_shell_candidates() -> Vec<PathBuf> {
    #[cfg(not(windows))]
    {
...
pub fn find_via_login_shell(command: &str) -> Option<PathBuf> {
...
pub fn refresh_login_shell_path() {
    let mut guard = path_cache().lock().unwrap_or_else(|e| e.into_inner());
...
#[cfg(test)]
pub fn is_login_shell_path_uninit() -> bool {
    matches!(
...
pub fn is_safe_nvm_tag(tag: &str) -> bool {
    if tag.is_empty() {
        return false;
    }
...
pub fn parse_semver_tag(s: &str) -> Option<(u64, u64, u64)> {
