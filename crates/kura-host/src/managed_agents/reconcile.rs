...
pub fn reconcile_agents_to_events(app: &HostHandle, keys: &nostr::Keys, db_path: &Path) {
    ...
    match reconcile_agents_in_dir_at(&base_dir, keys, db_path) {
        ...
        Ok(reconciled) => {
            eprintln!(
                "kura-desktop: agent-event-reconcile: {reconciled} agents reconciled to retention"
            );
        }
        Err(e) => {
            eprintln!("kura-desktop: agent-event-reconcile: {e}");
        }
    }
}
...
#[cfg(test)]
pub fn reconcile_agents_in_dir(base_dir: &Path, keys: &nostr::Keys) -> Result<u32, String> {
