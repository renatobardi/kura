...
fn merge_personas_adds_missing_built_ins() {
    ...
    assert!(records
        .iter()
        .any(|record| record.id == "builtin:hayate" && record.runtime.is_none()));
    let display_names: Vec<&str> = records
        .iter()
        .map(|record| record.display_name.as_str())
        .collect();
    assert_eq!(display_names, vec!["Hayate", "Kaede", "Rin"]);
    ...
    assert_eq!(
        active_ids,
        vec!["builtin:hayate", "builtin:kaede", "builtin:rin"]
    );
}
...
fn merge_personas_preserves_builtin_edits() {
    let mut edited_builtin = custom_persona("builtin:hayate", "My Hayate");
    ...
    let hayate = records
        .iter()
        .find(|record| record.id == "builtin:hayate")
        .expect("hayate built-in should exist");
    assert_eq!(hayate.display_name, edited_builtin.display_name);
    assert_eq!(hayate.system_prompt, edited_builtin.system_prompt);
    assert_eq!(hayate.name_pool, edited_builtin.name_pool);
    assert_eq!(hayate.env_vars, edited_builtin.env_vars);
    assert_eq!(hayate.is_active, edited_builtin.is_active);
}
...
fn merge_personas_restores_builtin_marker_without_resetting_edits() {
    ...
    let hayate = records
        ...
        .find(|record| record.id == "builtin:hayate")
        .expect("hayate built-in should exist");
    assert!(hayate.is_builtin);
    assert_eq!(hayate.display_name, "My Hayate");
}
...
fn ensure_persona_is_active_rejects_inactive_personas() {
    let mut persona = custom_persona("builtin:hayate", "Hayate");
    persona.is_builtin = true;
    persona.is_active = false;

    let err = ensure_persona_is_active(&[persona], "builtin:hayate").unwrap_err();

    assert_eq!(err, "Hayate is not in My Agents.");
}
...
fn validate_persona_activation_change_rejects_managed_agent_references() {
    ...
    assert_eq!(
        err,
        "Hayate is still assigned to a managed agent. Remove or reassign those agents first."
    );
}
...
fn validate_persona_activation_change_rejects_team_references() {
    ...
    assert_eq!(
        err,
        "Hayate is still referenced by a team. Remove it from those teams first."
    );
}
...
fn migrate_retires_unmodified_personas() {
    ...
    // Simulate a store from before the Hayate transition: all 6
...
// ── Hayate default harness ──────────────────────────────────────────────────────

#[test]
fn fizz_builtin_has_no_pinned_runtime() {
    // The Hayate built-in must not hard-pin a runtime so it inherits the
    // bundled default (kura-agent) rather than requiring goose on PATH.
    let records = built_in_persona_records("2026-01-01T00:00:00Z");
    let hayate = records
        .iter()
        .find(|r| r.id == "builtin:hayate")
        .expect("builtin:hayate must exist");
    assert_eq!(
        hayate.runtime, None,
        "Hayate built-in must not pin a runtime — it should inherit the default"
    );
}

#[test]
fn fizz_builtin_resolves_to_kura_agent() {
    // With no runtime pin, effective_agent_command must fall through to
    // default_agent_command(), which resolves the bundled kura-agent.
    let records = built_in_persona_records("2026-01-01T00:00:00Z");
    assert_eq!(
        effective_agent_command(Some("builtin:hayate"), &records, None),
        default_agent_command(),
        "Hayate must resolve to the bundled default harness, not goose"
    );
    assert_eq!(
        effective_agent_command(Some("builtin:hayate"), &records, None),
        "kura-agent",
        "Hayate must resolve to kura-agent specifically"
    );
}
