...
/// JSON as `"identifier":"pro.oute.kura.app.dev"` and in environment entries
...
#[cfg(target_os = "macos")]
pub fn reap_dead_instance_agents(our_instance_id: &str, skip_pids: &[u32]) {
    let my_uid = unsafe { libc::getuid() };
...
#[cfg(not(unix))]
pub fn reap_dead_instance_agents(_our_instance_id: &str, _skip_pids: &[u32]) {}
