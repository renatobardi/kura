//! `kura-team-snapshot v1` — manifest type, encoder, and decoder.
//!
...
//!   - `.team.png` — 1×1 placeholder PNG with manifest in a `kura_team_snapshot`
...
pub const PNG_CHUNK_KEYWORD: &str = "kura_team_snapshot";

/// Format discriminator — used for sniffing and validation.
pub const FORMAT_DISCRIMINATOR: &str = "kura-team-snapshot";
...
/// The top-level `kura-team-snapshot v1` manifest.
///
/// Serializes to / from JSON. Embedded in `.team.json` directly, or in the
/// `kura_team_snapshot` tEXt chunk of a `.team.png` (base64-encoded).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
...
/// `kura_team_snapshot` tEXt chunk (base64-encoded JSON).
...
pub fn decode_team_snapshot_png(png_bytes: &[u8]) -> Result<TeamSnapshot, String> {
    ...
    let chunk_text = info
        ...
        .map(|c| c.text.as_str())
        .ok_or_else(|| "PNG does not contain a kura_team_snapshot tEXt chunk".to_string())?;
...
pub fn validate_team_snapshot(snapshot: &TeamSnapshot) -> Result<(), String> {
...
mod tests {
    ...
    fn format_and_version_correct_in_output() {
        let json = team_json_string();
        assert!(
            json.contains("\"kura-team-snapshot\""),
    ...
    fn decode_rejects_agent_snapshot_json_as_team_snapshot() {
        // An agent snapshot JSON (format: "kura-agent-snapshot") must NOT
