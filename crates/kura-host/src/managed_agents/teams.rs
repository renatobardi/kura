...
pub fn teams_store_path(app: &HostHandle) -> Result<PathBuf, String> {
...
pub fn load_teams_readonly(path: &std::path::Path) -> Result<Vec<TeamRecord>, String> {
    let now = now_iso();
...
pub fn load_teams(app: &HostHandle) -> Result<Vec<TeamRecord>, String> {
...
pub fn save_teams(app: &HostHandle, records: &[TeamRecord]) -> Result<(), String> {
...
pub fn delete_team_with_cascade(app: &HostHandle, team_id: &str) -> Result<Vec<String>, String> {
...
pub fn deactivate_catalog_member_copies_with_ref_check(
