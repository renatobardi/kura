...
pub fn built_in_persona_definition(id: &str, now: &str) -> Option<AgentDefinition> {
...
pub fn load_personas(app: &HostHandle) -> Result<Vec<AgentDefinition>, String> {
    let now = now_iso();
...
pub fn load_personas_from_path(path: &std::path::Path) -> Result<Vec<AgentDefinition>, String> {
    if !path.exists() {
        return Ok(Vec::new());
    }
...
pub fn save_personas(app: &HostHandle, records: &[AgentDefinition]) -> Result<(), String> {
