...
pub fn managed_agents_base_dir(app: &HostHandle) -> Result<PathBuf, String> {
    let dir = app
        .app_data_dir()
...
pub fn managed_agents_store_path(app: &HostHandle) -> Result<PathBuf, String> {
    Ok(managed_agents_base_dir(app)?.join("managed-agents.json"))
}

fn managed_agents_logs_dir(app: &HostHandle) -> Result<PathBuf, String> {
...
pub fn install_log_path(app: &HostHandle, runtime_id: &str) -> Result<PathBuf, String> {
...
pub fn managed_agent_log_path(app: &HostHandle, pubkey: &str) -> Result<PathBuf, String> {
...
pub fn latest_managed_agent_log_path(app: &HostHandle, pubkey: &str) -> Result<PathBuf, String> {
    ...
    match store.probe(&name) {
        ...
        KeyringProbe::Present | KeyringProbe::ReachableButEmpty => {
            match store.write_and_verify(&name, &record.private_key_nsec) {
                ...
                Err(e) => {
                    eprintln!(
                        "kura-desktop: keyring write for agent {} failed ({e}), keeping inline",
                        record.pubkey
                    );
...
/// `KURA_PRIVATE_KEY`/`NOSTR_PRIVATE_KEY`, launching with no identity. Callers
/// (the spawn path) must fail closed (Wes storage.rs:158).
pub fn spawn_key_refusal(record: &ManagedAgentRecord) -> Option<String> {
...
fn load_agent_store(app: &HostHandle) -> Result<Vec<ManagedAgentRecord>, String> {
...
pub fn load_managed_agents(app: &HostHandle) -> Result<Vec<ManagedAgentRecord>, String> {
...
pub fn load_agent_definitions(app: &HostHandle) -> Result<Vec<ManagedAgentRecord>, String> {
    let mut records = load_agent_store(app)?;
    records.retain(|record| record.pubkey.is_empty());
    Ok(records)
}
...
pub fn backup_invalid_store(path: &Path) {
    let backup = path.with_extension("json.invalid");
    if let Err(e) = fs::copy(path, &backup) {
        eprintln!(
            "kura-desktop: failed to preserve malformed store {} as {}: {e}",
            path.display(),
            backup.display()
        );
    }
    ...
    for record in records.iter_mut() {
        ...
        if record.private_key_nsec.is_empty() {
            match store.load(&agent_keyring_name(&record.pubkey)) {
                ...
                Ok(None) => {
                    eprintln!(
                        "kura-desktop: agent {} has no key in JSON or keyring",
                        record.pubkey
                    );
                }
                ...
                Err(e) => {
                    eprintln!(
                        "kura-desktop: agent {} key unavailable — keyring read failed ({e}); \
...
pub fn save_managed_agents(app: &HostHandle, records: &[ManagedAgentRecord]) -> Result<(), String> {
...
/// (`"kura-desktop"`) to the dev service (`"kura-desktop-dev"`). Only runs
/// in debug builds — release builds never touch `"kura-desktop"` from this
/// path.
///
...
pub fn migrate_agent_keys_to_dev_service(app: &HostHandle) {
    if !cfg!(feature = "system-keyring") || keyring_service() != "kura-desktop-dev" {
        return;
    }
    ...
    let records = match load_agent_store(app) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("kura-desktop: keyring-dev-migration: cannot read agent store: {e}");
            return;
        }
    };
    ...
    // singleton's cache.
    let prod_store = crate::secret_store::SecretStore::keyring("kura-desktop");
...
fn copy_agent_keys_between_stores(pubkeys: &[String], src: &impl KeyStore, dst: &impl KeyStore) {
    ...
    let dst_map: HashMap<String, String> = match dst.load_all_readonly() {
        ...
        Ok(None) => HashMap::new(),
        Err(e) => {
            eprintln!("kura-desktop: keyring-dev-migration: cannot read dev keyring: {e}");
            return;
        }
    };
        ...
        .all(|pubkey| dst_map.contains_key(&agent_keyring_name(pubkey)))
    {
        ...
        match src.load_all_readonly() {
            ...
            Err(e) => {
                eprintln!("kura-desktop: keyring-dev-migration: cannot read prod keyring: {e}");
                return;
            }
        }
    };
    ...
    if let Err(e) = dst.store_all(&to_write) {
        eprintln!("kura-desktop: keyring-dev-migration: cannot write to dev keyring: {e}");
        return;
    }

    if copied > 0 {
        eprintln!(
            "kura-desktop: keyring-dev-migration: copied {copied} agent key(s) from kura-desktop"
        );
    }
}
...
pub fn try_delete_agent_key(pubkey: &str) -> Result<(), String> {
...
pub fn delete_agent_key(pubkey: &str) {
    if let Err(e) = try_delete_agent_key(pubkey) {
        eprintln!("kura-desktop: failed to delete agent {pubkey} key from keyring: {e}");
    }
}
...
pub fn atomic_write_json(path: &Path, payload: &[u8]) -> Result<(), String> {
...
/// already reached).
pub type StoreSnapshot = Option<Vec<u8>>;

/// Snapshot the raw bytes of `path`, or `None` if the file is absent.
pub fn snapshot_store(path: &Path) -> Result<StoreSnapshot, String> {
...
pub fn restore_store(path: &Path, snap: StoreSnapshot) -> Result<(), String> {
    match snap {
...
pub fn commit_stores_with_snapshots(
    personas_path: &Path,
    teams_path: &Path,
...
pub fn start_install_log_session(path: &Path) -> Result<File, String> {
    if path.exists() {
...
pub fn open_install_log_file(path: &Path) -> Result<File, String> {
...
fn agent_pids_dir(app: &HostHandle) -> Result<PathBuf, String> {
...
pub fn remove_agent_runtime_receipt(app: &HostHandle, key: &ManagedAgentRuntimeKey) {
...
pub fn remove_agent_pid_file(app: &HostHandle, pubkey: &str) {
    if let Ok(dir) = agent_pids_dir(app) {
...
pub fn read_all_agent_pid_files(app: &HostHandle) -> Vec<(String, u32)> {
    let Ok(dir) = agent_pids_dir(app) else {
        return Vec::new();
    };
    let Ok(entries) = fs::read_dir(&dir) else {
...
pub fn meaningful_agent_error_from_log(path: &Path) -> Option<AgentLogError> {
    ...
    tail.lines().rev().map(str::trim).find_map(|line| {
        ...
        // Legacy format (older kura-acp builds): "Agent reported error: ..."
