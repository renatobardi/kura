...
pub fn spawn_warm_init(app: HostHandle) {
    let handle = app.clone();
    app.spawn(Box::pin(async move {
        if let Err(error) = handle.state().archive_db.warm_init().await {
...
pub async fn create_save_subscription(
    state: &AppState,
    sync_state: &sync::ArchiveSyncState,
...
pub async fn read_unindexed_observer_rows(state: &AppState) -> Result<Vec<RawObserverRow>, String> {
    let identity_pk = identity_pubkey(state)?;
    let relay_url = relay_ws_url_with_override(state);
    state
        .archive_db
...
pub async fn get_observer_retention_days(state: &AppState) -> Result<i64, String> {
    state
        .archive_db
...
pub async fn set_observer_retention_days(state: &AppState, days: i64) -> Result<(), String> {
    state
        .archive_db
...
pub async fn archive_size_stats(state: &AppState) -> Result<retention::ArchiveSizeStats, String> {
