...
/// app handle.
pub trait ArchiveSyncIo: Send + Sync + 'static {
...
fn plan_subscriptions(
    ...
    for sub in subscriptions {
        let Some(scope_type) = parse_scope_type(&sub.scope_type) else {
            eprintln!(
                "kura-desktop: archive sync: unknown scope_type {:?}, skipping",
                sub.scope_type
            );
            continue;
        };
...
async fn reconcile<I: ArchiveSyncIo + ?Sized>(io: &I, scopes: &mut HashMap<String, MatchedScope>) {
    let subscriptions = match io.list_subscriptions().await {
        ...
        Err(error) => {
            eprintln!("kura-desktop: archive sync: list_save_subscriptions failed: {error}");
            return;
        }
    };
...
async fn flush<I: ArchiveSyncIo + ?Sized>(io: &I, candidates: Vec<ArchiveCandidate>) {
    ...
    match io.archive(candidates).await {
        ...
        Ok(_) => {}
        Err(error) => eprintln!("kura-desktop: archive sync: archive_events failed: {error}"),
    }
}
...
impl ArchiveSyncIo for AppIo {
    ...
    fn notify_agent_metrics_changed(&self) {
        let _ = self
            .app
            .emit(AGENT_METRICS_CHANGED_EVENT, serde_json::Value::Null);
    }
}
...
/// main-window-owned.
pub async fn announce_archive_sync_epoch(sync_state: &ArchiveSyncState) -> Result<u64, String> {
