...
#[cfg(not(unix))]
pub fn create_symlink(_target: &std::path::Path, _link: &std::path::Path) -> std::io::Result<()> {
    Ok(())
}
...
pub fn symlink_points_to(link: &std::path::Path, target: &std::path::Path) -> bool {
    link.is_symlink()
...
pub fn backup_path(dst: &std::path::Path) -> Option<std::path::PathBuf> {
...
#[cfg(unix)]
pub fn replace_with_symlink(src: &std::path::Path, dst: &std::path::Path) -> u32 {
    if dst.is_symlink() {
...
#[cfg(not(unix))]
pub fn replace_with_symlink(_src: &std::path::Path, _dst: &std::path::Path) -> u32 {
    0
}
...
pub fn create_restricted_backup_once(
    path: &std::path::Path,
    source_bytes: &[u8],
) -> Result<bool, String> {
...
///
/// [`atomic_write_json_restricted`]: crate::managed_agents::atomic_write_json_restricted
pub fn resolved_backup_path(store_path: &std::path::Path, backup_name: &str) -> std::path::PathBuf {
