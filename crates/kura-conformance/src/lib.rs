...
/// trace schema.
///
/// This deliberately does NOT reuse `kura_core::CommunityId`. Two reasons:
///
/// 1. **Production fence preservation.** `kura_core::CommunityId` has no
...
impl CommunityLabel {
    /// Wrap a UUID into a community label. Unlike `kura_core::CommunityId`
