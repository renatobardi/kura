...
//!
//! These types live in `kura-core` (zero I/O deps) so the DB, auth, pub/sub,
...
/// the `kura-admin` CLI's tenant resolution. All of these must derive the
...
mod tests {
    ...
    fn relay_url_authority_keeps_explicit_nondefault_port() {
        ...
        // kura-admin must all derive `localhost:3000` (NOT bare `localhost`),
