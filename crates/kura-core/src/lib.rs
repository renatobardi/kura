#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `kura-core` — zero-I/O foundation types for the Kura relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other Kura crates depend on this one.
...
pub mod invite;
/// Kura kind number registry — custom event type constants.
pub mod kind;
