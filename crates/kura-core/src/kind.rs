//! Kura V2 kind number registry.
//!
//! This module is the authoritative source for Kura kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Kura today.
...
/// Kura custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Kura command kinds (Pure Nostr plan)
...
/// `crates/kura-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `kura-db`.
///
...
/// Kura product feedback submission. Accepted at ingest, sidecarred to the
...
// Kura community moderation commands (mod-signed, processed like 9030-series:
...
/// Kura: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — Kura requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Kura workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Kura command kind that requires transactional execution.
...
/// Safe: all Kura kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Kura kind constants fit in nostr's u16-backed Kind.
