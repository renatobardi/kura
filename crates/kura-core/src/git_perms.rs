...
//!
//! This module defines the core data types for the Kura git permission system.
//! The permission model: channel role = repo role; `kura-protect` tags on
...
/// announcement with no `kura-channel` binding.
///
...
/// Maximum number of `kura-protect` tags per repo.
...
/// A single protection rule parsed from a `kura-protect` tag on kind:30617.
///
/// Format: `["kura-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `kura-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for RuleParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TooFewValues => write!(f, "kura-protect tag needs pattern + at least one rule"),
...
/// Parse a single `kura-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["kura-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("kura-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `kura-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `kura-protect` tag, returning unknown rules for logging.
...
/// Parse all `kura-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `kura-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("kura-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `kura-protect` tag matches.
