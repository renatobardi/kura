...
async fn insert_event_on(
    ...
    // Cast chain: nostr Kind (u16) → i32 (Postgres INT column). Safe: all Kura kinds fit in i32.
...
impl Db {
    ...
    /// seam is gated on `KURA_REPLICA_READ_MAX_AGE_MS` (default off): when
...
mod tests {
    ...
    async fn event_insert_ttl_trigger_handles_permanent_ephemeral_duplicate_and_activation_race() {
        ...
        sqlx::query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))")
            .bind(format!("kura_channel_ttl:{community_uuid}:{racing}"))
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let keys = Keys::generate();
        let kind = kura_core::kind::KIND_PROJECT as i32;
