...
pub use kura_core::channel::MemberRole;
...
/// `kura_channel_ttl:`.
const CHANNEL_MEMBERSHIP_LOCK_NAMESPACE: &str = "kura_channel_membership:";
...
impl LockedMemberSnapshot {
    ...
    pub async fn replace_member_event(
        ...
        channel_id: Uuid,
        event: &nostr::Event,
    ) -> Result<(kura_core::StoredEvent, bool)> {
        ...
        let kind = kura_core::kind::event_kind_i32(event);
        if kind != 39002 {
            return Err(DbError::InvalidData(
        ...
        if let Some((existing_ts, existing_id)) = existing {
            ...
            {
                return Ok((
                    kura_core::StoredEvent::with_received_at(
                        event.clone(),
                        Utc::now(),
                        Some(channel_id),
                        false,
