...
async fn replace_parameterized_event_in_transaction_impl(
    ...
    let kind_i32 = kura_core::kind::event_kind_i32(event);
    ...
    let read_state_t_tag_count = event
        ...
        .count();
    let is_nip_rs = kind_i32 == kura_core::kind::KIND_READ_STATE as i32
        && d_tag_count == 1
        && has_exact_d_tag
    ...
    let is_kura_mesh_status = kind_i32 == kura_core::kind::KIND_BOOKMARK_SET as i32
        && d_tag.starts_with("kura-mesh-member-status:")
        && event.tags.iter().any(|tag| {
            let parts = tag.as_slice();
            parts.len() == 2 && parts[0] == "k" && parts[1] == "kura-mesh-status"
        });
    let hard_delete_superseded = is_nip_rs || is_kura_mesh_status;
    ...
    if existing.is_some() {
        let previous_nip_rs_hard_delete: Option<String> = if is_nip_rs {
            sqlx::query_scalar(
                "SELECT NULLIF(current_setting('kura.nip_rs_hard_delete', true), '')",
            )
        ...
        if is_nip_rs {
            sqlx::query("SELECT set_config('kura.nip_rs_hard_delete', 'on', true)")
        ...
        if is_nip_rs {
            ...
            sqlx::query("SELECT set_config('kura.nip_rs_hard_delete', $1, true)")
                .bind(previous_value)
                .execute(&mut *savepoint)
                .await?;
        }
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://kura:kura_dev@localhost:5432/kura"; // sadscan:disable np.postgres.1 -- local test-only credentials
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(kura_core::kind::KIND_READ_STATE as u16), "old")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old");
        let new = EventBuilder::new(Kind::Custom(kura_core::kind::KIND_READ_STATE as u16), "new")
            .tags(tags)
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign new");
    ...
    async fn nip_rs_transaction_operation_restores_hard_delete_opt_in() {
        ...
        let event = |d_tag: &str, content: &str, timestamp: u64| {
            EventBuilder::new(
                Kind::Custom(kura_core::kind::KIND_READ_STATE as u16),
                content,
            )
            .tags(vec![
    ...
    async fn parameterized_replacement_in_existing_transaction_honors_revision_and_rollback() {
        ...
        let event = |content: &str, timestamp: u64| {
            EventBuilder::new(Kind::Custom(kura_core::kind::KIND_PROJECT as u16), content)
        ...
        .bind(community.as_uuid())
        .bind(kura_core::kind::KIND_PROJECT as i32)
        .bind(keys.public_key().to_bytes())
        .bind(&d_tag)
        .fetch_one(&db.pool)
        .await
        ...
        let missing = EventBuilder::new(
            Kind::Custom(kura_core::kind::KIND_PROJECT as u16),
            "missing",
        )
        .tags(vec![
    ...
    async fn parameterized_replacement_rolls_back_when_mention_indexing_fails() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(kura_core::kind::KIND_PROJECT as u16), "old")
            .tags(tags())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old project");
        let new = EventBuilder::new(Kind::Custom(kura_core::kind::KIND_PROJECT as u16), "new")
            .tags(tags())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let d_tag = "kura-mesh-member-status:owner-test";
        let tags = vec![
            Tag::parse(["d", d_tag]).expect("d tag"),
            Tag::parse(["k", "kura-mesh-status"]).expect("k tag"),
        ];
        let base = Timestamp::now().as_secs();
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(kura_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        for commit in [true, false] {
            ...
            let value: String =
                sqlx::query_scalar("SELECT set_config('kura.nip_rs_hard_delete', 'on', true)")
                    .fetch_one(&mut *tx)
                    .await
