...
const PUSH_GATE_LOCK_NAMESPACE: &str = "kura_push_gate:";
...
pub struct BatchedMatch {
    ...
    pub event: kura_core::StoredEvent,
    /// Attempt number, starting at one for the first claim.
    pub attempt: i32,
}
...
where
    ...
    Fut: std::future::Future<Output = Result<Vec<kura_core::StoredEvent>>>,
{
...
mod tests {
    ...
    async fn setup_pool() -> PgPool {
        let database_url = std::env::var("KURA_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://kura:kura_dev@localhost:5432/kura".into()); // sadscan:disable np.postgres.1 -- local test-only credentials
