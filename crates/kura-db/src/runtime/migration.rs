//! Embedded SQLx migrations for Kura.
//!
...
/// Run all pending Kura database migrations.
///
...
mod tests {
    ...
    /// URL. Only the shapes this test emits (`KURA_TEST_DATABASE_URL` /
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        assert!(mesh_retention.contains("kura-mesh-member-status:%"));
        assert!(mesh_retention.contains("kura-mesh-status"));
        assert!(mesh_retention
            .contains("CREATE TRIGGER trg_events_purge_soft_deleted_kura_mesh_status"));
        assert!(!migrations[0]
            .sql
            .as_str()
            .contains("purge_soft_deleted_kura_mesh_status"));
        ...
        assert!(push_gate.contains("'kura_push_gate:' || NEW.community_id::text"));
        ...
        assert!(ttl_shared.contains("'kura_channel_ttl:' || NEW.community_id::text"));
        ...
        assert!(roster_fence.contains("'kura_channel_membership:'"));
    ...
    fn migration_execution_cannot_bypass_schema_destruction_lock() {
        ...
        // exemption honest.
        let push_gateway_exception = crates_dir.join("kura-push-gateway/src/postgres.rs");
        let push_gateway_migrations = crates_dir.join("kura-push-gateway/migrations");
        for entry in
            std::fs::read_dir(&push_gateway_migrations).expect("read push gateway migrations")
        {
            ...
            assert!(
                ...
                "{} defines community-scoped data; its migrator would bypass the \
                 schema/destruction lock and must move under kura-db migrations",
                path.display()
            );
        }
        let mut files = Vec::new();
            ...
            if *path == this_file {
                ...
                assert_eq!(
                    ...
                    "{} embeds or runs a SQLx migrator outside the schema/destruction \
                     ...
                     kura_db migration::run_migrations",
                    path.display()
                );
            }
        }
    ...
    async fn cancelled_migration_cannot_expose_shared_lock_while_ddl_backend_lives() {
        ...
        let base_url = std::env::var("KURA_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| TEST_DB_URL.to_owned());
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("kura_lock_cancel_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn admin_schema_parity_between_desired_state_and_migrations() {
        ...
        let desired_db = format!("kura_admin_desired_{}", uuid::Uuid::new_v4().simple());
        let migrated_db = format!("kura_admin_migrated_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn run_migrations_applies_consolidated_initial_schema_on_fresh_database() {
        ...
        sqlx::query(
            "SELECT set_config('kura.deletion_executor_community', $1, true), \
                    set_config('kura.deletion_fence_generation', '1', true)",
        )
