            ...
            ReadSessionInner::Replica { tx, writer } => {
                match event::query_events_on(tx, q).await {
                    ...
                    Err(e) => {
                        ...
                        // Deliberately not a `kura_db_route_decision` event:
                        // the page's route was already recorded, and the
                        // offload metric must stay one-event-per-request.
                        metrics::counter!("kura_db_read_session_degraded").increment(1);
                        writer.clone()
                    }
                }
            }
...
pub(crate) enum RouteDecision {
    ...
    /// failure emits exactly one `kura_db_route_decision` event per request
...
impl RoutePredicate {
    ...
    ///
    /// `routing_enabled` is whether `KURA_REPLICA_READ_MAX_AGE_MS` is set
...
/// Map the configured read budget (`KURA_REPLICA_READ_MAX_AGE_MS`) to the
...
pub struct DbConfig {
    ...
    /// `KURA_DB_READ_POOL_SIZE`). `None` inherits [`Self::max_connections`].
    ...
    /// `KURA_REPLICA_READ_MAX_AGE_MS`). `0` disables bounded-staleness
...
impl Default for DbConfig {
    ...
    fn default() -> Self {
        Self {
            database_url: "postgres://kura:kura_dev@localhost:5432/kura".to_string(), // sadscan:disable np.postgres.1
...
impl Db {
    ...
    /// `kura.created_at_floor` GUC — this is what makes the replica fence
        ...
        let options = PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SELECT set_config('kura.created_at_floor', $1, false)")
    ...
    /// the migration decision. On a relay with `KURA_AUTO_MIGRATE` off, the
    ...
    > {
        ...
        .await
        {
            ...
            // `kura_db_route_decision{decision="writer",reason="reader_acquire_timeout"}`
            ...
            // Runbook: correlate with `kura_db_read_pool_active` / `_max`
            ...
            // sample (KURA_POOL_METRICS_INTERVAL_SECS, default 10s) while
    ...
    /// the writer's: `kura_db_read_pool_active / kura_db_read_pool_max` is
    /// the operator's utilisation signal for tuning `KURA_DB_READ_POOL_SIZE`,
    ...
    pub async fn insert_event_with_serving_write_guard(
        ...
        if kura_core::kind::is_ephemeral(kind_u32) {
