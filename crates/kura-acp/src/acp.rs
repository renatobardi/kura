...
pub struct AcpClient {
    ...
    ///
    /// Both goose and kura-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/kura-agent emit `activeRunId: null` at end of turn).
    ...
    /// Usage tracker for goose/kura-agent's cumulative notification format.
...
///
/// Returns `Some(json_string)` when `has_generated_codex_config` is true (Kura injected a
...
pub(crate) fn build_codex_config_env(
    ...
    // Without an explicit Kura-generated overlay signal, skip the merge entirely.
...
/// `_meta.goose.activeRunId`. Emitted by goose and kura-agent only.
...
/// `outcome` value meaning the steer was applied to the turn Kura is waiting
/// on, which therefore keeps running.
const STEER_OUTCOME_INJECTED: &str = "injected";

/// `outcome` value meaning the turn Kura was steering had already finished, so
...
fn build_client_capabilities() -> serde_json::Value {
    serde_json::json!({
        // Signal to ACP adapters that Kura can hand users to terminal-native
        // auth flows. Adapters decide which auth methods to expose; Kura does
...
impl AcpClient {
    ...
    pub async fn spawn(
        ...
        // Per-persona env vars (e.g., GOOSE_PROVIDER, KURA_AGENT_PROVIDER).
    ...
    ///   (ACP protocol v2, kura-agent, goose unused).
    ...
    /// `["/cmd args", "<kura context>"]` instead of one wrapped block.
    ...
    ///
    /// Both goose and kura-agent emit `session_info_update`; other agents
    ...
    /// Consume per-turn usage for NIP-AM publishing. Goose/kura-agent is an
    ...
    /// Notify the usage tracker that kura-acp just spawned a new session.
    ///
    ...
    /// Must be called only when kura-acp created the session via `session/new`;
                            ...
                            if let Some((steer_id, _, _)) = pending_steer.as_ref() {
                                if *id == serde_json::json!(*steer_id) {
                                    ...
                                    let ack = if let Some(error) = msg.get("error") {
                                        ...
                                        // turn Kura awaits is still running —
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and kura-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and kura-agent both
...
///
/// - **`Field`** — bare `systemPrompt` field (ACP protocol v2, kura-agent).
...
mod tests {
    ...
    async fn spawn_named_and_read_child_env(
        ...
        let dir = std::env::temp_dir().join(format!("kura-acp-env-probe-{}", uuid::Uuid::new_v4()));
    ...
    /// Kura-owned Hermes processes get the configured-MCP isolation default,
    ...
    async fn goose_system_prompt_request_uses_set_contract() {
        ...
        assert_eq!(received["params"]["key"], "kura");
    ...
    async fn session_new_full_sends_session_title_in_meta_when_some() {
        ...
        let resp = client
            .session_new_full("/tmp", vec![], None, Some("Fizz · #kura-dev"))
            .await
            .expect("session_new_full should succeed");
    ...
    /// `SessionInfoUpdate` schema), matching what goose and kura-agent
    /// emit on the wire.
    ...
    fn capture_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("kura-acp-steer-capture");
    ...
    /// Test 1b: no `_meta` at all (goose, kura-agent, any older adapter) must
    ...
    /// Kura maps `SteerAck::Success` to `queue.remove_event`, so decoding
    ...
    /// the turn Kura was steering had already ended and the adapter began a
    ...
    fn build_codex_config_env_generated_only_single_entry_with_signal_true_merges_with_parent() {
        // No persona: Kura injects one CODEX_CONFIG; signal=true.
    ...
    fn build_codex_config_env_persona_only_signal_false_returns_none() {
        // Persona set CODEX_CONFIG; Kura did not inject a generated overlay (signal=false).
