//! Harness-side Kura relay client.
//!
//! Connects to the Kura relay via NIP-01 WebSocket, authenticates via NIP-42,
...
//! - Forwards `KuraEvent`s through an `mpsc` channel
...
/// Override with `KURA_ACP_EVENT_BUFFER` env var at startup.
...
fn event_channel_capacity() -> usize {
    std::env::var("KURA_ACP_EVENT_BUFFER")
        .ok()
...
impl RestClient {
    ...
    /// lookup needs `#kura-channel`, which this path serializes verbatim.
...
///
/// Connects to the Kura relay, authenticates via NIP-42, and streams
...
pub struct HarnessRelay {
    ...
    event_rx: mpsc::Receiver<Option<KuraEvent>>,
...
impl HarnessRelay {
    ...
    pub async fn connect(
        ...
        let (event_tx, event_rx) = mpsc::channel::<Option<KuraEvent>>(event_channel_capacity());
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                kura_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
    ...
    pub async fn next_event(&mut self) -> Option<KuraEvent> {
...
async fn run_background_task(
    ...
    event_tx: mpsc::Sender<Option<KuraEvent>>,
            ...
            match relay_msg {
                RelayMessage::Event {
                    ...
                    if subscription_id == OBSERVER_CONTROL_SUB_ID {
                        ...
                        match event_tx.try_send(Some(kura_event)) {
                            Ok(()) => {
...
/// sends (`crates/kura-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        kura_sdk::build_agent_observer_frame(
