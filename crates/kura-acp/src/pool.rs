...
pub struct ChannelDeliveryState {
    ...
    /// Kura event IDs already delivered to this ACP session, either as trigger
...
pub struct OwnedAgent {
    ...
    /// Persisted startup effort value from `KURA_ACP_EFFORT_LEVEL` (carried from
...
pub struct PromptContext {
    ...
    /// `--no-memory` / `KURA_ACP_NO_MEMORY`.
    pub memory_enabled: bool,
    /// Harness identity string for NIP-AM `harness` field. Derived from the
    /// configured `agent_command` at startup (e.g. `"goose"`, `"kura-agent"`).
...
async fn create_session_and_apply_model(
    ...
    // application above. The held value comes from `KURA_ACP_EFFORT_LEVEL` and
...
fn mcp_servers_with_git_origin(
    ...
    let origin = match (channel_id, channel_type) {
        (Some(channel_id), Some("stream")) => Some(EnvVar {
            name: "KURA_GIT_ORIGIN_CHANNEL_ID".into(),
            value: channel_id.to_string(),
        }),
        (Some(_), _) => agent_name
            .filter(|name| !name.trim().is_empty())
            .map(|name| EnvVar {
                name: "KURA_GIT_ORIGIN_AGENT_NAME".into(),
...
pub async fn run_prompt_task(
    ...
    //
    // Operator opt-out: `--no-memory` / `KURA_ACP_NO_MEMORY` skips the fetch.
            ...
            if let Some(sid) = agent.state.sessions.get(cid) {
                ...
                .await
                {
                    Ok(sid) => {
                        ...
                        // Seed a zero usage baseline: kura-acp spawned this session
    ...
    // (`prompt[0].text.startsWith("/")`) fires; the wrapped Kura context
                ...
                mode = rx => {
                    ...
                    if agent.acp.has_in_flight_prompt() {
                            ...
                            .await
                        {
                            Ok(stop_reason) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(kura_core::agent_turn_metric::StopReason::Cancelled),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                            ...
                            Err(error) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(kura_core::agent_turn_metric::StopReason::Error),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                        ...
                        publish_agent_turn_metric(
                            ...
                            &session_id,
                            &turn_id,
                            Some(kura_core::agent_turn_metric::StopReason::EndTurn),
                        )
                        .await;
                        send_prompt_result(
                            &result_tx,
...
async fn fetch_channel_info_once(channel_id: Uuid, rest: &RestClient) -> Option<PromptChannelInfo> {
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            kura_core::kind::KIND_NIP29_GROUP_METADATA as u16,
        ))
...
pub(crate) async fn fetch_project_home_for_channel(
    ...
    let filters = [
        serde_json::json!({
            "kinds": [kura_core::kind::KIND_PROJECT],
            "#kura-channel": [channel],
        }),
        serde_json::json!({
            "kinds": [kura_core::kind::KIND_GIT_REPO_ANNOUNCEMENT],
            "#kura-channel": [channel],
        }),
    ];
    ...
    let (projects, repos): (Vec<_>, Vec<_>) = events.into_iter().partition(|event| {
        event.get("kind").and_then(serde_json::Value::as_u64)
            == Some(kura_core::kind::KIND_PROJECT as u64)
    });
...
async fn fetch_huddle_instructions(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            kura_core::kind::KIND_HUDDLE_GUIDELINES as u16,
        ))
        .author(*owner)
...
fn huddle_instructions_from_query_response(
    ...
    if event.pubkey != *owner
        || event.kind.as_u16() as u32 != kura_core::kind::KIND_HUDDLE_GUIDELINES
        || !event
            .tags
            .iter()
...
async fn fetch_canvas_section(channel_id: Uuid, rest: &RestClient) -> Option<String> {
    ...
    let h_tag = SingleLetterTag::lowercase(Alphabet::H);
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(kura_core::kind::KIND_CANVAS as u16))
        .custom_tags(h_tag, [channel_id.to_string()])
        .limit(1);
...
pub(crate) fn canvas_section_from_query_response(
    ...
    if event.kind != nostr::Kind::Custom(kura_core::kind::KIND_CANVAS as u16) {
        tracing::warn!(
            target: "canvas::fetch",
            channel = %channel_uuid,
    ...
    crate::prompt_framing::semantic_section(
        ...
        &format!(
            "Canvas revision (event ID): {event_id}\n\
             ...
             Fetch current content with: kura canvas get --channel {channel_uuid}"
        ),
    )
}
...
/// IDs are compared in Kura's canonical 64-character lowercase hex form: relay
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let replies_filter = nostr::Filter::new()
        .kinds([
            nostr::Kind::Custom(kura_core::kind::KIND_STREAM_MESSAGE as u16),
            nostr::Kind::Custom(kura_core::kind::KIND_STREAM_MESSAGE_V2 as u16),
        ])
...
fn delivery_receipt_line(channel_id: Uuid, event_ids: &HashSet<String>) -> String {
    ...
    format!(
        "turn delivered Kura events for channel {channel_id}: {}",
        event_ids.join(",")
    )
}
...
fn acp_stop_to_core(r: &StopReason) -> kura_core::agent_turn_metric::StopReason {
    use kura_core::agent_turn_metric::StopReason as CoreStop;
    match r {
...
pub(crate) fn build_turn_metric_counts(
    ...
    Option<kura_core::agent_turn_metric::TokenCounts>,
    Option<kura_core::agent_turn_metric::TokenCounts>,
) {
    use kura_core::agent_turn_metric::TokenCounts;
    ...
    let cumulative_counts = Some(TokenCounts {
        ...
        // reported this field (e.g. goose or older kura-agent sessions).
...
async fn publish_agent_turn_metric(
    ...
    session_id: &str,
    turn_id: &str,
    stop_reason: Option<kura_core::agent_turn_metric::StopReason>,
) {
    use kura_core::agent_turn_metric::AgentTurnMetricPayload;
    ...
    let ciphertext = match kura_core::agent_turn_metric::encrypt_agent_turn_metric(
        &ctx.agent_keys,
        owner_pk,
        &payload,
    ) {
    ...
    let event = match EventBuilder::new(
        Kind::Custom(kura_core::kind::KIND_AGENT_TURN_METRIC as u16),
        ciphertext,
    )
    .tags([
...
///
/// Builds a reaction event with `kura_sdk::build_reaction`, signs it with
...
pub(crate) async fn reaction_add(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match kura_sdk::build_reaction(target_id, emoji) {
        Ok(b) => b,
        Err(e) => {
...
pub(crate) async fn post_failure_notice(
    ...
    let thread_ref = thread_tags.root_event_id.as_deref().and_then(|root| {
        ...
        let parent_id = thread_tags
            ...
            .unwrap_or(root_id);
        Some(kura_sdk::ThreadRef {
            root_event_id: root_id,
            parent_event_id: parent_id,
        })
    });
    let builder =
        match kura_sdk::build_message(channel_id, content, thread_ref.as_ref(), &[], false, &[]) {
            Ok(b) => b,
            Err(e) => {
...
pub(crate) async fn reaction_remove(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match kura_sdk::build_remove_reaction(target_id) {
        Ok(b) => b,
        Err(e) => {
...
mod tests {
    ...
    fn delivery_receipt_line_sorts_event_ids() {
        ...
        assert_eq!(
            delivery_receipt_line(channel_id, &event_ids),
            format!("turn delivered Kura events for channel {channel_id}: alpha,beta")
        );
    }
    ...
    fn public_session_forwards_channel_origin_to_mcp() {
        ...
        assert!(servers[0].env.iter().any(|entry| {
            entry.name == "KURA_GIT_ORIGIN_CHANNEL_ID" && entry.value == channel_id.to_string()
        }));
        assert!(!servers[0]
            .env
            .iter()
            .any(|entry| entry.name == "KURA_GIT_ORIGIN_AGENT_NAME"));
    }
    ...
    fn private_session_forwards_agent_name_without_channel_id() {
        ...
        assert!(servers[0].env.iter().any(|entry| {
            entry.name == "KURA_GIT_ORIGIN_AGENT_NAME" && entry.value == "Builder"
        }));
        assert!(!servers[0]
            .env
            .iter()
            .any(|entry| entry.name == "KURA_GIT_ORIGIN_CHANNEL_ID"));
    }
    ...
    fn goose_uses_system_prompt_only_after_custom_method_succeeds() {
        ...
        assert!(has_system_prompt_support(2, "kura-agent", None));
        ...
        assert_eq!(
            session_new_system_prompt(false, 2, "kura-agent", Some("instructions")),
    ...
    fn test_workspace_section_preserves_windows_cwd() {
        assert_eq!(
            workspace_section(r"C:\Users\me\kura"),
    ...
    async fn run_prompt_task_commits_standing_context_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "kura-acp-standing-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn channel_prompt_commits_delivery_state_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "kura-acp-channel-delivery-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn merged_cancel_prompt_commits_and_deduplicates_all_rendered_event_ids() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "kura-acp-merged-delivery-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  count=$((count + 1))
    ...
    async fn late_successful_steer_ack_excludes_event_from_next_channel_wire_prompt() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "kura-acp-late-steer-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    /// A kura-agent-commanded context must not panic — verifies the harness
    /// field flows through encrypt/sign without error.
    #[tokio::test]
    async fn test_publish_agent_turn_metric_kura_agent_harness_name() {
        let agent_keys = nostr::Keys::generate();
        let owner_keys = nostr::Keys::generate();
        let mut ctx = make_prompt_context_with_owner(&agent_keys, owner_keys.public_key());
        ctx.harness_name = "kura-agent".to_string();
        let usage = crate::usage::TurnUsage {
            session_id: "sess-ba".to_string(),
            turn_seq: 1,
            delta_reliable: false, // first turn from kura-agent
    ...
    fn test_build_turn_metric_counts_cache_read_tokens_thread_through() {
        // Wire-parse a kura-agent payload with cache, run it through the tracker,
    ...
    fn huddle_instructions_require_owner_signature_and_channel() {
        ...
        let event = |keys: &Keys, channel_id: Uuid| {
            ...
            serde_json::to_value(
                EventBuilder::new(
                    Kind::Custom(kura_core::kind::KIND_HUDDLE_GUIDELINES as u16),
                    "reply immediately",
                )
                .tags([h_tag])
                .sign_with_keys(keys)
    ...
    fn test_render_canvas_section_produces_exact_shape() {
        ...
        assert_eq!(
            ...
            "<channel-canvas>\n\
             ...
             Fetch current content with: kura canvas get --channel 00f1ccaf-1506-4dd7-9a0e-fa67e9e486ae\n\
             </channel-canvas>"
        );
    }
    ...
    fn make_canvas_event_value(content: &str) -> serde_json::Value {
        ...
        let event = EventBuilder::new(Kind::Custom(kura_core::kind::KIND_CANVAS as u16), content)
            .tags([h_tag])
            .sign_with_keys(&keys)
            .expect("sign");
    ...
    fn test_canvas_section_from_query_response_happy_path() {
        ...
        assert!(section.contains("kura canvas get --channel"));
    ...
    fn test_canvas_section_from_query_response_string_timestamp_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(Kind::Custom(kura_core::kind::KIND_CANVAS as u16), "content")
                .tags([h_tag])
                .sign_with_keys(&keys)
                .expect("sign"),
        )
    ...
    fn test_canvas_section_from_query_response_tampered_event_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(
                Kind::Custom(kura_core::kind::KIND_CANVAS as u16),
                "original",
            )
            .tags([h_tag])
            .sign_with_keys(&keys)
    ...
    async fn expired_absence_refreshes_to_project_without_restart() {
        ...
        let responses = [
            json!([{
                "kind": 30621,
                "pubkey": owner,
                "tags": [["d", "app"], ["kura-channel", channel], ["a", coordinate]]
            }]),
            json!([{
                "kind": 30617,
                "pubkey": "a".repeat(64),
                "tags": [["d", "app"], ["kura-channel", id.to_string()]]
            }]),
        ];
    ...
    async fn indeterminate_project_context_never_reaches_acp_prompt_boundary() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "kura-acp-indeterminate-project-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
  ...
  printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'
    ...
    async fn resolve_finds_authoritative_project_beyond_first_bridge_page() {
        ...
        let first_page: Vec<_> = (0..500)
            .map(|index| {
                json!({
                    ...
                    "kind": 30621,
                    "pubkey": "b".repeat(64),
                    "tags": [["d", format!("decoy-{index}")], ["kura-channel", channel]]
                })
            })
            .collect();
        let responses = [
        ...
        let server = tokio::spawn(async move {
            while let Ok((mut socket, _)) = listener.accept().await {
                ...
                if index > 0 {
                    assert!(request.contains("#kura-channel"));
                }
                let body = responses[index].to_string();
                let response = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{}",
    ...
    async fn test_new_session_channel_context_qualifies_a_normal_channel() {
        ...
        let id = Uuid::new_v4();
        let response = channel_metadata_response(id, &[["name", "kura-dev"], ["t", "stream"]]);
        ...
        assert_eq!(title_channel.as_deref(), Some("kura-dev"));
        ...
        assert_eq!(again.as_deref(), Some("kura-dev"));
        assert_eq!(
            requests.load(Ordering::SeqCst),
            4,
