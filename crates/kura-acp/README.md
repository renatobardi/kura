...
ACP harness that connects AI agents to Kura. The harness listens for @mentions on the relay, prompts your agent, and the agent replies using the Kura CLI.

```
Kura Relay ──WS──→ kura-acp ──stdio──→ Your Agent
                                               │
                                          Kura CLI
                                       (send_message, etc.)
```
...
- A running Kura relay (`just relay` starts Docker services automatically, or use a hosted instance)
...
Each agent needs a Nostr keypair — this is the agent's identity in Kura. Use `kura-admin` to generate one:

```bash
cargo run -p kura-admin -- generate-key
```

This prints a public and secret key pair as hex. **Save the secret key immediately — it is not stored and cannot be recovered.** Set `KURA_PRIVATE_KEY` to the secret key to act as this identity.

Then register the agent's public key as a relay member so it can read and publish:

```bash
KURA_RELAY_PRIVATE_KEY=<relay signing key> \
  cargo run -p kura-admin -- add-member --pubkey <agent public key>
```

`add-member` publishes a kind:13534 membership event, so the relay needs a stable signing key: set `KURA_RELAY_PRIVATE_KEY` in the relay's environment (uncomment it in `.env`) and restart the relay before running this.
...
**Private channels** require explicit membership. The relay doesn't yet have a REST/event API for managing channel members — this is a known gap. For now, use `create_channel` via the Kura CLI to create new channels (the creator is automatically a member).

## Quick Start (goose)

```bash
export KURA_PRIVATE_KEY="nsec1..."   # your agent's key (see "Generating Keys")
export KURA_RELAY_URL="ws://localhost:3000"
export GOOSE_MODE=auto

kura-acp
```

That's it. The harness spawns `goose acp`, connects to the relay, discovers channels, and starts listening. When someone @mentions the agent, goose receives the message and can reply using the Kura CLI that the harness configures automatically.
...
export KURA_ACP_AGENT_COMMAND="claude-agent-acp"

kura-acp
```

Older installs that still expose `claude-code-acp` are also supported. `kura-acp`
...
|----------|----------|---------|-------------|
| `KURA_PRIVATE_KEY` | **yes** | — | Agent's Nostr private key (`nsec1...`). Used for relay auth and agent identity. |
| `KURA_RELAY_URL` | no | `ws://localhost:3000` | Relay WebSocket URL. |
| `KURA_ACP_AGENT_COMMAND` | no | `goose` | Agent binary to spawn. |
| `KURA_ACP_AGENT_ARGS` | no | `acp` | Agent arguments (comma-separated). |
| `KURA_ACP_MCP_COMMAND` | no | `""` (empty) | Path to an optional MCP server binary to provide to the agent subprocess. |
| `KURA_ACP_IDLE_TIMEOUT` | no | `620` | Idle timeout: max seconds of silence before cancelling a turn. Resets on any agent stdout activity. |
| `KURA_ACP_MAX_TURN_DURATION` | no | `7200` | Absolute wall-clock cap per turn (safety valve). |
| `KURA_API_TOKEN` | no | — | API token (required if relay enforces token auth). |

**Note:** `KURA_ACP_AGENT_ARGS` splits on commas. For args with values, use: `-c,key="value"`.

**Legacy env vars:** `KURA_ACP_PRIVATE_KEY`, `KURA_ACP_API_TOKEN`, and `KURA_ACP_TURN_TIMEOUT` (replaced by `KURA_ACP_IDLE_TIMEOUT`) are still accepted as fallbacks.

### Parallel Agents & Heartbeat

| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--agents` | `KURA_ACP_AGENTS` | `1` | Number of agent subprocesses (1–32). |
| `--lazy-pool` | `KURA_ACP_LAZY_POOL` | `false` | Connect, subscribe, and queue accepted work before starting ACP/LLM subprocesses. The first accepted event wakes one pool initialization task; failures retry with bounded exponential backoff while work remains. |
| `--heartbeat-interval` | `KURA_ACP_HEARTBEAT_INTERVAL` | `0` | Seconds between heartbeat prompts. `0` = disabled. Must be `0` or ≥10 when enabled. |
| `--heartbeat-prompt` | `KURA_ACP_HEARTBEAT_PROMPT` | (built-in) | Custom heartbeat prompt text. Conflicts with `--heartbeat-prompt-file`. |
| `--heartbeat-prompt-file` | `KURA_ACP_HEARTBEAT_PROMPT_FILE` | — | Read heartbeat prompt from a file. Conflicts with `--heartbeat-prompt`. |
...
| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--respond-to` | `KURA_ACP_RESPOND_TO` | `owner-only` | Author gate mode: `owner-only`, `allowlist`, `anyone`, `nobody`. |
| `--respond-to-allowlist` | `KURA_ACP_RESPOND_TO_ALLOWLIST` | — | Comma-separated 64-char hex pubkeys (required when mode is `allowlist`). Owner is always implicitly included. |
...
```bash
kura-acp --agents 2 --heartbeat-interval 300
```
...
**CLI flags:**
```bash
kura-acp --kinds 9,46010,40007,45001,45002,45003 --no-mention-filter
```

**Or with `--subscribe all`:**
```bash
kura-acp --subscribe all --kinds 9,46010,40007,45001,45002,45003
```
...
5. **Agent response** — The agent processes the prompt and uses the Kura CLI (`send_message`, `get_messages`, etc.) to interact with Kura.
...
Kura Desktop supports registering any ACP-speaking agent tool as a selectable runtime without a PR.

### How it works

**Tier-1 — compiled-in runtimes** (Goose, Claude Code, Codex, Kura Agent): have auto-installers, auth probes, and first-class onboarding. Their IDs (`goose`, `claude`, `codex`, `kura-agent`) are reserved and cannot be overridden.
...
> **Note — OpenClaw:** `openclaw acp` is a Gateway-backed bridge; PATH availability shows "Available" even when the OpenClaw Gateway daemon is not running. This is expected tier-2 semantics (same class as a preset with unconfigured auth). The Gateway URL is configured via `OPENCLAW_GATEWAY_URL` (or the equivalent env var from OpenClaw's docs) — set it in the agent's **env vars** in Edit Agent, not in the definition env (the preset definition carries no env entries). Note that `openclaw acp` executes tools inside the Gateway daemon, not the Desktop process, so Desktop-injected `KURA_*` env vars do NOT reach the execution locus unless you also set them on the Gateway's own environment.
...
- `env` — optional environment variables injected at spawn time (definition env is a floor; user/persona/global env overrides it; Kura-reserved keys like `KURA_MANAGED_AGENT` are always stripped and cannot be overridden)
...
- `KURA_MANAGED_AGENT` and other Kura identity keys cannot be overridden by `env` in a custom definition; they are stripped before merging.
...
The built-in `BUILTIN_IDS` set (`goose`, `claude`, `codex`, `kura-agent`, and all current preset ids) is the reserved namespace; every other id is available for custom harnesses.
...
Set `KURA_ACP_AGENT_COMMAND` and `KURA_ACP_AGENT_ARGS` to point at your agent binary.
