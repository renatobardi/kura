...
///
/// Shell children receive `path_env`, `git_env`, and `KURA_PRIVATE_KEY` (for
/// the kura CLI). `NOSTR_PRIVATE_KEY` is removed from the process env after
...
impl Shim {
    pub fn install() -> std::io::Result<Self> {
        let dir = tempfile::Builder::new().prefix("kura-dev-mcp-").tempdir()?;
...
fn write_keyfile(shim_dir: &Path, raw: &str) -> Option<KeyInfo> {
    ...
    let keys = match nostr::Keys::parse(raw) {
        ...
        Err(e) => {
            eprintln!(
                "kura-dev-mcp: warning: NOSTR_PRIVATE_KEY is set but invalid ({e}); \
    ...
    if write_keyfile_atomic(&keyfile, raw.as_bytes()).is_err() {
        eprintln!(
            "kura-dev-mcp: warning: failed to write nostr keyfile; git auth/signing disabled"
        );
        return None;
    }
    let keyfile_path = match keyfile.to_str() {
        ...
        None => {
            eprintln!(
                "kura-dev-mcp: warning: tempdir path is not valid UTF-8; git auth/signing disabled"
            );
            return None;
        }
    };
...
/// Format: `<hex_pubkey>@<relay_host>` (e.g., `ab12...cd@relay.kura.dev`).
/// Falls back to `<hex_pubkey>@kura` if no relay URL is configured.
fn derive_git_email(pubkey_hex: &str) -> String {
    let host = std::env::var("KURA_RELAY_URL")
        .ok()
        .and_then(|url| {
...
///
/// Deliberately distinct from `KURA_ACP_SESSION_TITLE`, which is per-session UI
...
const DISPLAY_NAME_ENV_VAR: &str = "KURA_ACP_DISPLAY_NAME";
...
/// Normalize a Kura display name into a git author name, or `None` to fall
/// back to the npub.
///
...
/// via kura-agent (which clears env), the base is always 0 — composition only
...
fn build_git_env(info: &KeyInfo) -> Vec<(String, String)> {
    ...
    let entries: Vec<(&str, String)> = vec![
        // Identity — Kura display name (npub fallback), NIP-05-style email
        ("user.name", user_name),
        ("user.email", email),
        // Nostr credential helper is additive — it silently declines non-Kura
        ...
        // Required: Kura relay verifies NIP-98 against the full repo-root URL.
...
mod git_user_name_tests {
    ...
    fn test_build_git_env_uses_display_name_and_leaves_email_on_the_pubkey() {
        ...
        std::env::remove_var("KURA_RELAY_URL");
        ...
        assert_eq!(
            ...
            Some(format!("{PUBKEY_HEX}@kura").as_str())
        );
        assert_eq!(
    ...
    fn test_build_git_env_falls_back_to_npub_when_display_name_unset() {
        ...
        // KURA_ACP_DISPLAY_NAME lands on the Desktop side.
    ...
    fn test_build_git_env_falls_back_to_npub_when_display_name_is_unusable() {
        ...
        for raw in ["<>", "\u{200B}"] {
            std::env::set_var("KURA_ACP_DISPLAY_NAME", raw);
            let env = build_git_env(&key_info());
            assert_eq!(
