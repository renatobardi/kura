...
//! # Run one binary, KURA_HEALTH_PORT=8180 KURA_METRICS_PORT=9202,
//! # KURA_RECONCILE_CHANNELS=false, KURA_GIT_CONFORMANCE_PROBE=false
//! ```
//!
//! Each test is `#[ignore]` so it only runs explicitly:
//! `cargo test -p kura-test-client --test nip42_host_binding_live -- --ignored --test-threads=1`
