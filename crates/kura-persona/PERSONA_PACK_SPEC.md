...
for deployment in Kura. It is a **superset of the [Open Plugin Spec](https://open-plugin-spec.org)**
...
1. **Portable** — zip file or git repo; no Kura tooling required to inspect
...
4. **Harness-honest** — explicit about what the agent runtime does vs. what kura-acp does
...
and Kura-specific extensions live alongside the OPS fields at the top level. Since the Open
...
```json
{
  ...
  "version": "1.2.0",
  "description": "A four-agent security review team for Kura.",
  "author": "Meadow Engineering",
  "license": "MIT",
  "homepage": "https://github.com/example/meadow-security-team",
  "keywords": ["security", "code-review", "kura"],
  "engines": {
    "kura": ">=0.9.0"
  },
  "personas": [
...
- **Kura**: reads both OPS fields and the Kura-specific fields; `personas` is authoritative.
- **Version negotiation**: `engines.kura` specifies minimum required Kura version; kura-acp
  rejects packs requiring a newer version.
- **Extension mechanism**: Kura-specific fields sit at the top level of `plugin.json` alongside
  OPS fields. No OPS core field is overloaded.
- **`defaults`**: ignored entirely by OPS consumers. kura-acp resolves it at deploy time before
...
└── my-pack-1.2.0.kurapack.sha256  # Checksum (required for zip distribution)
```
...
# === Behavioral Config (Kura-specific) ===
subscribe:
  - "#security-reviews"
  - "#code-reviews"
triggers:
...
| `display_name` | string | ✅ | Human-readable name shown in Kura UI. |
...
kura-acp assembles the agent's context from two distinct prompt layers before sending each
...
```
[Base]
<platform orientation — injected by kura-acp>
...
The `[Base]` layer is compiled into kura-acp and is **identical for every agent**. It covers:

| Content | Purpose |
|---------|---------|
| Platform identity | Tells the agent it is running inside Kura and what that means |
...
Pack authors do not write or configure the `[Base]` layer — it is maintained by the Kura team
and updated in kura-acp releases.

**Disabling or customizing the base layer**: Set `KURA_ACP_NO_BASE_PROMPT` to omit the `[Base]`
section entirely. To replace the compiled-in default with custom content, set
`KURA_ACP_BASE_PROMPT_FILE` to a file path — kura-acp reads it at startup and uses it instead.
...
- That the agent is running inside Kura (covered by `[Base]`)
...
> **Note**: `.agents/skills/` is kura-acp's canonical skill location. The other paths shown
...
$AGENT_CWD/.agents/skills/<skill-name>/SKILL.md   ← kura-acp uses this one
```
...
kura-acp process. kura-acp determines what value to pass as `NewSessionRequest.cwd` in this
order:

1. The `AGENT_CWD` environment variable on the kura-acp process, if set.
2. `std::env::current_dir()` as a fallback.
3. If both fail, kura-acp logs an error and **refuses to start**.
...
is **not overwritten**. This allows operators to pin custom skill versions. kura-acp **must log
...
kura-acp lists available skills in the user message prefix so the agent knows what's available.
...
by the agent runtime. `kura pack validate` warns on skill name mismatches but does not yet
...
kura-acp merges them and passes the result via the ACP protocol — no filesystem placement required.
...
When implemented, all `env` values will be scanned for `${VAR_NAME}`. kura-acp will resolve
from the process environment **before** passing to the agent runtime. Unresolved variables will
cause a startup error.

### Delivery

kura-acp passes the merged config via `NewSessionRequest.mcp_servers`. **No `.mcp.json` is written to the agent's working directory.**
...
in the pack. kura-acp appends it to the persona prompt in the user message prefix.

kura-acp appends `instructions.md` to the persona prompt in the user message prefix (see
...
Hooks are shell commands fired by kura-acp at agent lifecycle points. **Agent runtimes typically have no hook system**
...
Hooks run as the kura-acp user; working directory is `$AGENT_CWD`; agent env vars are available.
...
that must be implemented directly in kura-acp's dispatch loop, not via a hook.
...
`on_stop` fires on normal exit and on handled errors. It **will not fire** if kura-acp crashes
...
**Hooks are NOT agent runtime features.** They are implemented entirely in kura-acp. Bypassing
kura-acp means no hooks fire.
...
Kura conversations. These are all Kura-specific — the agent runtime has no awareness of them. They sit
...
In this spec, **"deploy time"** means when kura-acp loads the pack and constructs per-persona
session configurations — typically at kura-acp process startup. For git-based packs, this occurs
each time kura-acp starts and reads the installed pack directory.

When kura-acp resolves the effective configuration for a persona, it applies this order (highest
wins):

```
1. Operator env vars           — e.g. GOOSE_MODEL, GOOSE_PROVIDER (agent-runtime-specific)
                                 already set in the parent process environment
2. Desktop UI per-agent        — overrides set in the Kura desktop app per-agent settings
...
5. Built-in defaults           — kura-acp's hardcoded fallback values
```

kura-acp resolves levels 3–5 at deploy time (when the pack is loaded and sessions are
constructed). Levels 1–2 are applied at runtime and are outside the pack's control.

**Level 1 — Operator env vars**: If the operator has already set env vars for model, provider, temperature, or context limit in the parent process environment, kura-acp MUST NOT override them with pack/persona values. kura-acp only injects env vars for fields that are NOT already set in the parent environment. This ensures operators can always override pack configuration.

**Implementation**: when constructing the child process environment, kura-acp checks
...
| `temperature` | float | `0.7` | Provider-dependent (typically 0.0–2.0). kura-acp passes through without range validation; `kura pack validate` checks type only (must be a number), not range. | Passed as env var to agent runtime. |
...
**Unknown keys** in `defaults` (in `plugin.json`) are **validation warnings** in `kura pack
...
kura-acp logs a `WARN` and ignores unknown manifest keys, remaining fail-soft:
...
The `#` prefix in `subscribe` entries is a **display convention only**. Channel names in the Kura
relay are stored and queried **without** the `#` prefix. kura-acp strips the leading `#` before
making any relay API calls. `"#security-reviews"` and `"security-reviews"` are equivalent in this
field.

### Env Var Mapping

kura-acp resolves pack defaults and per-persona overrides (precedence levels 3–5) into a single
...
kura-acp translates persona behavioral config fields to agent configuration via environment
...
If `model` is omitted from both the persona frontmatter and `defaults`, kura-acp does not set
`GOOSE_PROVIDER` or `GOOSE_MODEL`, and the agent runtime uses its configured operator default.

> **Implementation note**: `AcpClient::spawn` accepts per-persona env vars via the `extra_env` parameter. kura-acp checks `std::env::var(key)` before injecting each var — if the parent environment already has the key set, injection is skipped (operator precedence, level 1).
...
> Field Schema table lists kura-acp's built-in fallbacks (precedence level 5). If `defaults` is
...
All fields are consumed entirely by kura-acp. None are passed to the agent runtime directly — they are projected as env vars or used by the harness's subscription/dispatch logic.
...
A pack is distributed as a `.kurapack` file (zip archive):

```bash
kura pack validate ./my-pack
kura pack ./my-pack --output my-pack-1.2.0.kurapack
kura install ./my-pack-1.2.0.kurapack
kura install https://example.com/releases/my-pack-1.2.0.kurapack
```

#### Pack Integrity (Required)

Zip packs **must** ship with `<pack-name>-<version>.kurapack.sha256` containing `sha256sum`
output (`<hex-digest>  <filename>`). kura-acp **must** verify before installation and refuse on
...
```json
{
  "com.example.meadow-security-team": {
    "source": "https://example.com/releases/my-pack-1.2.0.kurapack",
    "sha256": "a3f1c2d4e5b6...",
    "version": "1.2.0",
...
```bash
kura install github:example/meadow-security-team
kura install github:example/meadow-security-team@v1.2.0
kura install git+https://gitlab.example.com/team/pack.git
```
...
A Kura-hosted registry and in-app browser for discovering and installing packs. API-compatible
with OPS registries. Details TBD.

### Installed Pack Location

Installed packs live at `~/.kura/packs/<pack-id>/`. kura-acp reads packs from this location
at agent startup.

### Desktop App Import

The Kura desktop app's **Agents** page does not import persona-pack `.zip` archives or
...
- **Agent teams section → Import**: Accepts `.team.json` or `.team.png` (a `kura-team-snapshot
...
> `kura pack validate` / `kura pack inspect` (Section 11). A snapshot is captured *from* an
...
> `kura pack inspect`'s resolved config as reference.
...
|-----------|----------------|-----------|-------------------|
| Skills | Copy at deploy time (planned) | kura-acp will copy `skills/` → `$AGENT_CWD/.agents/skills/` | ✅ Yes (only one) |
| MCP servers | ACP protocol | `NewSessionRequest.mcp_servers` | ❌ No |
| Persona prompt | User message prefix | `[System]` block prepended to user message text by kura-acp | ❌ No |
| Pack instructions | User message prefix | Appended to `[System]` block in user message text | ❌ No |
| Lifecycle hooks | Harness internal | kura-acp fires shell commands directly | ❌ No |
| Model/provider | Child process env vars | Agent-runtime-specific env vars (e.g. `GOOSE_PROVIDER`, `GOOSE_MODEL`) | ❌ No |
| Behavioral config | Harness internal | kura-acp subscription + dispatch logic | ❌ No |
| Pack defaults (`defaults`) | Harness internal | Resolved at deploy time by kura-acp into per-persona effective config; never passed to the agent runtime directly | ❌ No |

> **Pack defaults are resolved at deploy time**, not at runtime. When kura-acp loads a pack and
...
kura-acp's `format_prompt()` in `queue.rs` prepends a `[System]` block to the **user message
text** before sending it to the agent runtime. This is a **kura-acp feature, not an agent runtime
...
harness-side interpolation is implemented, kura-acp will resolve them from the process
...
- **Phase 1 (zip)**: Packs **must** ship with `<pack-name>-<version>.kurapack.sha256` containing
  `sha256sum` output (`<hex-digest>  <filename>`). kura-acp **must** verify before installation
  and refuse on mismatch.
- **Phase 2 (git)**: `pack.lock` pins the resolved commit SHA; kura-acp verifies on install.
- **Phase 3 (registry)**: Registry signatures TBD.

### Hook Execution

Hooks run with kura-acp's privileges — significant attack surface. Only install packs from
trusted sources. Review all hook commands before installing. Consider sandboxing kura-acp
(container, restricted user) for untrusted packs. kura-acp should display hook commands before
...
### From V6 (kura-namespaced) Format

Field mapping from V6 `.persona.md` to current `.persona.md`:

| V6 location | Current location |
|---|---|
| `kura.model` | `model` (top-level frontmatter) |
| `kura.temperature` | `temperature` (top-level frontmatter) |
| `kura.max_context_tokens` | `max_context_tokens` (top-level frontmatter) |
| `kura.subscribe` | `subscribe` (top-level frontmatter) |
| `kura.respond_to` | `triggers` (top-level frontmatter) |
| `kura.thread_replies` | `thread_replies` (top-level frontmatter) |
| `kura.broadcast_replies` | `broadcast_replies` (top-level frontmatter) |
| `plugin.json` → `kura.defaults` | `plugin.json` → `defaults` (top-level) |
| `plugin.json` → `kura.personas` | `plugin.json` → `personas` (top-level) |
| `plugin.json` → `kura.pack_instructions` | `plugin.json` → `pack_instructions` (top-level) |
| `plugin.json` → `kura.mcp_config` | `plugin.json` → `mcp_config` (top-level) |
| `plugin.json` → `kura.hooks_config` | `plugin.json` → `hooks_config` (top-level) |
...
5. Run `kura pack validate ./my-pack`

### Backward Compatibility

The V6 namespaced `kura:` block format is not supported. Only the current flat top-level fields format is accepted. The `respond_to` key is accepted as a legacy alias for `triggers` in both persona frontmatter and `plugin.json` defaults.
...
1. **`session/set_model` as env var alternative**: The ACP runtime implements `on_set_model()` (ACP
   unstable feature). kura-acp could call `session/set_model` after `session/new` to set the
...
2. **`CONTEXT_FILE_NAMES` env var**: The goose agent runtime supports this env var to control which filenames are
   scanned for hints. Should kura-acp set this to include pack-specific filenames? Deferred
   pending use case.
...
`kura pack init` scaffolding; hot reload of skills/instructions; skill marketplace; pack dependencies; agent-to-agent handoff within a pack.
...
| PF-2 | `kura pack validate` CLI: **Implemented.** Schema-validates `plugin.json`; checks `.persona.md` required identity fields; validates behavioral config fields; warns on unknown keys and skill name mismatches. Remaining: verify `skills:` and `hooks:` paths exist; error on `SKILL.md` missing `name:` or `description:`. | `kura-cli` / `kura-admin` |
| PF-3 | Skill collision warning: emit `WARN` when a pack skill is skipped because a skill with the same load key already exists in `.agents/skills/`. | kura-acp skill copy logic |
| PF-4 | `$AGENT_CWD` resolution: determine `NewSessionRequest.cwd` from (1) `AGENT_CWD` env var, (2) `std::env::current_dir()`, (3) error and refuse to start. | kura-acp startup / session init |
| PF-5 | Skill parse failure warning: emit `WARN` when `parse_skill_content` returns `None` (missing `name:`, missing `description:`, or malformed frontmatter). Currently the agent runtime silently skips. kura-acp should pre-validate during skill copy. | kura-acp skill copy logic |
| PF-6 | Per-subprocess env var injection: **Implemented.** `AcpClient::spawn` accepts `extra_env: &[(String, String)]` injected via `Command::env()`. kura-acp checks `std::env::var(key)` before injecting — operator env vars take precedence (level 1). | `kura-acp/src/acp.rs` `AcpClient::spawn()` |
