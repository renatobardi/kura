...
//! (OPS metadata) and tells Kura where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the Kura runtime must satisfy (e.g. `">=0.9.0"`).
    #[serde(skip_serializing_if = "Option::is_none", alias = "kura")]
    pub kura: Option<String>,
}
...
/// validator issues advisory warnings for Kura-unknown keys.
...
mod tests {
    ...
    fn parse_full_manifest() {
        ...
        assert_eq!(m.engines.unwrap().kura.as_deref(), Some(">=0.9.0"));
