...
//! - Kura-agent personas emit KURA_AGENT_MODEL, KURA_AGENT_PROVIDER
...
use kura_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_kura_agent_persona_emits_kura_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert_eq!(
        env.get("KURA_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit KURA_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("KURA_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit KURA_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "kura-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "kura-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // Kura-agent persona
    fs::write(
        root.join("agents/kura-bot.persona.md"),
        r#"---
name: "kura-bot"
display_name: "Kura Bot"
description: "A kura-agent runtime bot"
runtime: "kura-agent"
model: "openai:gpt-4o"
---
You are a kura bot.
"#,
    ...
    let kura = pack
        .personas
        .iter()
        .find(|p| p.name == "kura-bot")
        .expect("kura-bot should exist");
    ...
    assert!(
        !goose_env.contains_key("KURA_AGENT_MODEL"),
        "goose persona must not emit KURA_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("KURA_AGENT_PROVIDER"),
        "goose persona must not emit KURA_AGENT_PROVIDER"
    );

    // Kura-agent persona gets KURA_AGENT_* env vars
    let kura_env: std::collections::HashMap<_, _> = kura
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(kura_env.get("KURA_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(kura_env.get("KURA_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !kura_env.contains_key("GOOSE_MODEL"),
        "kura-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !kura_env.contains_key("GOOSE_PROVIDER"),
        "kura-agent persona must not emit GOOSE_PROVIDER"
    );
}
