...
pub enum Inbound {
    ...
    /// to a request kura-agent issued. Today the only such request is
...
/// one. `expected_run_id` must match the run id kura-agent advertised via
...
/// session use `model_id` instead of the configured `KURA_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) answer a request kura-agent
...
/// `session/request_permission`. kura-acp's answering side selects the option
...
/// is applied.
///
/// - **v2** (what kura-agent negotiates with current kura-acp): tool context
...
/// method goose uses for custom usage and status events.  Used by kura-agent
/// to emit the `usage_update` payload so kura-acp's `UsageTracker` can treat
/// kura-agent and goose symmetrically.
...
/// exists to prevent.
///
/// All counts are SESSION-cumulative, matching goose, so kura-acp's
...
/// | `contextLimit` | `u64` | `0` — kura-agent has no context limit tracking |
...
pub fn usage_update_payload(
    ...
    let mut update = json!({
        ...
        // contextLimit: 0 (kura-agent has no context limit tracking).
...
mod tests {
    ...
    /// v2 (what kura-agent negotiates with current kura-acp): tool context is
    ...
    /// `optionId == kind` so kura-acp's `kind`-based selector and this side's
    ...
    /// request kura-agent issued; it routes to the broker with its `result`.
    #[test]
    ...
    /// fails closed. kura-agent never leaves the waiter hanging on an error.
    #[test]
