...
mod tests {
    ...
    fn build_hints_section_combined() {
        ...
        let skill_dir = cwd.join(".agents/skills/kura-cli");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: kura-cli\ndescription: CLI reference for Kura managed agents\n---\nUse `kura` to manage agents.\n",
        )
        .unwrap();
        ...
        assert!(
            result.contains("kura-cli: CLI reference for Kura managed agents"),
            "missing skill bullet"
        );
        // Body must NOT be inlined — lazy loading only.
        assert!(
            !result.contains("Use `kura` to manage agents."),
