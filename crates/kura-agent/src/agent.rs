...
const REPLY_GUARD_SERVER: &str = "kura-agent";
...
const REPLY_GUARD_NAG: &str = "You are about to end this turn without calling `kura messages send`. \
...
/// Whether `call` is a recognized attempt to publish a reply to Kura.
///
...
fn is_kura_reply_call(call: &ToolCall, mcp: &McpRegistry) -> bool {
    mcp.has(&call.name) && !mcp.is_hook(&call.name) && is_reply_shaped(&call.name, &call.arguments)
}

/// Whether a tool name and arguments have the shape of a Kura publish command.
///
/// Split from [`is_kura_reply_call`] only so the matcher is testable without a
...
/// (`echo "kura messages send"`) matches. Missing a real post is the expensive
...
impl RunCtx<'_> {
    ...
    pub async fn run(&mut self, prompt: Vec<ContentBlock>) -> Result<StopReason, AgentError> {
        ...
        *self.turn_total_state = TurnTotalState::Unseen;
        ...
        // session) so `KURA_AGENT_MAX_HANDOFFS` bounds compactions per
        ...
        // successful publish. See `is_kura_reply_call`.
        ...
        let mut kura_reply_call_seen = false;
        ...
        loop {
                ...
                || response.output_tokens.is_some()
            {
                ...
                *self.turn_cache_write_tokens = self
                ...
                // the consumer: kura-acp's UsageTracker advances its committed
            ...
            if response.tool_calls.is_empty() {
                ...
                //   1. `!kura_reply_call_seen` — no publish attempt in any
            ...
            if !kura_reply_call_seen {
                kura_reply_call_seen = calls.iter().any(|c| is_kura_reply_call(c, self.mcp));
            }
            self.history.push(HistoryItem::Assistant {
                text: response.text,
...
fn synthetic_hook_id(hook: &str, server: &str, ordinal: u64) -> String {
    format!("kura_hook_{hook}_{server}_{ordinal}")
}
...
///
/// `kura_reply_call_seen` is the publish-aware gate (from
/// `is_kura_reply_call`), updated unconditionally regardless of
...
fn warn_if_silent_turn(
    kura_reply_call_seen: bool,
    text_is_empty: bool,
    output_tokens: Option<u64>,
    stop: ProviderStop,
) {
    if kura_reply_call_seen || !text_is_empty {
        return;
    }
    match output_tokens {
...
mod tests {
    ...
    fn reply_shape_matches_documented_send_forms() {
        for cmd in [
            "kura messages send --channel X --content Y",
            "kura --relay wss://r messages send --channel X --content Y",
            "/abs/path/kura messages send",
            "printf 'hi' | kura messages send --content -",
            "kura messages send-diff --diff -",
            "kura reactions add --event E --emoji +",
            // Assembled through another shell: rev 3's tokenizer missed this.
            r#"sh -c "kura messages send --channel X""#,
        ] {
            assert!(
    ...
    fn reply_shape_rejects_non_reply_commands() {
        for cmd in [
            "kura messages get --channel X",
            "kura channels list",
            "kura reactions remove --event E",
            "kura pr open --title T",
            "kura social publish --content hi",
            "kura notes set --name n",
    ...
    fn reply_shape_requires_the_qname_separator() {
        let args = json!({ "command": "kura messages send --channel X" });
        for name in [
            "dev__powershell",
            "dev__noshell",
            "shell",
        ...
        assert!(is_reply_shaped("kura-dev-mcp__shell", &args));
    }
    ...
    fn reply_shape_reads_only_the_command_field() {
        assert!(!is_reply_shaped(
            "dev__shell",
            &json!({ "description": "kura messages send --channel X" })
        ));
        assert!(!is_reply_shaped(
            "dev__shell",
            &json!({ "workdir": "kura messages send" })
        ));
    ...
    ///
    /// Identifies silent-turn WARNs by target (`kura_agent::agent`) + WARN
    ...
    fn count_silent_turn_warnings(f: impl FnOnce()) -> usize {
        ...
        impl<S: tracing::Subscriber> tracing_subscriber::Layer<S> for Capture {
            fn on_event(
                ...
                if event.metadata().target() != "kura_agent::agent" {
                    return;
                }
    ...
    /// token check, or hardcoding `kura_reply_call_seen = true` are all caught.
    #[test]
    ...
    /// A turn that published (kura_reply_call_seen = true) then ended with a
    /// short final completion must not trigger the WARN.  This is the normal
    /// publish-then-wrap pattern.  Deleting the `kura_reply_call_seen` gate
