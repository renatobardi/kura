...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "SSL_CERT_FILE",
    "SSL_CERT_DIR",
    // Kura identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). KURA_PRIVATE_KEY
    // and KURA_RELAY_URL are kept for the kura CLI. KURA_AUTH_TAG is a
    ...
    // spawn kura-agent without declaring it.
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_kura_owner_attestation() {
        assert!(PASSTHROUGH_ENV.contains(&"KURA_AUTH_TAG"));
    }
