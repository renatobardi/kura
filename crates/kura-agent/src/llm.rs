...
/// consent for `databricks-cli` covers kura-agent too.
...
impl Llm {
    ...
    /// route through a mesh (Kura shared compute) resolve their own model name
        ...
        match phase {
            TimeoutPhase::Transport => format!(
                "read timeout: no response bytes received within {per_request_timeout:?} \
                 (consider raising KURA_AGENT_LLM_TIMEOUT_SECS)"
            ),
...
async fn openrouter_post(
    ...
    'attempt: for attempt in 0..MAX_RETRIES {
        ...
        let resp = match http
            .post(url)
            .header("content-type", "application/json")
            .header("HTTP-Referer", "https://github.com/renatobardi/kura")
            .header("X-OpenRouter-Title", "Kura")
            .bearer_auth(bearer)
            .body(body_bytes.clone())
            .timeout(per_request_timeout)
            .send()
...
fn apply_openrouter_mutations(
    ...
    if let Some(obj) = body.as_object_mut() {
        ...
        // `prompt_caching` (`KURA_AGENT_PROMPT_CACHING`) for the same reason as
...
mod tests {
    ...
    fn timeout_message_connect_true_shows_connect_timeout() {
        ...
        for phase in [TimeoutPhase::Transport, TimeoutPhase::BodyRead] {
            ...
            assert!(
                !msg.contains("KURA_AGENT_LLM_TIMEOUT_SECS"),
    ...
    /// Kura agents stall (server connected but emitting no bytes).
    #[tokio::test]
    ...
    /// `KURA_AGENT_PROMPT_CACHING=0` must reach the OpenRouter `anthropic/*`
    ...
    fn openrouter_body_caching_disabled_emits_no_cache_control() {
        ...
        assert!(
            ...
            "KURA_AGENT_PROMPT_CACHING=0 must suppress every breakpoint: {body}"
        );
    ...
    async fn openrouter_post_sends_attribution_headers() {
        ...
        let header_str = headers
            ...
            .to_lowercase();
        assert!(
            header_str.contains("http-referer: https://github.com/renatobardi/kura"),
            "got: {header_str}"
        );
        assert!(
            header_str.contains("x-openrouter-title: kura"),
            "got: {header_str}"
        );
    }
