...
async fn handshake(h: &mut Harness) -> String {
    ...
    assert_eq!(init["result"]["agentInfo"]["name"], "kura-agent");
    assert_eq!(
...
/// kura-agent must emit a `usage_update` notification with `accumulatedInputTokens`
...
/// When the Anthropic parser produces an input-sum overflow, kura-agent must
