#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `kura-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
...
//!         └── dynamic SUBSCRIBE kura:{community}:channel:{id} / kura:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: kura_core::CommunityId,
...
/// Central pub/sub manager for a Kura relay instance.
...
impl PubSubManager {
    ...
    /// `kura:{community}:channel:{id}` / `kura:{community}:global`, and
