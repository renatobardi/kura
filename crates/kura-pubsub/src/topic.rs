...
/// Redis key prefix for Kura-scoped pub/sub topics and keys.
pub const KURA_PREFIX: &str = "kura";
    ...
    pub fn redis_channel(&self) -> String {
        match self.topic {
            EventTopic::Channel(channel_id) => {
                format!("{KURA_PREFIX}:{}:channel:{channel_id}", self.community_id)
            }
            EventTopic::Global => format!("{KURA_PREFIX}:{}:global", self.community_id),
        }
    }
...
mod tests {
    ...
    fn channel_key_includes_community_and_channel() {
        ...
        assert_eq!(
            ...
            format!("kura:{}:channel:{channel_id}", ctx.community())
        );
    }
    ...
    fn global_key_includes_community() {
        ...
        assert_eq!(global_key(&ctx), format!("kura:{}:global", ctx.community()));
    }
    ...
    fn parses_channel_topic() {
        ...
        let channel_id = Uuid::from_u128(0xbbbb);
        let raw = format!("kura:{community_id}:channel:{channel_id}");
    ...
    fn parses_global_topic() {
        let community_id = CommunityId::from_uuid(Uuid::from_u128(0xaaaa));
        let raw = format!("kura:{community_id}:global");
    ...
    fn rejects_malformed_or_wrong_prefix_topics() {
        for raw in [
            "",
            "not-kura:00000000-0000-0000-0000-00000000aaaa:global",
            "kura:not-a-uuid:global",
            "kura:00000000-0000-0000-0000-00000000aaaa",
            "kura:00000000-0000-0000-0000-00000000aaaa:global:extra",
            "kura:00000000-0000-0000-0000-00000000aaaa:channel",
            "kura:00000000-0000-0000-0000-00000000aaaa:channel:not-a-uuid",
            "kura:00000000-0000-0000-0000-00000000aaaa:presence:abc",
        ] {
            assert!(
