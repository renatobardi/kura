//! `kura-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! kura-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! kura-pair target [--relay wss://relay.example.com]
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(kura_core::pairing::types::AbortReason::SasMismatch)?
        {
