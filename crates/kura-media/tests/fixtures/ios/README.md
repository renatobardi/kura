...
2. Copy the encoded files into both fixture directories:
   ...
   cp uikit-encoded.png crates/kura-media/tests/fixtures/ios/
   cp uikit-encoded.jpg crates/kura-media/tests/fixtures/ios/
   ```
...
5. Run `cargo test -p kura-media test_ios_uikit` to verify that UIKit's encoded output is rejected and the matching sanitizer output is accepted by the relay contract.
