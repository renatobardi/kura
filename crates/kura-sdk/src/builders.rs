...
///
/// Shortcodes are case-insensitive in Kura's relay-global set; lowercase
...
/// Build a Kura-native delete event (kind 9005).
...
/// Build a Kura-native delete event (kind 9005) with optional moderation metadata.
...
pub const CUSTOM_EMOJI_SET_D_TAG: &str = "kura:custom-emoji";
...
/// "kura:custom-emoji")`. Replaces the caller's prior set. The workspace
...
pub fn build_update_channel(
    ...
    if name
        .map(kura_core::channel::canonical_channel_name)
...
/// Maximum byte length of a project `kura-channel` tag value.
pub const PROJECT_CHANNEL_MAX: usize = 256;
/// Maximum byte length of a project `kura-visibility` tag value.
...
///
/// Rules enforced (matches relay `kura-db` ingest logic):
...
/// 7. Singleton metadata: each of `name`, `description`, `kura-channel`,
///    `kura-visibility` appears at most once.
/// 8. Metadata byte lengths: `name` ≤256, `description` ≤2048,
///    `kura-channel` ≤256, `kura-visibility` ≤256.
...
pub fn build_project(
    ...
    if let Some(ch) = channel {
        uuid::Uuid::parse_str(ch).map_err(|_| {
            SdkError::InvalidInput(format!("kura-channel must be a valid UUID (got {ch:?})"))
        })?;
    }
    ...
    if let Some(vis) = visibility {
        if vis != "listed" && vis != "unlisted" {
            return Err(SdkError::InvalidInput(format!(
                "kura-visibility must be 'listed' or 'unlisted' (got {vis:?})"
            )));
        }
    }
    ...
    if let Some(ch) = channel {
        tags.push(tag(&["kura-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(tag(&["kura-visibility", vis])?);
    }
...
mod tests {
    ...
    fn agent_observer_frame_happy_path() {
        ...
        let recipient = keys();
        let agent = keys();
        let encrypted = kura_core::observer::encrypt_observer_payload(
            &sender,
    ...
    fn repo_announcement_with_tags_preserves_metadata_and_canonicalizes_d() {
        let tags = vec![
            ...
            Tag::parse(["kura-channel", "channel-id"]).unwrap(),
        ...
        assert!(has_tag(&ev, "kura-channel", "channel-id"));
    ...
    fn git_patch_rejects_whitespace_only_content() {
        // Regression: a failed `git format-patch | kura patches send
    ...
    fn build_project_emitted_envelope_has_correct_shape() {
        ...
        // name, description, kura-channel, kura-visibility present.
        ...
        let ch_tags: Vec<_> = all_tags.iter().filter(|t| t[0] == "kura-channel").collect();
        assert_eq!(ch_tags.len(), 1);
        assert_eq!(ch_tags[0][1], VALID_UUID);

        let vis_tags: Vec<_> = all_tags
            .iter()
            .filter(|t| t[0] == "kura-visibility")
            .collect();
        ...
        assert_eq!(a_tags[0][1], format!("30617:{OWNER64}:kura"));
    }
