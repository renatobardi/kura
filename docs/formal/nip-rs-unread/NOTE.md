...
(below): every context ID Kura actually generates (channel UUID,
...
**Cost:** zero bytes for every context ID Kura generates today (channel
...
">256-byte key drop hazard" section: Kura's own key shapes cannot
