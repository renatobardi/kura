...
`Kura Desktop → kura-acp → kura-agent → MeshLLM SDK → local/remote compute`
...
resolves to the bundled `kura-agent`. No API key is required.
...
**kura-agent** as the runtime, **Kura shared compute** as the LLM provider,
...
`KURA_MESH_IROH_RELAYS` defaults to Iroh's production relay set. Set it to `0`
