...
- Slice 2 — OS-level deep links: the `repo`/`project`/`pr`/`issue` hosts in
  ...
  clicks.
- Slice 3 (create-command part) — `crates/kura-cli/src/links.rs`, `link`
...
`kura-pull-request`, `kura-issue`, `kura-repository`, `kura-project`), or —
...
One addition to `crates/kura-acp/src/base_prompt.md`, next to the existing
