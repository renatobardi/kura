...
2. Enter a `target_ref` from this repo (`my-branch`, `refs/heads/my-branch`, `my-tag`, or `refs/tags/my-tag`).
...
This path is intentionally separate from the production/main relay image path (`ghcr.io/renatobardi/kura`) so a staging-only branch deployment is obvious in BPCI.
...
While bb-block remains a shared staging environment, do not deploy a target ref through this runtime-only path if it changes `migrations/` or requires changes to `deploy/charts/kura/`. Backwards-compatible migrations can still leave the shared database ahead of the restored `main` image. Use an isolated environment or deployment-time enforcement for those changes.
