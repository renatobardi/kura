...
- **`at+jwt` access token**: a Kura-resource access token whose protected
  `typ` is exactly `at+jwt` and whose `aud` contains the configured Kura
...
- **dedicated Kura assertion**: a separately minted assertion whose protected
