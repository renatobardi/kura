...
credential to desktop clients. An operator configures `KURA_KLIPY_API_KEY` in
...
or paths later. Clients must require `kura-gif`, recognize the provider, and use
...
minute and can be tuned with `KURA_RATE_LIMIT_GIF_SEARCHES_PER_MIN`. The relay
...
`kura-gif`. Only the picker is capability-gated.
