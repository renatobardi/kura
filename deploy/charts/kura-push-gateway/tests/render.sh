...
helm lint deploy/charts/kura-push-gateway >/dev/null
helm template push deploy/charts/kura-push-gateway >"$out"
# Production values must attach push.kura.xyz to an explicit Gateway.
production_args=(
...
helm lint deploy/charts/kura-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/kura-push-gateway "${production_args[@]}" >"$production_out"
...
assert!(jenv.dig("KURA_PUSH_RUNTIME_DATABASE_ROLE", "value") == "kura_push_gateway_runtime")
...
required = Set.new(%w[
  DATABASE_URL KURA_PUSH_DOGFOOD_APNS_CERT_PATH
  KURA_PUSH_DOGFOOD_APNS_TOPIC KURA_PUSH_DOGFOOD_APP_ATTEST_APP_ID
  KURA_PUSH_GRANT_KEYS KURA_PUSH_TOKEN_KEYS KURA_PUSH_MAX_GRANT_LIFETIME_SECONDS
])
...
np = nps.find { |x| x.dig("metadata", "name") == "push-kura-push-gateway" }
migration_np = nps.find { |x| x.dig("metadata", "name") == "push-kura-push-gateway-migration" }
...
assert!(route.dig("spec", "hostnames").include?("push.kura.xyz"))
RUBY

# Legacy token-auth values must fail rather than silently selecting the default
# certificate Secret.
if helm template push deploy/charts/kura-push-gateway \
...
if helm template push deploy/charts/kura-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/kura-push-gateway -f deploy/charts/kura-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/kura-push-gateway \
...
np = xs.find do |x|
  x["kind"] == "NetworkPolicy" && x.dig("metadata", "name") == "push-kura-push-gateway"
end
