...
A tiny non-AI Kura bot example.

The bot is deliberately boring and algorithmic: it listens to one Kura channel
...
It demonstrates that Kura participants do not have to be LLM agents. Any
...
members list and in Kura's mention autocomplete.
...
```bash
KURA_RELAY_URL=ws://localhost:3000 \
KURA_CHANNEL_ID=<channel-uuid> \
KURA_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
KURA_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that Kura agents
...
KURA_CHANNEL_ID=<channel-uuid> \
KURA_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
KURA_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
KURA_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
KURA_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
KURA_BOT_AUTH_MODE=owner-attested \
# plus KURA_RELAY_URL, KURA_CHANNEL_ID, KURA_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `KURA_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `KURA_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Kura UI adds that tag when the bot is selected from
