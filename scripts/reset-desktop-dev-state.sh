...
case "$(uname -s)" in
  Darwin)
    ...
    if command -v security >/dev/null 2>&1; then
      while security delete-generic-password -s kura-desktop-dev >/dev/null 2>&1; do :; done
...
remove_path "$HOME/.kura-dev"
remove_path "$HOME/.sprout-dev"

# A fresh dev nest must not re-import the installed app's ~/.kura contents on
# its next boot. The sentinel is the same one used by migrate_dev_nest().
mkdir -p "$HOME/.kura-dev"
: > "$HOME/.kura-dev/.dev-nest-migrated"

log "Development desktop state removed; production Kura state was not touched"
