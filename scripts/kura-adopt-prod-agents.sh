#!/usr/bin/env bash
#
# kura-adopt-prod-agents.sh — copy your installed (production) Kura agent
...
#   every kura-desktop-dev keychain entry, and the `_dev_migration_v1` marker.
...
#   reset wipes that Kura cannot re-derive on its own: the agent RECORDS and
...
#
# WHAT THIS DOES NOT DO (verified against block/kura origin/main)
#   - It does NOT touch the kura-desktop-dev keychain. Agent private keys are
...
#   KURA_KEYCHAIN_SVC   keychain service    (default kura-desktop)
#   KURA_KEYCHAIN_ACCT  keychain account    (default secrets)
#
set -euo pipefail
...
#     `kura-desktop` for both the installed DMG
#     (/Applications/Kura.app/Contents/MacOS/kura-desktop) and dev builds
#     (target/<profile>/kura-desktop via `tauri dev`). Match that path component
#     exactly so sidecars/helpers (kura, kura-dev-mcp, kura-agent) don't
#     false-positive.
#
...
#     state) blocks.
ALLOWED_PROD_EXE="/Applications/Kura.app/Contents/MacOS/kura-desktop"
...
running_pids="$(pgrep -f '/kura-desktop( |$)' 2>/dev/null || true)"
dmg_running=0
...
if [[ ${#dev_blocking[@]} -gt 0 ]]; then
  warn "A non-installed kura-desktop process is running (dev build or unresolvable):"
...
if [[ $dmg_running -eq 1 ]]; then
  ...
  info "no running Kura process detected"
fi
...
cat <<EOF
  1. Start your dev build (\`just production\` or \`just dev\`).
     - First boot re-runs the agent-key migration → ONE prod-keychain prompt
       (copies agent:<pubkey> keys kura-desktop → kura-desktop-dev).
  ...
  2. Worktree launches: worktree-suffixed dev dirs are symlinked to the
     canonical dev dir by sync_shared_agent_data ONLY when KURA_SHARE_IDENTITY=1.
     If you launch from a worktree, export KURA_SHARE_IDENTITY=1 (and
     KURA_PRIVATE_KEY) or the worktree will mint its own duplicate agents.
EOF
if [[ "${KURA_SHARE_IDENTITY:-}" != "1" ]]; then
  say "  NOTE: KURA_SHARE_IDENTITY is not set in this shell — set it for worktree launches."
fi
exit 0
