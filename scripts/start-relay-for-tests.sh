#!/usr/bin/env bash
# =============================================================================
# start-relay-for-tests.sh — Start the Kura relay and its backing services
# =============================================================================
...
wait_healthy "Postgres" "kura-postgres"
wait_healthy "Redis" "kura-redis"
wait_healthy "MinIO" "kura-minio"
...
export PGSCHEMA_PLAN_DB=kura
export PGSCHEMA_PLAN_USER=kura
export PGSCHEMA_PLAN_PASSWORD=kura_dev

./bin/pgschema apply --file schema/schema.sql --auto-approve
docker exec -i -e PGPASSWORD="${PGPASSWORD}" kura-postgres \
...
# psql is not on PATH in the hermit env; postgres runs as the kura-postgres
...
if command -v psql >/dev/null 2>&1; then
  ...
  seed_psql() { docker exec -e PGPASSWORD="${PGPASSWORD}" kura-postgres psql -U "${PGUSER}" -d "${PGDATABASE}" -qtA "$@"; }
fi
seed_psql -c "
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  for bin in kura-relay git-credential-nostr; do
  ...
  cargo build --profile "${CARGO_PROFILE}" -p kura-relay -p git-credential-nostr
  ok "Relay built"
fi
...
TEST_RELAY_PRIVATE_KEY="${KURA_RELAY_PRIVATE_KEY:-$(openssl rand -hex 32)}"
...
MEMBERSHIP_ENV=()
if [[ "${KURA_REQUIRE_RELAY_MEMBERSHIP:-}" == "true" ]]; then
  MEMBERSHIP_ENV+=(
    KURA_REQUIRE_RELAY_MEMBERSHIP=true
    RELAY_OWNER_PUBKEY="${RELAY_OWNER_PUBKEY:?RELAY_OWNER_PUBKEY required with KURA_REQUIRE_RELAY_MEMBERSHIP=true}"
  )
  : "${KURA_RELAY_PRIVATE_KEY:?KURA_RELAY_PRIVATE_KEY required with KURA_REQUIRE_RELAY_MEMBERSHIP=true}"
...
nohup env \
  ...
  KURA_RELAY_PRIVATE_KEY="${TEST_RELAY_PRIVATE_KEY}" \
  KURA_REQUIRE_AUTH_TOKEN=false \
  KURA_RECONCILE_CHANNELS=true \
  KURA_GIT_PROBE_WRITERS=8 \
  ${MEMBERSHIP_ENV[@]+"${MEMBERSHIP_ENV[@]}"} \
  "./target/${CARGO_PROFILE}/kura-relay" > /tmp/kura-relay.log 2>&1 &
echo $! > /tmp/kura-relay.pid

# ── Poll readiness ───────────────────────────────────────────────────────────

log "Waiting for relay readiness..."
for attempt in $(seq 1 60); do
  if ! kill -0 "$(cat /tmp/kura-relay.pid)" 2>/dev/null; then
    err "Relay process died"
    cat /tmp/kura-relay.log
    exit 1
  fi
