#!/usr/bin/env bash
# Build Sprig — one deploy-anywhere multicall binary for the Kura ACP
# harness, agent, and developer MCP. The archive exposes these command names:
#
#   sprig            implementation binary
#   kura-acp       link to sprig (ACP harness)
#   kura-agent     link to sprig (ACP-compliant agent)
#   kura-dev-mcp   link to sprig (developer MCP server; also dispatches
#                    rg/tree/kura/git-credential-nostr/git-sign-nostr)
#
# Usage:
...
BUNDLE_BIN="sprig"
COMMANDS=(kura-acp kura-agent kura-dev-mcp)
...
Sprig is the all-in-one Kura agent binary for deploy-anywhere environments.
...
- `kura-acp` — ACP harness that bridges Kura channel events to an
  ACP-compliant agent over stdio.
- `kura-agent` — ACP-compliant agent (spawns MCP servers, calls LLMs).
- `kura-dev-mcp` — Developer MCP server (shell, str_replace, todo) and
  multicall entrypoint for `rg`, `tree`, `kura`, `git-credential-nostr`,
  `git-sign-nostr`.
...
```bash
# Agent provider
export KURA_AGENT_PROVIDER=anthropic            # or openai
export ANTHROPIC_API_KEY=sk-...
export ANTHROPIC_MODEL=claude-sonnet-4-20250514

# Nostr identity (shared by kura-acp, git auth, signing, and kura CLI)
export NOSTR_PRIVATE_KEY=nsec1...
export KURA_PRIVATE_KEY="$NOSTR_PRIVATE_KEY"
export KURA_RELAY_URL=https://your-relay.example.com
```
README
