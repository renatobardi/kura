...
//    Both KURA_UPDATER_PUBLIC_KEY and KURA_UPDATER_ENDPOINT are required -
...
const updaterPubkey = process.env.KURA_UPDATER_PUBLIC_KEY;
const updaterEndpoint = process.env.KURA_UPDATER_ENDPOINT;

const missing = [];
if (!updaterPubkey) missing.push("KURA_UPDATER_PUBLIC_KEY");
if (!updaterEndpoint) missing.push("KURA_UPDATER_ENDPOINT");
if (missing.length > 0) {
  console.error(
