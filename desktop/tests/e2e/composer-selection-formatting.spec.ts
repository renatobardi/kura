    ...
    .poll(() =>
      page.evaluate(
        () =>
          (
            window as Window & {
              __KURA_E2E_SIGNED_EVENTS__?: Array<{ content: string }>;
            }
          ).__KURA_E2E_SIGNED_EVENTS__?.at(-1)?.content,
      ),
    )
    .toBe("```\none\ntwo\nthree\n```");
});
...
test("right-clicking selected composer text hides the selection formatter", async ({
  ...
  await input.evaluate((element) => {
    (
      window as Window & {
        __KURA_E2E_CONTEXTMENU_DEFAULT_PREVENTED__?: boolean;
      }
    ).__KURA_E2E_CONTEXTMENU_DEFAULT_PREVENTED__ = false;
    element.addEventListener(
      "contextmenu",
      (event) => {
        (
        ...
        ).__KURA_E2E_CONTEXTMENU_DEFAULT_PREVENTED__ = event.defaultPrevented;
      },
      { once: true },
    );
  });
    ...
    .poll(() =>
      page.evaluate(
        () =>
          ...
          ).__KURA_E2E_CONTEXTMENU_DEFAULT_PREVENTED__,
      ),
    )
    .toBe(false);
