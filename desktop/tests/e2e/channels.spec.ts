...
async function hasOutgoingEventWithContent(
  ...
  return page.evaluate((expectedContent) => {
    return (
      ...
      ).__KURA_E2E_COMMAND_LOG__?.some((entry) => {
...
test("creates the DM before preparing a persona mention", async ({ page }) => {
  ...
  await expect(page.getByTestId("chat-title")).toContainText("Hayate");
  ...
  await expect(
    page
      .getByTestId("message-row")
      .last()
      .locator("[data-mention].agent-mention-highlight", { hasText: "Hayate" }),
  ).toBeVisible();
});
...
test("routes an agent mention from an existing DM to the expanded conversation", async ({
  ...
  await expect(page.getByTestId("chat-title")).toContainText("Hayate");
  await expect(sourceDm).not.toContainText("Hayate");
...
test("drops an expanded DM after the first message fails", async ({ page }) => {
  ...
  await expect(input).toContainText("Hayate");
...
test("drops an expanded DM after agent startup fails", async ({ page }) => {
  ...
  await expect(page.getByTestId("chat-title")).not.toContainText("Hayate");
});
...
test("closes direct message results while opening", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __KURA_E2E__?: { mock?: { openDmDelayMs?: number } };
    };
    testWindow.__KURA_E2E__ ??= {};
    testWindow.__KURA_E2E__.mock ??= {};
    testWindow.__KURA_E2E__.mock.openDmDelayMs = 1_000;
  });
...
test("opens a sent direct message without waiting for a channel-list refresh", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __KURA_E2E__?: { mock?: { channelsReadDelayMs?: number } };
    };
    testWindow.__KURA_E2E__ ??= {};
    testWindow.__KURA_E2E__.mock ??= {};
    testWindow.__KURA_E2E__.mock.channelsReadDelayMs = 3_000;
  });
...
test("create ephemeral stream shows sidebar and header affordances", async ({
  ...
  await page.evaluate(
    ({ agentPubkey, channelId: activeChannelId }) => {
      (window as MockFeedWindow).__KURA_E2E_SEED_ACTIVE_TURNS__?.({
        agentPubkey,
  ...
  await page.evaluate(
    ({ channelName: targetChannelName, mentionPubkey, senderPubkey }) => {
      (window as MockFeedWindow).__KURA_E2E_EMIT_MOCK_MESSAGE__?.({
...
test("profile renders live activity for a viewer-owned relay agent", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__KURA_E2E_SEED_ACTIVE_TURNS__ ===
      "function",
  );
  await page.evaluate(
...
test("channel management keeps both description paragraphs visible for editors and readers", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof window.__KURA_E2E_MUTATE_CHANNEL__ === "function" &&
      typeof window.__KURA_E2E_INVALIDATE_CHANNELS__ === "function",
  );
  await page.evaluate(
    async ({ generalChannelId, randomChannelId, description }) => {
      const bridge = window as Window & {
        __KURA_E2E_INVALIDATE_CHANNELS__: () => Promise<void>;
        __KURA_E2E_MUTATE_CHANNEL__: (options: {
          channelId: string;
          description?: string;
        }) => void;
      };
      for (const channelId of [generalChannelId, randomChannelId]) {
        bridge.__KURA_E2E_MUTATE_CHANNEL__({ channelId, description });
      }
...
test("channel settings only prompt editors to add an empty description", async ({
  ...
  await page.evaluate(
    async ({ generalChannelId, randomChannelId }) => {
      ...
      bridge.__KURA_E2E_MUTATE_CHANNEL__({
...
test("stopping a managed bot in one community leaves its other communities running", async ({
  ...
  // (`seedDefaultCommunity` follows KURA_E2E_RELAY_URL); the agent also has a
