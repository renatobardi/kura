...
async function countCommandInvocations(
  ...
  return page.evaluate(
    (targetCommand) =>
      ...
      ).__KURA_E2E_COMMANDS__?.filter((invoked) => invoked === targetCommand)
        .length ?? 0,
    command,
  );
}
...
async function readAgentShareCommands(
  ...
  return page.evaluate(
    () =>
      (
        window as Window & {
          __KURA_E2E_COMMAND_LOG__?: AgentShareCommand[];
        }
...
test("custom personas share with people and keep export separate", async ({
  ...
  await page.evaluate(() => {
    ...
    (
      window as Window & {
        __KURA_RECIPIENT_POPOVER_OBSERVER__?: MutationObserver;
        __KURA_RECIPIENT_POPOVER_STATE_CHANGES__?: string[];
      }
    ).__KURA_RECIPIENT_POPOVER_OBSERVER__ = observer;
    (
      window as Window & {
        __KURA_RECIPIENT_POPOVER_STATE_CHANGES__?: string[];
      }
    ).__KURA_RECIPIENT_POPOVER_STATE_CHANGES__ = stateChanges;
  });
  await recipientSearch.click();
  await waitForAnimations(page);
  const recipientPopoverStateChanges = await page.evaluate(() => {
    ...
    trackedWindow.__KURA_RECIPIENT_POPOVER_OBSERVER__?.disconnect();
    return trackedWindow.__KURA_RECIPIENT_POPOVER_STATE_CHANGES__ ?? [];
  });
...
test("people sharing blocks a timeout before encoding or upload", async ({
  ...
  await page.evaluate(() => {
    ...
    ).__KURA_E2E_ACTIVATE_TIMEOUT__?.(Date.now() + 60_000);
  });
...
test("people sharing rechecks destination eligibility after encoding", async ({
  ...
  await page.evaluate(() => {
    ...
    testWindow.__KURA_E2E_MUTATE_CHANNEL__?.({
      channelId: "d1ec7000-d000-4000-8000-000000000001",
      channelType: "forum",
    });
    return testWindow.__KURA_E2E_INVALIDATE_CHANNELS__?.();
  });
