...
type E2eConfig = {
  ...
  mock?: {
     ...
     *  `__KURA_E2E_RELEASE_THREAD_REPLIES__()` is called. Unlike
...
declare global {
  interface Window {
    ...
    __KURA_E2E_RELEASE_MEDIA_PROXY__?: () => number;
    ...
    __KURA_E2E_EMIT_MEDIA_UPLOAD_PHASE__?: (input: {
      id: string;
      phase: string;
    }) => Promise<void>;
    __KURA_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__?: (input: {
      id: string;
      sent: number;
      total: number;
    }) => Promise<void>;
    __KURA_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__?: (payload: {
      pubkey: string | null;
      level: number;
    }) => Promise<void>;
    __KURA_E2E_WEBVIEW_ZOOM__?: number;
    __KURA_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?: (input: {
      channelName: string;
      kind?: number;
    }) => boolean;
    __KURA_E2E_HAS_MOCK_OWNER_KIND_SUBSCRIPTION__?: (input: {
      ownerPubkey: string;
      kind: number;
    }) => boolean;
    ...
    __KURA_E2E_PREPEND_MOCK_HISTORY__?: (input: {
      channelName: string;
      count: number;
      startIndex?: number;
      lineCount?: number;
     ...
     */
    __KURA_E2E_REPLACE_MOCK_TEAM_CATALOG_HEAD__?: (event: RelayEvent) => void;
    __KURA_E2E_PUSH_MOCK_FEED_ITEM__?: (item: RawFeedItem) => RawFeedItem;
     ...
     */
    __KURA_E2E_EXTRA_PROJECT_EVENTS__?: Array<{
      id: string;
      kind: number;
      pubkey: string;
      created_at: number;
    ...
    __KURA_E2E_PROJECT_MERGE_ERROR__?: {
      code: string;
      message: string;
      recovery: {
    ...
    __KURA_E2E_PROJECT_OWNER_OVERRIDE__?: string;
    /** Project history kinds rejected with CLOSED for aggregate-query tests. */
    __KURA_E2E_REJECT_PROJECT_QUERY_KINDS__?: number[];
    /** Captured aggregate project-history filters for request-count assertions. */
    __KURA_E2E_PROJECT_QUERY_FILTERS__?: MockFilter[];
    /** Optional local repository snapshot returned for project branch tests. */
    __KURA_E2E_PROJECT_LOCAL_REPO_SNAPSHOT__?: unknown;
    /** Optional bounded file contents returned by repository content reads. */
    __KURA_E2E_PROJECT_REPO_FILE_CONTENTS__?: Record<string, string | null>;
    __KURA_E2E_PROJECT_REPO_SYNC_STATUS__?: {
    ...
    __KURA_E2E_SET_RELAY_CONNECTION_STATE__?: (state: ConnectionState) => void;
    __KURA_E2E_GET_RELAY_CONNECTION_STATE__?: () => ConnectionState;
    /** Queue deterministic mock AUTH outcomes, consumed in order. */
    __KURA_E2E_QUEUE_AUTH_RESPONSES__?: (
      responses: Array<{ success: boolean; message: string }>,
    ) => void;
    /** Inject CLOSED into every active mock live subscription. */
    __KURA_E2E_CLOSE_LIVE_SUBSCRIPTIONS__?: (reason: string) => number;
    /** Queue CLOSED responses for channel history REQs. */
    __KURA_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__?: (reasons: string[]) => void;
    ...
    __KURA_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__?: (
      unavailable: boolean,
    ) => void;
    __KURA_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__?: () => number[];
    __KURA_E2E_ACTIVATE_RELAY_RATE_LIMIT__?: (seconds: number) => void;
    __KURA_E2E_RESET_WEBSOCKET_CONNECT_ATTEMPTS__?: () => void;
    ...
    __KURA_E2E_EMIT_MOCK_READ_STATE__?: (input: {
      clientId: string;
      contexts: Record<string, number>;
      createdAt: number;
      slotId: string;
    }) => unknown;
    __KURA_E2E_SEED_MOCK_REMINDERS__?: (reminders: RelayEvent[]) => void;
    ...
    __KURA_E2E_MD_PARSE_COUNT__?: () => number;
    /**
     ...
     */
    __KURA_E2E_ACTIVATE_TIMEOUT__?: (expiresAtMs: number) => void;
    /**
     ...
     * __KURA_E2E_INVOKE_MOCK_COMMAND__. Call after the mutation to make the
     ...
     * follow up with __KURA_E2E_INVALIDATE_CHANNELS__ to flush the cache.
     *
     ...
     * are held in a queue and not resolved until `__KURA_E2E_RELEASE_GET_EVENT__()`
    ...
    __KURA_E2E_LINK_PREVIEW_UPLOAD_STARTS__?: number;
  }
}
...
// When `window.__KURA_E2E_DEFER_GET_EVENT__` is set to a target event ID,
// `handleGetEvent` holds calls for that ID in this queue.  All other event IDs
// continue to resolve immediately.
// `window.__KURA_E2E_RELEASE_GET_EVENT__()` flushes the queue and returns the
...
function buildMockConfigSurface(pubkey: string): {
  ...
  switch (pubkey) {
    ...
    case PUBKEY_KURA_AGENT:
      return kuraAgentSurface;
    default:
      return gooseSurface;
  }
}
...
function buildSeededManagedAgent(seed: MockManagedAgentSeed): MockManagedAgent {
  ...
  // is incorrect: kura-agent ships with no default args.
  const DEFAULT_RUNTIME_COMMAND: Record<
    string,
    { command: string; args: string[] }
  > = {
    goose: { command: "goose", args: ["acp"] },
    "kura-agent": { command: "kura-agent", args: [] },
  ...
  const builtInPersonas = [
    {
      ...
      display_name: "Hayate",
      avatar_url: null,
      system_prompt: "You are Hayate.",
    },
    {
      id: "builtin:kaede",
      display_name: "Kaede",
      avatar_url: null,
      system_prompt: "You are Kaede.",
    },
    {
      id: "builtin:rin",
      display_name: "Rin",
...
//
// This entire module (e2eBridge.ts) is loaded only when `window.__KURA_E2E__`
...
function handleUpdateWorkflow(args: {
  ...
  const workflow = mockWorkflows.find((w) => w.id === args.workflowId);
  if (!workflow) throw new Error(`Workflow ${args.workflowId} not found`);
  const configuredError = window.__KURA_E2E__?.mock?.workflowUpdateError;
...
function handleDeleteWorkflow(args: { workflowId: string }) {
  const configuredError = window.__KURA_E2E__?.mock?.workflowDeleteError;
  if (configuredError) throw new Error(configuredError);
...
function handleTriggerWorkflow(args: { workflowId: string }) {
  const workflow = mockWorkflows.find((w) => w.id === args.workflowId);
  if (!workflow) throw new Error(`Workflow ${args.workflowId} not found`);
  const configuredError = window.__KURA_E2E__?.mock?.workflowTriggerError;
  if (configuredError) throw new Error(configuredError);
...
const MOCK_PROJECT_SEEDS = [
  ...
  },
  {
    ...
    cloneUrl: "https://github.com/renatobardi/relay-tools.git",
    webUrl: "https://github.com/renatobardi/relay-tools",
    owner: ALICE_PUBKEY,
...
function buildMockProjectEvents(): RelayEvent[] {
  if (window.__KURA_E2E_EMPTY_PROJECTS__) {
    return [];
  }
  ...
  for (const [projectIndex, seed] of MOCK_PROJECT_SEEDS.entries()) {
    const owner =
      projectIndex === 0
        ? (window.__KURA_E2E_PROJECT_OWNER_OVERRIDE__ ?? seed.owner)
        : seed.owner;
  ...
  if (!window.__KURA_E2E_REPOSITORY_ONLY_PROJECTS__) {
    const projectOwner =
      window.__KURA_E2E_PROJECT_OWNER_OVERRIDE__ ?? MOCK_PROJECT_SEEDS[0].owner;
    events.push(
      createMockEvent(
        KIND_PROJECT_ANNOUNCEMENT,
        "",
...
function getMockProjectEventStore(): RelayEvent[] {
  if (!mockProjectEventStore) {
    ...
    const extras = window.__KURA_E2E_EXTRA_PROJECT_EVENTS__;
...
async function handleDiscoverAcpRuntimes(
  ...
  const defaultCatalog: RawAcpRuntimeCatalogEntry[] = [
    ...
    },
    {
      ...
      install_instructions_url: "https://github.com/renatobardi/kura",
      can_auto_install: false,
...
async function handleCreateManagedAgent(
  ...
  const pubkey = crypto
    ...
    .padEnd(64, "0")
    .slice(0, 64);
  const agentCommand = args.input.agentCommand ?? "kura-agent";
  const agentArgs =
...
async function handleGetEvent(
  ...
  // Defer/release seam: when __KURA_E2E_DEFER_GET_EVENT__ is set to this
  // event's ID, hold this call in the queue until __KURA_E2E_RELEASE_GET_EVENT__()
  ...
  if (
    window.__KURA_E2E_DEFER_GET_EVENT__ &&
    window.__KURA_E2E_DEFER_GET_EVENT__ === args.eventId
  ) {
    // Increment the count only for calls that are actually deferred.
    window.__KURA_E2E_GET_EVENT_CALL_COUNT__ =
      (window.__KURA_E2E_GET_EVENT_CALL_COUNT__ ?? 0) + 1;
...
function sendToMockSocket(args: {
  ...
  if (type === "REQ") {
    ...
    if (
      ...
      window.__KURA_E2E_PROJECT_QUERY_FILTERS__ ??= [];
      window.__KURA_E2E_PROJECT_QUERY_FILTERS__.push(filter);
      const rejectedKinds =
  ...
  if (type === "EVENT") {
    ...
    if (isMockProjectScopedEvent(event)) {
      ...
      if (
        ...
        window.__KURA_E2E_UNSUPPORTED_PROJECT_ANNOUNCEMENTS__
      ) {
        sendWsText(socket.handler, [
          "OK",
          event.id,
      ...
      const rejectionIndex =
        window.__KURA_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(event.kind) ??
        -1;
      if (rejectionIndex >= 0) {
        window.__KURA_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
          rejectionIndex,
          1,
        );
        sendWsText(socket.handler, [
      ...
      window.__KURA_E2E_ACCEPTED_PROJECT_EVENTS__ = acceptedProjectEvents;
      const failedAckIndex =
        window.__KURA_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.indexOf(event.kind) ??
        -1;
      if (failedAckIndex >= 0) {
        window.__KURA_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.splice(
          failedAckIndex,
          1,
        );
        sendWsText(socket.handler, [
...
export function maybeInstallE2eTauriMocks() {
  ...
  window.__KURA_E2E_LINK_PREVIEW_UPLOAD_STARTS__ = 0;
  window.__KURA_E2E_RELEASE_LINK_PREVIEW_METADATA__ = () => {
    const queued = deferredLinkPreviewMetadataQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__KURA_E2E_RELEASE_LINK_PREVIEW_UPLOADS__ = () => {
    const queued = deferredLinkPreviewUploadQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__KURA_E2E_RELEASE_THREAD_REPLIES__ = () => {
    const queued = deferredThreadRepliesQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__KURA_E2E_THREAD_REPLIES_PENDING__ = () =>
  ...
  window.__KURA_E2E_RELEASE_MEDIA_PROXY__ = () => {
    mockMediaProxyPort = MOCK_MEDIA_PROXY_PORT;
    resetMediaCaches();
    ensureRelayOriginFetch();
    return mockMediaProxyPort;
  };
  window.__KURA_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__ = (payload) =>
    emit("huddle-tts-speaker-level", payload);
  window.__KURA_E2E_SIGNED_EVENTS__ = [];
  window.__KURA_E2E_WEBVIEW_ZOOM__ = 1;
  window.__KURA_E2E_EMIT_MEDIA_UPLOAD_PHASE__ = async (input) => {
    await emit("media-upload-phase", input);
  };
  window.__KURA_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__ = async (input) => {
    await emit("media-upload-progress", input);
  };
  window.__KURA_E2E_SET_MOCK_HUDDLE_SNAPSHOT__ = async ({
    members,
  ...
  window.__KURA_E2E_PREPEND_MOCK_HISTORY__ = prependMockHistory;
  ...
  window.__KURA_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__ = ({ channelName, kind }) => {
    const channel = mockChannels.find(
      (candidate) => candidate.name === channelName,
    );
    if (!channel) {
  ...
  window.__KURA_E2E_REPLACE_MOCK_TEAM_CATALOG_HEAD__ = (event) => {
    const dTag = event.tags.find((tag) => tag[0] === "d")?.[1];
    const existingIndex = mockTeamCatalogEvents.findIndex(
      (candidate) =>
        candidate.pubkey.toLowerCase() === event.pubkey.toLowerCase() &&
  ...
  window.__KURA_E2E_ACTIVATE_TIMEOUT__ = (expiresAtMs: number) => {
  ...
  window.__KURA_E2E_INVALIDATE_CHANNELS__ = async () => {
    await window.__KURA_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["channels"],
    });
  };
  window.__KURA_E2E_MUTATE_CHANNEL__ = ({
    channelId,
    channelType,
    description,
    removeMemberPubkey,
  ...
  window.__KURA_E2E_GET_EVENT_CALL_COUNT__ = 0;
  window.__KURA_E2E_DEFER_GET_EVENT__ = null;
  deferredGetEventQueue = [];
  deferNextChannelsRead = false;
  deferredChannelsReadResolve = null;
  window.__KURA_E2E_CHANNELS_READ_PENDING__ = 0;
  window.__KURA_E2E_DEFER_NEXT_CHANNELS_READ__ = () => {
    if (deferredChannelsReadResolve) {
      throw new Error("a channel read is already deferred");
    }
    deferNextChannelsRead = true;
  };
  window.__KURA_E2E_RELEASE_CHANNELS_READ__ = () => {
    deferNextChannelsRead = false;
  ...
  window.__KURA_E2E_RELEASE_GET_EVENT__ = () => {
  ...
  window.__KURA_E2E_EMIT_MOCK_READ_STATE__ = ({
    clientId,
    contexts,
    createdAt,
    slotId,
  ...
  window.__KURA_E2E_SET_RELAY_CONNECTION_STATE__ = (state) => {
  ...
  window.__KURA_E2E_QUEUE_AUTH_RESPONSES__ = (responses) => {
    mockAuthResponses.push(...responses);
  };
  window.__KURA_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__ = (reasons) => {
    mockChannelHistoryCloses.push(...reasons);
  };
  window.__KURA_E2E_CLOSE_LIVE_SUBSCRIPTIONS__ = (reason) => {
    let closed = 0;
    for (const socket of mockSockets.values()) {
  ...
  window.__KURA_E2E_SEED_MOCK_REMINDERS__ = (reminders) => {
  ...
  window.__KURA_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__ = (unavailable) => {
  ...
  window.__KURA_E2E_ACTIVATE_RELAY_RATE_LIMIT__ = (seconds) => {
    activateRateLimit(seconds);
  };
  window.__KURA_E2E_RESET_WEBSOCKET_CONNECT_ATTEMPTS__ = () => {
    relayWebsocketConnectAttemptStarts.length = 0;
  };
  window.__KURA_E2E_RELEASE_SEND_MESSAGE_LIVE_ECHO__ = () => {
  ...
  window.__KURA_E2E_SET_MESH__ = (mesh) => {
  ...
  window.__KURA_E2E_SEED_ACTIVE_TURNS__ = ({
    agentPubkey,
    channelId,
    turnId,
    kind = "turn_started",
  ...
  window.__KURA_E2E_SEED_OBSERVER_EVENTS__ = ({ agentPubkey, events }) => {
  ...
  const handleMockCommand = async (
    ...
    window.__KURA_E2E_COMMANDS__?.push(command);
    const loggedPayload = (() => {
    ...
    window.__KURA_E2E_COMMAND_PAYLOADS__?.push({
      command,
      payload: loggedPayload,
    });
    window.__KURA_E2E_COMMAND_LOG__?.push({ command, payload });
    ...
    switch (command) {
      ...
      case "check_builderlab_community_name":
        return {
          available: true,
          normalized_host: `${(payload as { name?: string })?.name ?? "community"}.communities.kura.oute.pro`,
        };
      case "create_builderlab_community": {
        ...
        return {
          community: activeConfig?.mock?.builderlabCreatedCommunity ?? {
            id: `hosted-${name}`,
            name,
            normalized_host: `${name}.communities.kura.oute.pro`,
          },
        };
      }
      ...
      case "sign_out":
        ...
        // assert invocation via __KURA_E2E_COMMANDS__ and the pending UI.
        return;
      ...
      case "get_project_local_repo_snapshot":
        return window.__KURA_E2E_PROJECT_LOCAL_REPO_SNAPSHOT__ ?? null;
      ...
      case "get_project_local_repo_file_content": {
        ...
        return path
          ? (window.__KURA_E2E_PROJECT_REPO_FILE_CONTENTS__?.[path] ?? null)
          : null;
      }
      ...
      case "push_project_local_repository": {
        ...
        const status = window.__KURA_E2E_PROJECT_REPO_SYNC_STATUS__;
      ...
      case "publish_project_owner_announcement": {
        ...
        window.__KURA_E2E_SIGNED_EVENTS__?.push({
          content: event.content,
          createdAt: event.created_at,
          kind: event.kind,
          tags: event.tags,
        ...
        if (
          ...
          const rejectionIndex =
            window.__KURA_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(
              event.kind,
            ) ?? -1;
          if (rejectionIndex >= 0) {
            window.__KURA_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
            ...
            window.__KURA_E2E_ACCEPTED_PROJECT_EVENTS__ = acceptedProjectEvents;
            const failedAckIndex =
              window.__KURA_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.indexOf(
                event.kind,
              ) ?? -1;
            if (failedAckIndex >= 0) {
              window.__KURA_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.splice(
      ...
      case "sign_project_pull_request_status": {
        ...
        window.__KURA_E2E_SIGNED_EVENTS__?.push(event);
        getMockProjectEventStore().push(event);
        return null;
      }
      ...
      case "merge_project_pull_request": {
        ...
        if (window.__KURA_E2E_PROJECT_MERGE_ERROR__) {
          throw window.__KURA_E2E_PROJECT_MERGE_ERROR__;
        }
      ...
      case "get_channels": {
        ...
        if (deferred) {
          await new Promise<void>((resolve) => {
            ...
            window.__KURA_E2E_CHANNELS_READ_PENDING__ = 1;
          });
        }
        return channels;
      }
      ...
      case "export_agent_snapshot":
        ...
        // Specs assert invocation via __KURA_E2E_COMMANDS__.
        return true;
      ...
      case "upload_media_bytes": {
        ...
        if (input.filename?.startsWith("link-preview-")) {
          window.__KURA_E2E_LINK_PREVIEW_UPLOAD_STARTS__ =
            (window.__KURA_E2E_LINK_PREVIEW_UPLOAD_STARTS__ ?? 0) + 1;
        }
      ...
      case "save_agent_card":
        ...
        // command was invoked via `__KURA_E2E_COMMANDS__`, not the dialog.
        return true;
      ...
      case "plugin:webview|set_webview_zoom":
        window.__KURA_E2E_WEBVIEW_ZOOM__ = (payload as { value: number }).value;
        return;
      case "start_pairing": {
      ...
      case "list_save_subscriptions": {
        ...
        if (!win.__KURA_E2E_IPC_COUNTERS__) {
          win.__KURA_E2E_IPC_COUNTERS__ = {};
        }
        const ipcCounters = win.__KURA_E2E_IPC_COUNTERS__ as Record<
          string,
          number
        >;
  ...
  window.__KURA_E2E_INVOKE_MOCK_COMMAND__ = (command, payload) =>
    handleMockCommand(command, payload ?? null);
  window.__KURA_E2E_EMIT_TAURI_EVENT__ = (event, payload) =>
    emit(event, payload);
