...
function AnimatedCountSlot({
  ...
  if (direction === 0 || previous === current) {
    return (
      <span
        className={cn(
          "kura-animated-count__slot",
          isDigit && "kura-animated-count__slot--digit",
        )}
      >
  ...
  if (!isDigit) {
    return (
      <span
        className="kura-animated-count__symbol"
  ...
  return (
    <span
      className="kura-animated-count__slot kura-animated-count__slot--digit"
      data-direction={direction > 0 ? "up" : "down"}
    >
      <span
        className="kura-animated-count__reel"
        key={`${version}-${previous}-${current}`}
      >
        {direction > 0 ? (
          <>
...
export function AnimatedCount({ className, value }: AnimatedCountProps) {
  ...
  return (
    <span className={cn("kura-animated-count", className)}>
      <span className="sr-only">{transition.current}</span>
      <span aria-hidden className="kura-animated-count__motion">
        {slots.map((slot) => (
          <AnimatedCountSlot
      ...
      <span aria-hidden className="kura-animated-count__static">
        {transition.current}
      </span>
    </span>
  );
