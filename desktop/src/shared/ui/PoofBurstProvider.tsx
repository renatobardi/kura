...
export const POOF_TRIGGER_CLASS = "kura-poof-trigger";
export const POOF_ORIGIN_CLASS = "kura-poof-origin";
export const POOF_POINTER_ORIGIN_CLASS = "kura-poof-pointer-origin";
...
export function PoofBurstProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      <div aria-hidden="true" className="kura-poof-layer">
        {bursts.map((burst) => (
          <div
            className="kura-poof-burst"
            key={burst.id}
            style={
              {
                "--kura-poof-size": `${burst.size}px`,
                "--kura-poof-x": `${burst.x}px`,
                "--kura-poof-y": `${burst.y}px`,
              } as PoofStyle
            }
          >
            {POOF_FRAMES.map((frame, index) => (
              <img
                alt=""
                className={`kura-poof-frame kura-poof-frame-${index + 1}`}
                decoding="async"
                draggable={false}
