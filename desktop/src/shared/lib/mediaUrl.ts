 ...
 *
 * Only URLs hosted on the Kura relay are rewritten. External Blossom URLs
...
/**
 * Cached relay origin (e.g. "https://kura-oss.stage.blox.sqprod.co"),
 ...
 * (`normalize_host` in kura-core), but the saved community relay URL keeps
...
/**
 * The relay origin (e.g. `https://kura-oss.stage.blox.sqprod.co`) if it has
...
/**
 * If `url` is a Blossom media URL hosted on the Kura relay, rewrite it
 * to go through the local streaming proxy. External Blossom URLs and
 * non-Blossom URLs are returned unchanged.
 *
 * Falls back to kura-media:// if the proxy port isn't available yet.
 */
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (e.g. wss://PENDING-SEED.communities.kura.oute.pro).
  if (cachedRelayOrigin) {
  ...
  return `kura-media://localhost/media/${m[1]}`;
}
