...
export function resolveLinkPreview(
  ...
  return {
    ...preview,
    snapshotReady: !preview.href.startsWith("kura://"),
...
/**
 * Recipient-side `kura://` entity cards must render even when the relay
...
export function useResolvedLinkPreviews(
  ...
  React.useEffect(() => {
    ...
    if (refetchNewNegatives) {
      ...
      // kura:// entity links resolve off the relay, not this cache — skip them.
    ...
    for (const preview of previews) {
      const loader = preview.href.startsWith("kura://")
