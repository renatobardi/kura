...
const CHANNELS = ["kura-bugs", "kura dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#kura", CHANNELS), {
    query: "kura",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #kura", CHANNELS), {
    query: "kura",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#kura", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#kura de", CHANNELS), {
    query: "kura de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#kura de` — `#` preceded by `x`, no boundary → no query even though
  // "kura de" is a prefix of a known channel.
  assert.equal(at("#", "x#kura de", CHANNELS), null);
});
