...
export function ProjectsView() {
  ...
  // One blobless clone per primary Kura repository, only while the overview
  // header is visible.
  const snapshotProjects = React.useMemo(
    () =>
      filter === "all"
        ? projects.filter(
            (project) =>
              projectRepoHostForProject(project, relayOrigin).kind === "kura",
          )
        : [],
        ...
        >
          ...
          <div className="flex min-h-0 min-w-0 flex-1">
            <div className="relative min-h-0 min-w-0 flex-1">
              ...
              />
              <div
                className="kura-content-scrollbar h-full min-h-0 min-w-0 overflow-x-hidden overflow-y-scroll"
