...
export type Project = {
   ...
   * `kura-related-channel` tags. Client convention: NIP-MP treats the tag as
...
/** Repeatable project tag naming an extra stream besides `kura-channel`. */
export const PROJECT_RELATED_CHANNEL_TAG = "kura-related-channel";
...
export function eventToRepository(
  ...
  const channel = getTag(event, "kura-channel");
  return {
    id: `${owner}:${dtag}`,
    dtag,
...
export function eventToExplicitProject(
  ...
  const rawVisibility = getTag(event, "kura-visibility");
  const visibility =
 ...
 * including Kura's NIP-OA owner delegation for agent-authored events.
 */
