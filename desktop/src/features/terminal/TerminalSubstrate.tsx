...
export function TerminalSubstrate({
  ...
  const [owner, setOwner] = React.useState<"kura" | "terminal">("kura");
  ...
  const [dockHeight, setDockHeight] = React.useState(() => {
    const stored = Number.parseInt(
      window.localStorage.getItem("kura-terminal-dock-height") ?? "",
      10,
    );
  ...
  const terminalStyle = terminalPalette
    ? ({
        "--kura-terminal-background": terminalPalette.background,
        "--kura-terminal-foreground": terminalPalette.foreground,
      } as React.CSSProperties)
    : undefined;

  const forceKuraFallback = React.useEffectEvent(() => {
  ...
  React.useLayoutEffect(() => {
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      ...
      if (onToggle) onToggle();
        ...
        setOwner((current) => {
          const next = current === "terminal" ? "kura" : "terminal";
          if (next === "terminal") {
  ...
  return (
    <section
      ...
      className="kura-terminal-substrate"
    ...
    >
      {mode === "docked" ? (
        <hr
          ...
          className="kura-terminal-resize-handle"
          onKeyDown={(event) => {
      ...
      <div className="kura-terminal-contract-bar">
        <div className="kura-terminal-tabs" role="tablist">
          {sessions.map((session, index) => (
            <div
              className={cn(
                "kura-terminal-tab",
                session.active && "kura-terminal-tab-active",
              )}
              key={session.id}
              role="presentation"
            >
              ...
              <button
                ...
                className="kura-terminal-tab-select"
                disabled={session.closing}
                onClick={() => runTabAction(() => onSelectSession(session.id))}
                role="tab"
                type="button"
              >
                <span className="kura-terminal-designator kura-terminal-tab-title">
                  {session.title === "SHELL" ? (
                    <>
                      <ChevronRight />
                      <span>{index + 1}</span>
                ...
                {session.closing ? (
                  <span className="kura-terminal-tab-title">Closing…</span>
                ) : null}
              </button>
            </div>
          ))}
          <button
            ...
            className="kura-terminal-new-tab"
        ...
        <div className="kura-terminal-readout">
          <button
            aria-label={
              mode === "maximized" ? "Restore Kura Term" : "Maximize Kura Term"
            }
            className="kura-terminal-window-action"
            onClick={() =>
      ...
      <div className="kura-terminal-viewport px-5 pt-2">
        <canvas ref={canvasRef} />
        <div
          aria-hidden="true"
          className="kura-terminal-selection-layer"
          onCopy={(event) => {
        ...
        {welcomeVisible && banner ? (
          <canvas className="kura-terminal-welcome" ref={bannerCanvasRef} />
        ) : null}
        <textarea
          aria-label="Terminal input"
          autoCapitalize="off"
          autoComplete="off"
          className="kura-terminal-input"
          onCompositionEnd={() => {
