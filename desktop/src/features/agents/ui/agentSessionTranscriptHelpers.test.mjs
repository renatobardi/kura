...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "Kura event", there is no event content to surface, so
  // userText is empty and the title falls through to "Kura event".
  ...
  assert.equal(result.userTitle, "Kura event");
...
test("parsePromptText yields a null pubkey when From has no hex", () => {
  const text = ["[Kura event: note]", "From: Someone", "Content: hi"].join(
    "\n",
  );
...
test("parsePromptText defaults the title to 'Kura event' when no kind is present", () => {
  const text = ["[Kura event]", "Content: x"].join("\n");
  const result = parsePromptText(text);
  assert.equal(result.userTitle, "Kura event");
});
...
test("parsePromptText splits a legacy tagged standing prefix from the dynamic turn", () => {
  ...
  assert.deepEqual(
    ...
    ["Base", "System", "Context", "Kura event: @mention"],
  );
});
...
test("extractToolIdentity ignores Kura tool names that only appear in file contents", () => {
...
test("extractToolIdentity still recognizes explicit Kura tool fields", () => {
  const identity = extractToolIdentity({
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
    { title: "Base", body: "You are a helpful AI assistant running in Kura." },
    {
      title: "System",
