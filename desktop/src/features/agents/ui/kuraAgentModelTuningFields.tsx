/**
 * Tier-1 kura-agent model-tuning UI fields.
 *
...
export function EffortSelectField({
  ...
  const effortOptions: AgentDropdownOption[] = [
    ...
    ...KURA_AGENT_THINKING_EFFORT_VALUES.flatMap((v) => {
...
/**
 * Auto-clear hook: resets `KURA_AGENT_THINKING_EFFORT` to "" (Inherit) when
 ...
 *
 * - `maxOutputTokens` / `contextLimit`: minimum 1 — the kura-agent runtime
 *   rejects 0 for these fields (crates/kura-agent/src/config.rs:921-928).
...
export function KuraAgentModelTuningFields({
  ...
  const currentEffort = envVars[KURA_AGENT_THINKING_EFFORT] ?? "";
  useEffortAutoClear({
    currentEffort,
    effortValid,
    onClear: () => onEnvVarChange(KURA_AGENT_THINKING_EFFORT, ""),
  });

  return (
    <div className="space-y-4">
      <p className="text-2xs font-semibold uppercase tracking-wide text-muted-foreground">
        kura-agent model tuning
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <EffortSelectField
            ...
            inheritedEffort={inheritedEnvVars[KURA_AGENT_THINKING_EFFORT]}
            inheritFallbackLabel="Inherit (agent default)"
            label="Thinking / Effort"
            onChange={(value) =>
              onEnvVarChange(KURA_AGENT_THINKING_EFFORT, value)
            }
