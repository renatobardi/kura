...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "kura-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "kura-agent");
  const tail = options.at(-1);
...
test("getDefaultPersonaRuntime ignores an unavailable global preference", () => {
  ...
  assert.equal(getDefaultPersonaRuntime(runtimes, "claude")?.id, "kura-agent");
});

test("getDefaultPersonaRuntime returns kura-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("kura-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "kura-agent");
});

test("getDefaultPersonaRuntime falls back to goose when kura-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("kura-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither kura-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("kura-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for kura-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("kura-agent"), true);
...
test("resetConfigForHarnessChange clears harness-specific values", () => {
  const config = {
    env_vars: { KURA_AGENT_THINKING_EFFORT: "high", KEEP_ME: "yes" },
    model: "claude-opus",
...
test("getPersonaModelOptions for kura-agent with anthropic filters out zero-value default", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("kura-agent", "anthropic");
...
test("getPersonaModelOptions for kura-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("kura-agent", "");
