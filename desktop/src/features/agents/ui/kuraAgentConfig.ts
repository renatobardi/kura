/**
 * Source-of-truth constants for kura-agent model-tuning configuration knobs.
 *
 ...
 * `./modelCapabilities`; this module owns only the kura-agent env-var keys and
 * the runtime-id guard. Mirrors the `config.rs` ⇄ `model_capabilities.rs` seam
 * in `crates/kura-agent`, where effort resolution is delegated to the manifest.
...
export const KURA_AGENT_THINKING_EFFORT = "KURA_AGENT_THINKING_EFFORT";

/** Env var key for the maximum output token count per turn. */
export const KURA_AGENT_MAX_OUTPUT_TOKENS = "KURA_AGENT_MAX_OUTPUT_TOKENS";

/** Env var key for the context window token limit. */
export const KURA_AGENT_MAX_CONTEXT_TOKENS = "KURA_AGENT_MAX_CONTEXT_TOKENS";

/** Env var key for the maximum number of LLM/tool rounds per turn. */
export const KURA_AGENT_MAX_ROUNDS = "KURA_AGENT_MAX_ROUNDS";

/**
 * Ordered set of valid thinking-effort values accepted by kura-agent.
 * Re-exported from the manifest interpreter, which owns the canonical list
 * (mirrors `parse_thinking_effort` in `crates/kura-agent/src/config.rs`).
 */
export const KURA_AGENT_THINKING_EFFORT_VALUES = THINKING_EFFORT_VALUES;
...
/**
 * Returns true when the given runtime id is kura-agent, which is the only
 * runtime that supports the tier-1 model-tuning knobs above.
 */
export function isKuraAgentRuntime(runtimeId: string): boolean {
  return runtimeId === "kura-agent";
}
