...
import {
  ...
  KURA_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isKuraAgentRuntime,
} from "./kuraAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("KURA_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...KURA_AGENT_THINKING_EFFORT_VALUES],
...
test("KURA_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(KURA_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, KURA_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected KURA_AGENT_* names", () => {
  assert.equal(KURA_AGENT_THINKING_EFFORT, "KURA_AGENT_THINKING_EFFORT");
  assert.equal(KURA_AGENT_MAX_OUTPUT_TOKENS, "KURA_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(KURA_AGENT_MAX_CONTEXT_TOKENS, "KURA_AGENT_MAX_CONTEXT_TOKENS");
  assert.equal(KURA_AGENT_MAX_ROUNDS, "KURA_AGENT_MAX_ROUNDS");
});
...
test("isKuraAgentRuntime returns true only for kura-agent id", () => {
  assert.equal(isKuraAgentRuntime("kura-agent"), true);
});

test("isKuraAgentRuntime returns false for other runtimes", () => {
  assert.equal(isKuraAgentRuntime("goose"), false);
  assert.equal(isKuraAgentRuntime("custom"), false);
  assert.equal(isKuraAgentRuntime(""), false);
  assert.equal(isKuraAgentRuntime("kura-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, KURA_AGENT_THINKING_EFFORT, "high");
  assert.equal(result[KURA_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [KURA_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, KURA_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, KURA_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact KURA_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  ...
  assert.equal(result[KURA_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [KURA_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, KURA_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, KURA_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact KURA_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    KURA_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[KURA_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [KURA_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, KURA_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, KURA_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact KURA_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, KURA_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[KURA_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [KURA_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, KURA_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, KURA_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [KURA_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, KURA_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[KURA_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[KURA_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      KURA_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(KURA_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[KURA_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isKuraAgent` derivation in CreateAgentRuntimeFields.
...
test("isKuraAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves kura-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is kura-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "kura-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "kura-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isKuraAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isKuraAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'kura-agent' MUST trigger model-tuning fields",
  );
});

test("isKuraAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned kura-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "kura-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "kura-agent";

  assert.equal(isKuraAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isKuraAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isKuraAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-kura-agent", () => {
  // E.g. user switches from kura-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isKuraAgentRuntime(prospectiveRuntimeId), false);
});

test("isKuraAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
