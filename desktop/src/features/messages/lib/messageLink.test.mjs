...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `kura://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `kura://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `kura://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects kura:// with wrong host", () => {
  const r = parseMessageLink(`kura://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`kura://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`kura://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy kura://message links", () => {
  const r = parseMessageLink(`kura://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("isMessageLink matches kura://message and legacy kura://message", () => {
  assert.equal(
    isMessageLink(`kura://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`kura://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("kura://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("kura://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `kura://message?channel=${CHANNEL}&id=${MESSAGE}`;
