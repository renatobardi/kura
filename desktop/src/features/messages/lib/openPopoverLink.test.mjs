...
test("kura://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(`kura://message?channel=${CHANNEL}&id=${MESSAGE}`, handlers);
...
test("non-message kura:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("kura://channel?foo=bar", handlers);
  assert.deepEqual(external, ["kura://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed kura://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with kura://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("kura://message?nope=1", handlers);
  assert.deepEqual(external, ["kura://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
