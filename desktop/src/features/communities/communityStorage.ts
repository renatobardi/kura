...
const COMMUNITIES_KEY = "kura-communities";
const ACTIVE_COMMUNITY_KEY = "kura-active-community-id";
const LEGACY_WORKSPACES_KEY = "kura-workspaces";
const LEGACY_ACTIVE_WORKSPACE_KEY = "kura-active-workspace-id";
const COMMUNITY_DISCOVERY_AFTER_LEAVE_KEY =
  "kura-community-discovery-after-leave";
...
export function migrateLegacyCommunityStorage(
  ...
  try {
    ...
    // for the origin (block/kura#5078). Fencing here so the app can still
...
export function loadCommunityDiscoveryAfterLeave(
  ...
  try {
    ...
    // block/kura#5078 — storage access can be denied for the origin; degrade
...
export function loadActiveCommunityId(): string | null {
  migrateLegacyCommunityStorage();
  // block/kura#5078 — WebKit can throw SecurityError from a denied-storage
...
export function deriveCommunityName(relayUrl: string): string {
  try {
    ...
    // Detect staging environments (e.g. kura-oss.stage.blox.sqprod.co)
...
export function initFirstCommunity(
  ...
  // block/kura#5078 — read the prior active id through the throw-safe helper;
