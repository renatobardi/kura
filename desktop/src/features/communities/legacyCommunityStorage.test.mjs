...
test("applyLegacyCommunityStorage seeds missing communities and active community", () => {
  ...
  assert.equal(storage.getItem("kura-communities"), legacyCommunities);
  assert.equal(storage.getItem("kura-active-community-id"), "legacy-community");
});

test("applyLegacyCommunityStorage preserves existing non-local Kura communities", () => {
  const storage = createMemoryStorage({
    "kura-communities": currentCommunities,
    "kura-active-community-id": "current-community",
  });
  ...
  assert.equal(storage.getItem("kura-communities"), currentCommunities);
  assert.equal(
    storage.getItem("kura-active-community-id"),
    "current-community",
  );
});
...
test("applyLegacyCommunityStorage replaces broken localhost first-run community", () => {
  const storage = createMemoryStorage({
    "kura-communities": localhostCommunities,
    "kura-active-community-id": "local-community",
  });
...
test("applyLegacyCommunityStorage treats trailing-slash localhost as broken", () => {
  const storage = createMemoryStorage({
    "kura-communities": JSON.stringify([
      {
        id: "local-community",
        name: "Local Dev",
        relayUrl: "ws://localhost:3000/",
...
test("applyLegacyCommunityStorage migrates onboarding completion keys", () => {
  ...
  assert.equal(storage.getItem("kura-onboarding-complete.v1:abc123"), "true");
});
