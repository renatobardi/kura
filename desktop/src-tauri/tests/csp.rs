...
#[test]
fn media_directives_allow_the_kura_media_scheme() {
    // `rewriteRelayUrl` emits `kura-media://localhost/...` until the loopback
    // proxy port resolves, so cold-start media renders through the custom
    // scheme (mapped to `http://kura-media.localhost` on Windows).
    for directive in ["img-src", "media-src", "connect-src"] {
        let allowed = sources(directive);
        assert!(
            allowed.contains(&"kura-media:".to_owned()),
            "{directive} must allow kura-media:"
        );
        assert!(
            allowed.contains(&"http://kura-media.localhost".to_owned()),
            "{directive} must allow http://kura-media.localhost"
        );
    }
}
