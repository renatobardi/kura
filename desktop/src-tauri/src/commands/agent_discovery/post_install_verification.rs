...
pub(super) fn run(
    ...
    // Observe PATH changes and binaries added after Kura launched.
...
fn failure(
    ...
    Some(InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "The installer finished, but Kura still could not use {runtime_id} (observed: {observed})."
        ),
        exit_code: None,
        hint: Some(
            "Kura requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Kura was open, restart Kura and check again."
                .to_string(),
        ),
    })
}
