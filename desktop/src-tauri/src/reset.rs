...
pub(crate) struct ResetContext<'a> {
    ...
    /// Nest dir (`~/.kura` or `~/.kura-dev`) scoped to this build's variant,
...
pub(crate) fn run_boot_reset_with_keychain(ctx: ResetContext<'_>) -> ResetOutcome {
    ...
    // ── Step 3: remove nest, ~/.sprout, ~/.config/kura-agent, CLI symlink ────
    ...
    if let Some(ref home) = ctx.home_dir {
        ...
        let _ = std::fs::remove_dir_all(home.join(".config").join("kura-agent"));
    ...
    if let Err(e) = ctx.keychain.delete_all_with_legacy() {
        eprintln!("kura-desktop reset: keychain delete: {e}");
    ...
    {
        eprintln!(
            "kura-desktop reset: verification failed (keychain_wiped={keychain_ok}, \
    ...
    if let Err(e) = delete_sentinel(app_data_dir) {
        eprintln!("kura-desktop reset: delete sentinel: {e}");
...
mod tests {
    ...
    fn test_dev_build_wipes_dev_nest_not_prod() {
        ...
        // Create both nests.
        let dev_nest = tmp.path().join(".kura-dev");
        let prod_nest = tmp.path().join(".kura");
    ...
    fn test_completed_dev_reset_suppresses_dev_repos_dir_migration() {
        ...
        // Seed prod ~/.kura/.repos-dir so the migration has something to copy.
        let home = tmp.path().join("home");
        let prod_nest = home.join(".kura");
        std::fs::create_dir_all(&prod_nest).unwrap();
        ...
        let dev_nest_2 = tmp.path().join(".kura-dev-control");
        ...
        let dev_nest_3 = tmp.path().join(".kura-dev-prod");
