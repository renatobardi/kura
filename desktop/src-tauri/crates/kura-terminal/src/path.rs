...
//!
//! Kura's own process runs under Hermit activation, so its `PATH` leads with
//! the repo's hermit `bin` and the hermit cache. Inheriting that verbatim
//! hands the user a shell whose `cargo`, `node`, and `python` are Kura's
...
//! answer for the user's machine, not for Kura's build.
//!
...
//! Kura never been in the picture.

/// The default user `PATH` for a spawned shell.
///
/// This intentionally does not consult Kura's own `PATH`. A login shell reads
...
/// normally instead of layering it on top of Kura's build toolchain.
#[cfg(unix)]
