...
//!
//! Kura's own process holds `KURA_PRIVATE_KEY` (an nsec), `KURA_AUTH_TAG`, and
...
/// Keys the child is allowed to inherit from Kura's own environment.
///
...
/// Values Kura sets on the child unconditionally, overriding any inherited
...
/// it is injected rather than inherited. Kura's own `SHELL` and the shell we
...
pub fn fence_env(cmd: &mut CommandBuilder, path: &str, shell: &str) {
    ...
    // 3. Apply Kura's own terminal identity.
