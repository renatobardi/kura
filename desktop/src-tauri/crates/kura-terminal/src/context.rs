...
//! shells interpolate into prompts. A `PS1` containing `$KURA_CHANNEL` turns a
...
/// Returns true if `name` is safe to expose as `KURA_CHANNEL`.
///
...
/// The value to expose as `KURA_CHANNEL`: the name when it is safe, otherwise
/// the channel UUID.
...
/// first `=`. A key `KURA_CHANNEL=x` with value `y` lands as
/// `KURA_CHANNEL=x=y`, so `getenv("KURA_CHANNEL")` returns `"x=y"` — a way to
...
///
/// `KURA_CHANNEL` carries the validated display value; `KURA_CHANNEL_ID` is
...
pub fn context_vars(context: &GuiContext) -> Vec<(&'static str, String)> {
    let mut vars = vec![
        ("KURA_CHANNEL_ID", context.channel_id.clone()),
        ("KURA_CHANNEL", channel_display(context).to_owned()),
        ("KURA_NPUB", context.npub.clone()),
        ("KURA_RELAY_URL", context.relay_url.clone()),
        ("KURA_TERM_SESSION", context.session_id.clone()),
        ("KURA_TERM_VERSION", env!("CARGO_PKG_VERSION").to_owned()),
    ];
    // Absent rather than empty when the user is not in a thread: `-n
    // "$KURA_THREAD_ID"` and `${KURA_THREAD_ID+set}` should agree.
    if let Some(thread_id) = &context.thread_id {
        vars.push(("KURA_THREAD_ID", thread_id.clone()));
    }
    vars
}
