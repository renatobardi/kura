...
use kura_terminal::damage::Encoder;
use kura_terminal::fences::Fences;
use kura_terminal::{SharedTerminal, Size, Terminal};
...
fn flood(shared: &SharedTerminal, stop: &AtomicBool) {
    ...
    while payload.len() < (2 << 20) {
        payload.extend_from_slice(b"\x1b[1;32mkura\x1b[0m substrate line of output 0123456789\r\n");
    }
...
fn measure(fences: Fences) -> kura_terminal::AcquireStats {
    let size = Size {
        columns: 200,
        screen_lines: 50,
        scrollback: 10_000,
