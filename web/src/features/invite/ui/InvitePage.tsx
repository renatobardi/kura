import kuraAppIcon from "@/assets/app-icon@3x.png";
...
export function InvitePage({ code }: { code: string }) {
  ...
  const [downloadUrl, setDownloadUrl] = React.useState(KURA_RELEASES_URL);
  ...
  const openInvite = async () => {
    ...
    try {
      ...
      window.location.href = `kura://join?${query.toString()}`;
    } finally {
      setOpening(false);
    }
  };
    ...
    >
      <div className="w-full max-w-xl space-y-4">
        <div className="flex w-full flex-col items-center rounded-3xl bg-white px-6 py-10 sm:px-12 sm:py-12">
          ...
          >
            <img alt="Kura" className="h-full w-full" src={kuraAppIcon} />
          </div>
          ...
          <div className="mt-9 w-full max-w-md space-y-2">
            ...
            {policy === null ? (
              ...
              >
                <a
                  href={`kura://join?relay=${encodeURIComponent(relay)}&code=${encodeURIComponent(code)}`}
                >
                  Accept invite in Kura
                </a>
              </Button>
            ) : (
              <Button
